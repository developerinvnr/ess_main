...
if($DojY==$CurY)
{ //$Mnt_cal=12; $Qtr_cal=4; $Hlf_cal=2;
  //echo 'bb'.$DojY.'-'.$OldY.'-'.$DojM;
...
*/
 ...
 else if(lgc=='Logic15a')
 {
  ...
  var Per40=Math.round(((tgt*40)/100)*100)/100; var Per60=Math.round((tgt-Per40)*100)/100;
  var Per10=Math.round(((tgt*10)/100)*100)/100; var Per90=Math.round((tgt-Per10)*100)/100;  var Per110=Math.round((tgt+Per10)*100)/100;  
	...
	<tr style="background-color:#FFFFFF;height:24px;">
	 ...
	 <td class="d" align="center">
	  ...
	  /*
		...
		if($PerM==1)
	      { 
		   $sqlchk=mysql_query("select * from hrm_pms_kra_tgtdefin where ".$fn."=".$_REQUEST['kid']." AND NtgtN=".$i,$con); $rrow=mysql_num_rows($sqlchk); if($rrow==0){ $upIn=mysql_query("insert into hrm_pms_kra_tgtdefin(".$fn.", EmployeeID, Tital, Ldate, Wgt, Tgt, NtgtN, Remark) values(".$_REQUEST['kid'].", ".$_REQUEST['eid'].", '".$t."', '".$ld."', '".$wgt."', '".$tgt."', ".$i.", '".$resk['KRA']."')",$con); } 
		  }
		  else
		  {
		   $sqlchk=mysql_query("select * from hrm_pms_kra_tgtdefin where ".$fn."=".$_REQUEST['kid']." AND NtgtN=".$i,$con); $rrow=mysql_num_rows($sqlchk); if($rrow==0){ $upIn=mysql_query("insert into hrm_pms_kra_tgtdefin(".$fn.", EmployeeID, Tital, Ldate, Wgt, Tgt, NtgtN, Remark) values(".$_REQUEST['kid'].", ".$_REQUEST['eid'].", '".$t."', '".$ld."', 0, 0, ".$i.", '')",$con); }
		  }
