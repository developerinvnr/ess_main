...
if($_REQUEST['action']=='PrsPms' AND $_REQUEST['value']=='true')
{
 ...
 { 
  ...
  /*if($row3>0){ $sqlIns=mysql_query("update hrm_employee_pms set EmpCurrGrossPM=".$TotGoss.", EmpCurrAnnualBasic=".$AnnBasic.", EmpCurrCtc=".$TotCtc." where AssessmentYear=".$resSY['CurrY']." AND YearId=".$resSY['CurrY']." AND EmployeeID=".$res['EmployeeID']."", $con); }*/
...
if($_REQUEST['action']=='Newkraentrypro' AND $_REQUEST['value']=='true')
{
 ...
 if($sqlIns){echo '<script>alert("Successfully Curr. Year KRA Entry Process...");</script>';}
}




if($_REQUEST['action']=='NNewkraentryproNewYr' AND $_REQUEST['value']=='true')
{
 $sqlSY=mysql_query("select * from hrm_pms_setting where CompanyId=".$CompanyId." AND Process='KRA'",$con);  
 $sqlSYP=mysql_query("select * from hrm_pms_setting where CompanyId=".$CompanyId." AND Process='PMS'",$con);  
 $resSY=mysql_fetch_array($sqlSY); $resSYP=mysql_fetch_array($sqlSYP);
 ...
 { 
  ...
  $sql2=mysql_query("select Appraiser_EmployeeID,Reviewer_EmployeeID,HOD_EmployeeID from hrm_employee_pms where YearId=".$resSY['CurrY']." AND EmployeeID=".$res['EmployeeID'], $con); $row2=mysql_num_rows($sql2); 
  ...
  $sql3=mysql_query("select EmpPmsId from hrm_employee_pms where YearId=".$resSY['NewY']." AND EmployeeID=".$res['EmployeeID'], $con); $row3=mysql_num_rows($sql3);
	
  if($row3>0){ $sqlIns=mysql_query("update hrm_employee_pms set Appraiser_EmployeeID=".$app.", Reviewer_EmployeeID=".$rev.", HOD_EmployeeID=".$hod.", CompanyId=".$CompanyId." where AssessmentYear=".$resSY['NewY']." AND YearId=".$resSY['NewY']." AND EmployeeID=".$res['EmployeeID']."", $con); }
  elseif($row3==0)
  { 
      $sqlIns=mysql_query("insert into hrm_employee_pms(AssessmentYear, CompanyId, EmployeeID, Appraiser_EmployeeID, Reviewer_EmployeeID, HOD_EmployeeID, YearId) values(".$resSY['NewY'].", ".$CompanyId.", ".$res['EmployeeID'].", ".$app.", ".$rev.", ".$hod.", ".$resSY['NewY'].")", $con); 
          
  }	
 }  
 
 if($sqlIns){echo '<script>alert("Successfully New Year KRA Entry Process...");</script>';}
...
{ agree=confirm("Are you sure, you want to process for (Curr. Year) new KRA entry?");
  if(agree){ window.location="SetKraPms.php?ee=1&aa=1&rr=1&hh=1&form=v%v&action=Newkraentrypro&value=true"; }
  else{ return false; }
}

function NCheckk3(){ if(document.getElementById("checkk3").checked==true){document.getElementById("newK-Process").disabled=false;}else{document.getElementById("newK-Process").disabled=true;} }
function newProcessNeKra() 
{ agree=confirm("Are you sure, you want to process for (New Year) new KRA entry?");
  if(agree){ window.location="SetKraPms.php?ee=1&aa=1&rr=1&hh=1&form=v%v&action=NNewkraentryproNewYr&value=true"; }
  else{ return false; }
}
  ...
  <tr>
    <td align="left" style="width:100%;">
	 <table border="1" style="width:100%;" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
	  <tr><td colspan="15" class="th" style="font-size:16px;">[2] Setting Process Year</b></td></tr>
	  <tr>
	   <td rowspan="2" class="th" style="width:3%;">Sn</td>
	   <td rowspan="2" class="th" style="width:7%;">Process</td>
	   <td rowspan="2" class="th" style="width:3%;">Edit</td>
	   <td rowspan="2" class="th" style="width:5%;">Old<br>YearId</td>
	   <td rowspan="2" class="th" style="width:5%;">Curr<br>YearId</td>
	   <td colspan="2" class="th">New YearId</td>
	   <td rowspan="2" class="th" style="width:10%;">Letter<br>Print Date</td>
	   <td rowspan="2" class="th" style="width:10%;">Letter<br>Effect Date</td>
	   <td rowspan="2" class="th" style="width:8%;">Salary<br>Effect Date</td>
	   <td rowspan="2" class="th" style="width:8%;">Employee Last<br>Joining Date</td>
	   <td rowspan="2" class="th" style="width:4%;">Arrear<br>NOM</td>
	   <td rowspan="2" class="th" style="width:17%;">Wishing Msg</td>
	   <td rowspan="2" class="th" style="width:17%;">Period Msg</td>
	  </tr>
	  <tr>
	   <td class="th" style="width:5%;">ID</td>
	   <td class="th" style="width:5%;">KRA Entry</td>
	  </tr>
...
<?php if($_SESSION['User_Permission']=='Edit'){?>&nbsp;<input type="submit" name="SaveEdit"  value="" class="SaveButton">&nbsp;<?php } ?></td>
	   ...
	   <td class="tdc"><select name="NewY_AllowEntry" id="NewY_AllowEntry" class="tdinput"><option value="<?php echo $resSet['NewY_AllowEntry']; ?>"><?php echo $resSet['NewY_AllowEntry']; ?></option><option value="<?php if($resSet['NewY_AllowEntry']=='Y') echo 'N'; else echo 'Y'; ?>"><?php if($resSet['NewY_AllowEntry']=='Y')echo 'N'; else echo 'Y'; ?></option></select></td>
...
<?php } else { ?>	 
	   <tr style="height:22px;">
		...
		<td class="tdc"><?php echo $resSet['NewY_AllowEntry']; ?></td>
		<?php if($resSet['Process']=='PMS'){ ?>
  ...
  <tr>
   <td align="left" style="width:100%;">
	<table border="1" style="width:50%;" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
	  ...
	  <tr style="height:30px;">
	   <td class="tdc">1</td>
	   <td class="tdl"><b style="font-size:15px;">&nbsp;For <font color="#FF0000">Curr. YearId</font> New KRA Entry</b></td>
...
<?php
$sqlSY=mysql_query("select NewY_AllowEntry from hrm_pms_setting where CompanyId=".$CompanyId." AND Process='KRA'",$con);  
$resSY=mysql_fetch_array($sqlSY); 
?>	  
	  <tr style="height:30px;">
	   <td class="tdc">2</td>
	   <td class="tdl"><b style="font-size:15px;">&nbsp;For <font color="#FF0000">New YearId</font> New KRA Entry</b></td>
	   <td class="tdc"><input type="checkbox" id="checkk3" onClick="NCheckk3()" <?php if($resSY['NewY_AllowEntry']=='N'){echo 'disabled';} ?> />
	   <input type="button" id="newK-Process" onClick="newProcessNeKra()" value="Process" style="width:80px;" disabled/></td>
	  </tr>
	 </table>
   </td>
  </tr>
