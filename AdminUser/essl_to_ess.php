...
foreach($obj->data as $key =>$value)
{
 ...
 if($value->CompanySName=='VSPL' || $value->CompanySName=='VNPL' || $value->CompanySName=='Farm Division')  // || $value->CompanySName=='VFPL'
 {  
   ...
   $com=trim($value->CompanySName); //VSPL VVNR VNPL
   ...
   $dv=''; $comid=0; $YearId=0;
   if($com=='VSPL'){$comid=1;}
   elseif($com=='Farm Division'){$comid=2;}
   elseif($com=='VNPL'){$comid=3;}
   
   $sY=mysqli_query($con,"select YearId from hrm_year where Company".$comid."Status='A'"); 
   $rY=mysqli_fetch_assoc($sY); $YearId=$rY['YearId'];
   
   if($comid==3 OR $comid==2){ $condn="EsslCode=".$ec; }else{ $condn="EmpCode=".$ec; }
   
   //echo "select EmployeeID, TimeApply, InTime, OutTime from hrm_employee where ".$condn." AND CompanyId=".$comid.'<br>';
...
$row2E=mysqli_num_rows($s2E); $r2E=mysqli_fetch_assoc($s2E);
 ...
 if($row2E>0) //Check 3 Open
 {
  ...
  $SL=mysqli_query($con,"select * from hrm_employee_monthlyleave_balance where EmployeeID=".$ei." AND Month='".$mm."' AND Year='".$y."'");  $RowL=mysqli_num_rows($SL); //echo "aa=".$RowL;
  if($RowL>0)
  { 
