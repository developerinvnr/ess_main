...
<?php $sY=mysql_query("select * from hrm_year where YearId=".$_REQUEST['y']."", $con); $rY=mysql_fetch_assoc($sY); $FD=date("Y",strtotime($rY['FromDate'])); $TD=date("Y",strtotime($rY['ToDate'])); $PRD=$FD.'-'.$TD; ?>					
   ...
   <td style=" width:155px;"><?php $Sql=mysql_query("select * from core_departments where id=".$_REQUEST['d'], $con); $Res=mysql_fetch_assoc($Sql); ?><select style="font-size:12px;width:150px;background-color:#DDFFBB;display:block;" name="DepartmentE" id="DepartmentE" onChange="SelectDeptEmp(this.value)"><option value="<?php echo $_REQUEST['d']; ?>" selected><?php echo $Res['department_name']; ?></option><?php $SqlDepartment=mysql_query("
  select * from core_departments where is_active=1 order by department_name", $con); while($ResDepartment=mysql_fetch_array($SqlDepartment)) { ?><option value="<?php echo $ResDepartment['id']; ?>"><?php echo '&nbsp;'.$ResDepartment['department_name'];?></option><?php } ?><option value="999">All</option></select><input type="hidden" name="DeptId" id="DeptId" value="<?php echo $_REQUEST['DPid']; ?>" /></td>
   <td style=" width:150px;"><select style="font-size:12px;width:150px;background-color:#DDFFBB;display:block;" name="AB" id="AB"><option value="<?php echo $_REQUEST['a']; ?>" selected><?php echo $_REQUEST['a']; ?></option><option value="form-A">form-A</option><option value="form-A (SubKRA)">form-A (SubKRA)</option><option value="form-B">form-B</option></select></td>
	...
	<?php } ?>
	<td><input style="font:Times New Roman; width:200px; color:#4A0000; font-size:14px; background-color:#E0DBE3;border-style:none; font-weight:bold;" value="<?php echo $Res['department_name']; ?>" /></td>
...
{ $sql=mysql_query("select Tital,Wgt,Tgt,Ach,Remark,Cmnt,AppAch,AppCmnt,EmpCode,Fname,Sname,Lname,department_code as DepartmentCode from hrm_pms_kra_tgtdefin k left join hrm_pms_kra kr on k.KRAId=kr.KRAId left join hrm_employee e on kr.EmployeeID=e.EmployeeID left join hrm_employee_general g on kr.EmployeeID=g.EmployeeID left join core_departments d on g.DepartmentId=d.id where kr.YearId=".$_REQUEST['y']." AND e.EmpStatus='A' order by e.EmpCode ASC, k.KRAId ASC, Ldate ASC",$con); }
elseif($_REQUEST['a']=='form-A (SubKRA)')
{ $sql=mysql_query("select Tital,Wgt,Tgt,Ach,Remark,Cmnt,AppAch,AppCmnt,EmpCode,Fname,Sname,Lname,department_code as DepartmentCode from hrm_pms_kra_tgtdefin k left join hrm_pms_krasub krs on k.KRASubId=krs.KRASubId left join hrm_pms_kra kr on krs.KRAId=kr.KRAId left join hrm_employee e on kr.EmployeeID=e.EmployeeID left join hrm_employee_general g on kr.EmployeeID=g.EmployeeID left join core_departments d on g.DepartmentId=d.id where kr.YearId=".$_REQUEST['y']." AND e.EmpStatus='A' order by e.EmpCode ASC, k.KRASubId ASC, Ldate ASC",$con); }
elseif($_REQUEST['a']=='form-B')
{ $sql=mysql_query("select Tital,Wgt,Tgt,Ach,Remark,Cmnt,AppAch,AppCmnt,EmpCode,Fname,Sname,Lname,department_code as DepartmentCode from hrm_pms_formb_tgtdefin k left join hrm_employee e on k.EmployeeID=e.EmployeeID left join hrm_employee_general g on k.EmployeeID=g.EmployeeID left join core_departments d on g.DepartmentId=d.id where k.YearId=".$_REQUEST['y']." AND e.EmpStatus='A' order by e.EmpCode ASC, k.FormBId ASC, Ldate ASC",$con); }		  
...
if($_REQUEST['a']=='form-A')
{ $sql=mysql_query("select Tital,Wgt,Tgt,Ach,Remark,Cmnt,AppAch,AppCmnt,EmpCode,Fname,Sname,Lname,department_code as DepartmentCode from hrm_pms_kra_tgtdefin k inner join hrm_pms_kra kr on k.KRAId=kr.KRAId inner join hrm_employee e on kr.EmployeeID=e.EmployeeID inner join hrm_employee_general g on kr.EmployeeID=g.EmployeeID left join core_departments d on g.DepartmentId=d.id where kr.YearId=".$_REQUEST['y']." AND g.DepartmentId=".$_REQUEST['d']." AND e.EmpStatus='A' order by e.EmpCode ASC, k.KRAId ASC, Ldate ASC",$con); }
elseif($_REQUEST['a']=='form-A (SubKRA)')
{ $sql=mysql_query("select Tital,Wgt,Tgt,Ach,Remark,Cmnt,AppAch,AppCmnt,EmpCode,Fname,Sname,Lname,department_code as DepartmentCode from hrm_pms_kra_tgtdefin k inner join hrm_pms_krasub krs on k.KRASubId=krs.KRASubId inner join hrm_pms_kra kr on krs.KRAId=kr.KRAId inner join hrm_employee e on kr.EmployeeID=e.EmployeeID inner join hrm_employee_general g on kr.EmployeeID=g.EmployeeID left join core_departments d on g.DepartmentId=d.id where kr.YearId=".$_REQUEST['y']." AND g.DepartmentId=".$_REQUEST['d']." AND e.EmpStatus='A' order by e.EmpCode ASC, k.KRASubId ASC, Ldate ASC",$con); }
elseif($_REQUEST['a']=='form-B')
{ $sql=mysql_query("select Tital,Wgt,Tgt,Ach,Remark,Cmnt,AppAch,AppCmnt,EmpCode,Fname,Sname,Lname,department_code as DepartmentCode from hrm_pms_formb_tgtdefin k inner join hrm_employee e on k.EmployeeID=e.EmployeeID inner join hrm_employee_general g on k.EmployeeID=g.EmployeeID left join core_departments d on g.DepartmentId=d.id where k.YearId=".$_REQUEST['y']." AND g.DepartmentId=".$_REQUEST['d']." AND e.EmpStatus='A' order by e.EmpCode ASC, k.FormBId ASC, Ldate ASC",$con); }
