...
if ($_REQUEST['action'] = 'export') {
  ...
  if ($_REQUEST['value'] == 'All') {
    ...
    $sql = mysql_query("select s.*,EmpCode,Fname,Sname,Lname,grade_name as GradeValue,designation_name as DesigName,department_name as DepartmentName,VCode from hrm_employee_separation s 
LEFT JOIN hrm_employee e ON s.EmployeeID=e.EmployeeID 
LEFT JOIN hrm_employee_general g on s.EmployeeID=g.EmployeeID 
...
order by s.Emp_ResignationDate DESC", $con);
  ...
  if ($row > 0) {
    ...
    while ($res = mysql_fetch_array($sql)) {
      ...
      $schema_insert .= $res['VCode'] . '' . $res['EmpCode'] . $sep;
      ...
      if ($res['Rep_SaveDate'] != '' and $res['Rep_SaveDate'] != '1970-01-01' and $res['Rep_SaveDate'] != '0000-00-00') {
        ...
        $emp_earlier = new DateTime($EmpSave);
        $rep_later = new DateTime($RepSave);
      ...
      $ne = mysql_query("select FormSubmitDate,RepExtSubmit from hrm_employee_separation_exitint where EmpSepId=" . $res['EmpSepId'], $con);
      $rowe = mysql_num_rows($ne);
      $rese = mysql_fetch_assoc($ne);
      $nr = mysql_query("select NocSubmitDate from hrm_employee_separation_nocrep where EmpSepId=" . $res['EmpSepId'], $con);
      $rowr = mysql_num_rows($nr);
      $resr = mysql_fetch_assoc($nr);
      $ni = mysql_query("select NocSubmitDate from hrm_employee_separation_nocit where EmpSepId=" . $res['EmpSepId'], $con);
      $rowi = mysql_num_rows($ni);
      $resi = mysql_fetch_assoc($ni);
      $nh = mysql_query("select NocSubmitDate from hrm_employee_separation_nochr where EmpSepId=" . $res['EmpSepId'], $con);
      $rowh = mysql_num_rows($nh);
      $resh = mysql_fetch_assoc($nh);
      $na = mysql_query("select NocSubmAccDate from hrm_employee_separation_nocacc where EmpSepId=" . $res['EmpSepId'], $con);
      $rowa = mysql_num_rows($na);
      $resa = mysql_fetch_assoc($na);
      $logistic = mysql_query("SELECT Logistic_Noc_Submit_Date FROM `hrm_employee_separation_nocrep` where EmpSepId=" . $res['EmpSepId'], $con);
      $rowLog = mysql_num_rows($logistic);
