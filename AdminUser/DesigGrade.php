					...
					<tr>
						<td valign="top" align="center" width="100%" id="MainWindow"><br>
							...
							<?php if ($_REQUEST['DPid']) {
								...
								$Sql = mysql_query("select * from hrm_department where DepartmentId=" . $_SESSION['DPid'], $con);
											...
											<tr>
												...
												<td class="td1" style="font-size:11px; width:150px;">
													<select style="font-size:11px; width:150px; height:18px; background-color:#DDFFBB; display:block;" name="DepartmentE" id="DepartmentE" onChange="SelectDeptEmp(this.value)">
														<option value="" style="margin-left:0px; background-color:#84D9D5;" selected>Select Department</option><?php $SqlDepartment = mysql_query("select * from hrm_department where CompanyId=" . $CompanyId . " AND DeptStatus='A' order by DepartmentName ASC", $con);
																																								while ($ResDepartment = mysql_fetch_array($SqlDepartment)) { ?><option value="<?php echo $ResDepartment['DepartmentId']; ?>"><?php echo '&nbsp;' . $ResDepartment['DepartmentCode']; ?></option><?php } ?>
													</select>
													...
													<td align="left" width="1200">
														<table border="1" width="1200" border="1" bgcolor="#FFFFFF">
															...
															<tr bgcolor="#7a6189">
																...
																<?php if ($CompanyId == 1) {
																	$sql = mysql_query("select GradeId from hrm_grade where GradeStatus='A' AND CompanyId=" . $CompanyId . " AND CreatedDate>='2014-02-01'", $con);
																} elseif ($CompanyId == 2 or $CompanyId == 3) {
																	$sql = mysql_query("select GradeId from hrm_grade where GradeStatus='A' AND CompanyId=" . $CompanyId, $con);
																}
																$n = 1;
																...
																<td style="color:#ffffff; font-family:Georgia; font-size:12px; width:80px;" valign="top" align="center"><b>MW</b></td>
															...
															<tr bgcolor="#7a6189">
																...
																<?php if ($CompanyId == 1) {
																	$sqlG = mysql_query("select GradeId,GradeValue from hrm_grade where GradeStatus='A' AND CompanyId=" . $CompanyId . " AND CreatedDate>='2014-02-01'", $con);
																} elseif ($CompanyId == 2 or $CompanyId == 3) {
																	$sqlG = mysql_query("select GradeId,GradeValue from hrm_grade where GradeStatus='A' AND CompanyId=" . $CompanyId, $con);
																}
																$count = 1;
																...
																while ($resGrade = mysql_fetch_array($sqlGrade)) {
																	if ($resGrade['DesigId'] != 69) { ?>
																		<tr>
																			...
																			<?php if ($CompanyId == 1) {
																				$sqlG2 = mysql_query("select GradeId from hrm_grade where GradeStatus='A' AND CompanyId=" . $CompanyId . " AND CreatedDate>='2014-02-01'", $con);
																			} elseif ($CompanyId == 2 or $CompanyId == 3) {
																				$sqlG2 = mysql_query("select GradeId from hrm_grade where GradeStatus='A' AND CompanyId=" . $CompanyId, $con);
																			}
																			$GNo = 1;
																			while ($resG2 = mysql_fetch_array($sqlG2)) { //$resGrade['DesigId']==69
																				$sqlG3 = mysql_query("select DeptGradeDesigId from hrm_deptgradedesig where DesigId=" . $resGrade['DesigId'] . " AND (GradeId=" . $resG2['GradeId'] . " OR GradeId_2=" . $resG2['GradeId'] . " OR GradeId_3=" . $resG2['GradeId'] . " OR GradeId_4=" . $resG2['GradeId'] . " OR GradeId_5=" . $resG2['GradeId'] . ")", $con);
																				$row2 = mysql_num_rows($sqlG3); ?>
																				<td id="<?php echo $_REQUEST['DPid'] . '_' . $resGrade['DesigId'] . '_' . $resG2['GradeId']; ?>" style="width:30px;<?php if ($row2 > 0) {
																																																echo 'background-color:#008000;';
																																															} ?>" align="center"><input type="checkbox" style="height:10px;" name="<?php echo 'FormB' . $Sno . '_' . $GNo; ?>" id="<?php echo 'FormB' . $Sno . '_' . $GNo; ?>" value="<?php echo $resG2['GradeId']; ?>" <?php if ($row2 > 0) {
																																																																																																													echo 'checked';
																																																																																																												} ?> onClick="Click(<?php echo $resGrade['DesigId'] . ',' . $resG2['GradeId'] . ',' . $Sno . ',' . $GNo; ?>)" disabled />
																				</td>
