...
{ 
 ...
 print("\n");
 $sql=mysql_query("select EmpSepId,EmpCode,Fname,Sname,Lname,grade_name as GradeValue,designation_name as DesigName,department_name as DepartmentName,DateJoining,Emp_ResignationDate,HR_RelievingDate, HR_RelievingDate3, HR_RelievingDate2, NoticeBankName, BankName, BranchName, AccountNo, IT_NOC, Rep_NOC, HR_NOC, Acc_NOC, Note  from hrm_employee_separation s INNER JOIN hrm_employee e ON s.EmployeeID=e.EmployeeID INNER JOIN hrm_employee_general g on s.EmployeeID=g.EmployeeID left join core_grades gr on g.GradeId=gr.id left join core_departments d on g.DepartmentId=d.id left join core_designation de on g.DesigId=de.id where s.ResignationStatus=4 AND e.CompanyId=".$_REQUEST['value']." order by s.Emp_ResignationDate DESC", $con); $row=mysql_num_rows($sql);
