...
if(isset($_POST['submit'])) {
    ...
    if(!is_dir("../Employee/HealthIDCard/".$CompanyId."/".$_POST["EC"]))
    {
	 mkdir("../Employee/HealthIDCard/".$CompanyId."/".$_POST["EC"], 0777, true);
    } 
   
  
   
    // Configure upload directory and allowed file types
    $upload_dir = '../Employee/HealthIDCard/'.$CompanyId.'/'.$_POST["EC"].DIRECTORY_SEPARATOR;
		   ...
		   <table border="0" width="100%">
			 <tr><td>
		            ...
		            <tr>
                       ...
                       <select style="font-size:11px; width:120px; height:21px; background-color:#DDFFBB; display:block;" name="DepartmentE" id="DepartmentE" onChange="SelectDeptEmp(this.value)"><?php if($_REQUEST['d'] AND $_REQUEST['d']!='') { if($_REQUEST['d']=='All'){$DN='ALL';} else { $SqlDep=mysql_query("select department_name as DepartmentCode from core_departments where id=".$_REQUEST['d'], $con); $ResDep=mysql_fetch_array($SqlDep); $DN=$ResDep['DepartmentCode']; }?><option value="<?php echo $_REQUEST['d']; ?>"><?php echo '&nbsp;'.$DN;?></option><?php } else { ?><option value="" style="margin-left:0px; background-color:#84D9D5;" selected>Select Department</option><?php } ?>   
...
<table border="1" id="table1" cellspacing="1" style="width:100%;">
 ...
 $sql=mysql_query("select EmpCode,Fname,Sname,Lname,department_code as DepartmentCode from hrm_employee e inner join hrm_employee_general g on e.EmployeeID=g.EmployeeID inner join core_departments d on g.DepartmentId=d.id where e.EmpStatus='A' and ".$deptCon." and e.CompanyId=".$CompanyId." order by e.ECode ASC",$con);   $sn=1; while($res=mysql_fetch_assoc($sql)){?>
 <div class="tbody">
 <tbody>
 ...
 <tr style="background-color:#FFFFFF;height:22px;"> 
  ...
  <td class="tdc" style="color:#FFFFFF;background-color:<?php if(file_exists("../Employee/HealthIDCard/".$CompanyId."/".$res['EmpCode']."/".$res['EmpCode']."_A.pdf")){ echo '#004000';}?>;cursor:pointer;">
  <?php 
    if(file_exists("../Employee/HealthIDCard/".$CompanyId."/".$res['EmpCode']."/".$res['EmpCode']."_A.pdf")){?><a href="<?="../Employee/HealthIDCard/".$CompanyId."/".$res['EmpCode']."/".$res['EmpCode']."_A.pdf";?>" target="_blank" style="color:#FFFFFF;">A</a><?php } 
	 
    if(file_exists("../Employee/HealthIDCard/".$CompanyId."/".$res['EmpCode']."/".$res['EmpCode']."_B.pdf")){?>&nbsp;&nbsp;&nbsp;<a href="<?="../Employee/HealthIDCard/".$CompanyId."/".$res['EmpCode']."/".$res['EmpCode']."_B.pdf";?>" target="_blank" style="color:#FFFFFF;">B</a><?php } 
	 
	if(file_exists("../Employee/HealthIDCard/".$CompanyId."/".$res['EmpCode']."/".$res['EmpCode']."_C.pdf")){?>&nbsp;&nbsp;&nbsp;<a href="<?="../Employee/HealthIDCard/".$CompanyId."/".$res['EmpCode']."/".$res['EmpCode']."_C.pdf";?>" target="_blank" style="color:#FFFFFF;">C</a><?php }  
	
	if(file_exists("../Employee/HealthIDCard/".$CompanyId."/".$res['EmpCode']."/".$res['EmpCode']."_D.pdf")){?>&nbsp;&nbsp;&nbsp;<a href="<?="../Employee/HealthIDCard/".$CompanyId."/".$res['EmpCode']."/".$res['EmpCode']."_D.pdf";?>" target="_blank" style="color:#FFFFFF;">D</a><?php }  
  ?>
  </td>
 </tr>
 </tbody>
