...
<?php if($_REQUEST['value']!='All') { $sqlA=mysql_query("select DepartmentName from hrm_department where DepartmentId=".$_REQUEST['value'], $con); $resA=mysql_fetch_assoc($sqlA); } ?>	
 ...
 <tr bgcolor="#7a6189">
	...
	<td align="center" style="color:#FFFFFF;" class="All_100"><b>Section</b></td>
...
<?php if($_REQUEST['value']=='All') {$sql=mysql_query("select hrm_employee.*, hrm_employee_general.* from hrm_employee_general INNER JOIN hrm_employee ON hrm_employee_general.EmployeeID=hrm_employee.EmployeeID where hrm_employee.CompanyId=".$CompanyId." AND hrm_employee.EmpStatus='A' order by ECode ASC", $con); }
else {$sql=mysql_query("select hrm_employee.*, hrm_employee_general.* from hrm_employee_general INNER JOIN hrm_employee ON hrm_employee_general.EmployeeID=hrm_employee.EmployeeID where hrm_employee_general.DepartmentId=".$_REQUEST['value']." AND hrm_employee.CompanyId=".$CompanyId." AND hrm_employee.EmpStatus='A' order by ECode ASC", $con); } 
...
?> 
     <tr style="background-color:<?php if($rowch>0){echo '#FFFF6C';}else{echo '#FFFFFF';} ?>;">
		...
		<?php $sqlSec=mysql_query("select SectionName from hrm_department_section where SectionId=".$res['EmpSection'], $con); $resSec=mysql_fetch_assoc($sqlSec); ?>
		<td align="" style="background-color:#E8D2FB;" class="All_150" valign="top"><?php echo strtoupper($resSec['SectionName']); ?></td>
