...
if($resE['Gender']=='M'){$M='Mr.';} elseif($resE['Gender']=='F' AND $resE['Married']=='Y'){$M='Mrs.';} elseif($resE['Gender']=='F' AND $resE['Married']=='N'){$M='Miss.';}
	  $sqlDe=mysql_query("select designation_name from core_designation where id=".$resE['DesigId'], $con) or die(mysql_error()); $resDe=mysql_fetch_assoc($sqlDe); ?>	
<option value="<?php echo $resE['EmployeeID']; ?>"><?php echo $resE['EmpCode'].'&nbsp;&nbsp;-&nbsp;&nbsp;'.$M.' '.$Ename.'&nbsp;&nbsp;-&nbsp;&nbsp;('.$resDe['designation_name'].')'; ?></option>
<?php } ?>
</select>
<?php } ?>
