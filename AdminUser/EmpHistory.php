...
<?php $sqlE=mysql_query("select EmpCode, Fname, Sname, Lname, CompanyId, DepartmentId, DesigId, GradeId, HqId, Gender, DR, Married, DateJoining, EmpStatus, DateOfSepration, DOB, EmailId_Vnr, VNRExpYear, TerrId, PreviousExpYear from hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID INNER JOIN hrm_employee_personal ON hrm_employee.EmployeeID=hrm_employee_personal.EmployeeID where hrm_employee.EmployeeID=".$EmployeeId, $con); $resE=mysql_fetch_assoc($sqlE); 
...
$sqlD=mysql_query("select department_name as DepartmentName from core_departments where id=".$resE['DepartmentId'], $con); $resD=mysql_fetch_assoc($sqlD);
$sqlDe=mysql_query("select designation_name as DesigName from core_designation where id=".$resE['DesigId'], $con); $resDe=mysql_fetch_assoc($sqlDe);
$sqlG=mysql_query("select grade_name as GradeValue from core_grades where id=".$resE['GradeId'], $con); $resG=mysql_fetch_assoc($sqlG);
$sqlHq=mysql_query("select city_village_name as HqName from core_city_village_by_state where id=".$resE['HqId'], $con); $resHq=mysql_fetch_assoc($sqlHq); 
$sqlTerr=mysql_query("select territory_name from core_territory where id=".$resE['TerrId'], $con); $resTerr=mysql_fetch_assoc($sqlTerr);

if($resE['TerrId']>0){$Hq=$resTerr['territory_name'];}else{$Hq=$resHq['HqName']; }
...
<?php $sqlQ=mysql_query("select QualificationId from hrm_employee_qualification where EmployeeID=".$EmployeeId." AND (Qualification='10th' OR Qualification='12th' OR Qualification='Graduation' OR Qualification='Post_Graduation') AND Institute!='' AND PassOut!='' AND PassOut!=0", $con); $rowQ=mysql_num_rows($sqlQ); ?>			
			<tr>
			  ...
			  <td class="data" style="text-transform:uppercase;">&nbsp;<?php $sqlQ=mysql_query("select Qualification, Specialization from hrm_employee_qualification where EmployeeID=".$EmployeeId." AND Qualification='10th' AND Institute!='' AND PassOut!='' AND PassOut!=0", $con); $rowQ=mysql_num_rows($sqlQ); $resQ=mysql_fetch_assoc($sqlQ); 
		  $sqlQ2=mysql_query("select Qualification, Specialization from hrm_employee_qualification where EmployeeID=".$EmployeeId." AND Qualification='12th' AND Institute!='' AND PassOut!='' AND PassOut!=0", $con); $rowQ2=mysql_num_rows($sqlQ2); $resQ2=mysql_fetch_assoc($sqlQ2); 
		  $sqlQ3=mysql_query("select Qualification, Specialization from hrm_employee_qualification where EmployeeID=".$EmployeeId." AND Qualification='Graduation' AND Institute!='' AND PassOut!='' AND PassOut!=0", $con); $rowQ3=mysql_num_rows($sqlQ3); $resQ3=mysql_fetch_assoc($sqlQ3);
		  $sqlQ4=mysql_query("select Qualification, Specialization from hrm_employee_qualification where EmployeeID=".$EmployeeId." AND Qualification='Post_Graduation' AND Institute!='' AND PassOut!='' AND PassOut!=0", $con); $rowQ4=mysql_num_rows($sqlQ4); $resQ4=mysql_fetch_assoc($sqlQ4); 
		  if($sqlQ4>0 && $resQ4['Specialization']!=''){ echo $resQ4['Specialization'].', '; } if($sqlQ3>0 && $resQ3['Specialization']!=''){ echo $resQ3['Specialization'].', '; } if($sqlQ2>0 && $resQ2['Specialization']!=''){ echo $resQ2['Qualification'].', '; }if($sqlQ>0 && $resQ['Specialization']!=''){ echo $resQ['Qualification']; }
		  ?>
			  
			  <?php //if($rowQ==1){echo '10th';}elseif($rowQ==2){echo '12th';}elseif($rowQ==3){$sqlQ3=mysql_query("select Specialization from hrm_employee_qualification where EmployeeID=".$EmployeeId." AND Qualification='Graduation'", $con); $resQ3=mysql_fetch_assoc($sqlQ3); echo $resQ3['Specialization'];}elseif($rowQ==4){$sqlQ4=mysql_query("select Specialization from hrm_employee_qualification where EmployeeID=".$EmployeeId." AND Qualification='Post_Graduation'", $con); $resQ4=mysql_fetch_assoc($sqlQ4); echo $resQ4['Specialization'];}
			   //$sqlQ2=mysql_query("select Specialization from hrm_employee_qualification where EmployeeID=".$EmployeeId." AND Qualification!='10th' AND Qualification!='12th' AND Qualification!='Graduation' AND Qualification!='Post_Graduation' AND PassOut!='' AND PassOut!=0", $con); $rowQ2=mysql_num_rows($sqlQ2); if($rowQ2>0){$resQ2=mysql_fetch_assoc($sqlQ2); echo ', '.$resQ2['Specialization'];} ?>
			  </td>
...
<?php $sqlEx=mysql_query("select * from hrm_employee_experience where EmployeeID=".$EmployeeId." order by ExpFromDate DESC", $con); 
      $Sno2=1; while($resEx=mysql_fetch_array($sqlEx)){ ?>	
       <tr bgcolor="#FFFFFF" style="height:22px;">
		...
		<?php if($resEx['ExpFromDate']=='0000-00-00' OR $resEx['ExpFromDate']=='1970-01-01' OR $resEx['ExpFromDate']=='') { echo ''; }else {echo date("d-m-Y",strtotime($resEx['ExpFromDate'])); } ?></td>
		<td align="center" class="data" style="width:80px;">
		<?php if($resEx['ExpToDate']=='0000-00-00' OR $resEx['ExpToDate']=='1970-01-01' OR $resEx['ExpToDate']=='') { echo ''; }else {echo date("d-m-Y",strtotime($resEx['ExpToDate'])); }?></td>
