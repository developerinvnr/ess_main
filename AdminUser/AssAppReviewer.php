		   ...
		   <table border="0">
			 ...
			 <tr><td>
		            ...
		            <tr>
                       ...
                       <select class="tdsel" style="background-color:#DDFFBB;" name="DepartmentE" id="DepartmentE" onChange="SelectDeptEmp(this.value)"><?php if($_REQUEST['DPid']!=''){ $SqlDept=mysql_query("select * from core_departments where id=".$_REQUEST['DPid'], $con); $ResDept=mysql_fetch_array($SqlDept); ?><option value="<?php echo $ResDept['id']; ?>"><?php echo $ResDept['department_name'];?></option><?php }else{ ?><option value="" selected>Department</option><?php } $SqlDepartment=mysql_query("select * from core_departments where is_active=1 order by department_name", $con); while($ResDepartment=mysql_fetch_array($SqlDepartment)) { ?><option value="<?php echo $ResDepartment['id']; ?>"><?php echo $ResDepartment['department_name'];?></option><?php } ?><option value="9999" <?php if($_REQUEST['DPid']==9999){echo 'selected';}?>>All</option></select></td>
					  <td>
...
<table border="1" id="table1" style="width:100%;" cellspacing="1">
 ...
 if($CompanyId==1)
 {  
  if($_REQUEST['tp']==1 AND $_REQUEST['tk']==0){ $sqlDP = mysql_query("SELECT e.EmployeeID,EmpCode,Fname,Sname,Lname,DateJoining,EmpPmsId,Reviewer_EmployeeID,Rev2_EmployeeID,HOD_EmployeeID,Appraiser_EmployeeID,designation_name FROM hrm_employee e left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_pms pms ON e.EmployeeID=pms.EmployeeID left join core_designation de on g.DesigId=de.id WHERE e.EmpStatus='A' AND e.EmpType='E' AND e.CompanyId=".$CompanyId." AND pms.AssessmentYear=".$YId." AND (g.DateJoining<='".$Y."-06-30' OR pms.Appraiser_EmployeeID>0) AND ".$subQD." order by e.ECode ASC", $con); }
  elseif($_REQUEST['tp']==0 AND $_REQUEST['tk']==1){ $sqlDP = mysql_query("SELECT e.EmployeeID,EmpCode,Fname,Sname,Lname,DateJoining,EmpPmsId,Reviewer_EmployeeID,Rev2_EmployeeID,HOD_EmployeeID,Appraiser_EmployeeID,designation_name FROM hrm_employee e left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_pms pms ON e.EmployeeID=pms.EmployeeID left join core_designation de on g.DesigId=de.id WHERE e.EmpStatus='A' AND e.EmpType='E' AND e.CompanyId=".$CompanyId." AND pms.AssessmentYear=".$YId." AND g.DateJoining<='".date("Y-m-d")."' AND ".$subQD." order by e.ECode ASC", $con); }
 }
 else
 { 
...
<?php if($resDP['Appraiser_EmployeeID']!=0){$sqlR = mysql_query("SELECT e.*,DesigId2,Gender,Married,designation_name as DesigCode from hrm_employee e left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join core_designation de on g.DesigId=de.id where e.EmpStatus='A' and e.CompanyId=".$CompanyId." AND e.EmployeeID=".$resDP['Appraiser_EmployeeID'], $con); $resR=mysql_fetch_assoc($sqlR); 
$Name=ucwords(strtolower($resR['Fname'].' '.$resR['Sname'].' '.$resR['Lname'])).'-'.$resR['EmpCode'].'&nbsp;/&nbsp;('.$resR['DesigCode'].')'; 
echo '<option value='.$resDP['Appraiser_EmployeeID'].' selected>'.$Name.'</option>'; }else{ echo '<option value="0">Select Appraiser</option>'; } $sqlApp=mysql_query("select a.*,e.EmployeeID,EmpCode,Fname,Sname,Lname,Gender,Married,designation_name as DesigCode from hrm_pms_appraiser a left join hrm_employee e ON a.EmployeeID=e.EmployeeID left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join core_designation de on g.DesigId=de.id where e.EmpStatus='A' and e.CompanyId=".$CompanyId." AND a.DepartmentId=".$DPid." Order By a.AppraiserId ASC", $con); $no=1; while($resApp=mysql_fetch_array($sqlApp)){ $NameApp=ucwords(strtolower($resApp['Fname'].' '.$resApp['Sname'].' '.$resApp['Lname'])).'-'.$resApp['EmpCode'].'&nbsp;/&nbsp;('.$resApp['DesigCode'].')'; ?> 
...
<?php if($resDP['Reviewer_EmployeeID']!=0){$sqlR = mysql_query("SELECT e.*,DesigId2,Gender,Married,designation_name as DesigCode from hrm_employee e left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join core_designation de on g.DesigId=de.id where e.EmpStatus='A' and e.CompanyId=".$CompanyId." AND e.EmployeeID=".$resDP['Reviewer_EmployeeID'], $con); $resR=mysql_fetch_assoc($sqlR); 
$Name2=ucwords(strtolower($resR['Fname'].' '.$resR['Sname'].' '.$resR['Lname'])).'-'.$resR['EmpCode'].'&nbsp;/&nbsp;('.$resR['DesigCode'].')'; 
echo '<option value='.$resDP['Reviewer_EmployeeID'].' selected>'.$Name2.'</option>'; }else{ echo '<option value="0">Select Reviewer</option>'; } $sqlApp=mysql_query("select a.*,e.EmployeeID,EmpCode,Fname,Sname,Lname,Gender,Married,designation_name as DesigCode from hrm_pms_reviewer a left join hrm_employee e ON a.EmployeeID_UserId=e.EmployeeID left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join core_designation de on g.DesigId=de.id where e.EmpStatus='A' and e.CompanyId=".$CompanyId." AND a.DepartmentId=".$DPid." Order By a.ReviewerId ASC", $con); $no=1; while($resApp=mysql_fetch_array($sqlApp)){ $NameApp=ucwords(strtolower($resApp['Fname'].' '.$resApp['Sname'].' '.$resApp['Lname'])).'-'.$resApp['EmpCode'].'&nbsp;/&nbsp;('.$resApp['DesigCode'].')'; ?> 
<option style="background-color:#FFFFFF;" value="<?php echo $resApp['EmployeeID']; ?>"><?php echo $NameApp;?></option>
<?php $no++; } ?><option value="0">&nbsp;</option></select>
</td>		
...
<?php if($resDP['Rev2_EmployeeID']!=0){$sqlR = mysql_query("SELECT e.*,DesigId2,Gender,Married,designation_name as DesigCode from hrm_employee e left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join core_designation de on g.DesigId=de.id where e.EmpStatus='A' and e.CompanyId=".$CompanyId." AND e.EmployeeID=".$resDP['Rev2_EmployeeID'], $con); $resR=mysql_fetch_assoc($sqlR); 
$Name=ucwords(strtolower($resR['Fname'].' '.$resR['Sname'].' '.$resR['Lname'])).'-'.$resR['EmpCode'].'&nbsp;/&nbsp;('.$resR['DesigCode'].')'; 
echo '<option value='.$resDP['Rev2_EmployeeID'].' selected>'.$Name.'</option>'; }else{ echo '<option value="0">Select HOD</option>'; } $sqlApp=mysql_query("select a.*,e.EmployeeID,EmpCode,Fname,Sname,Lname,Gender,Married,designation_name as DesigCode from hrm_pms_reviewer a left join hrm_employee e ON a.EmployeeID_UserId=e.EmployeeID left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join core_designation de on g.DesigId=de.id where e.EmpStatus='A' and e.CompanyId=".$CompanyId." AND a.DepartmentId=".$DPid." Order By a.ReviewerId ASC", $con); $no=1; while($resApp=mysql_fetch_array($sqlApp)){ $NameApp=ucwords(strtolower($resApp['Fname'].' '.$resApp['Sname'].' '.$resApp['Lname'])).'-'.$resApp['EmpCode'].'&nbsp;/&nbsp;('.$resApp['DesigCode'].')'; ?> 
<option style="background-color:#FFFFFF;" value="<?php echo $resApp['EmployeeID']; ?>"><?php echo $NameApp;?></option>
<?php $no++; } ?><option value="0">&nbsp;</option></select>
</td>
...
<?php if($resDP['HOD_EmployeeID']!=0){$sqlR = mysql_query("SELECT e.*,DesigId2,Gender,Married,designation_name as DesigCode from hrm_employee e left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join core_designation de on g.DesigId=de.id where e.EmpStatus='A' and e.CompanyId=".$CompanyId." AND e.EmployeeID=".$resDP['HOD_EmployeeID'], $con); $resR=mysql_fetch_assoc($sqlR); 
$Name=ucwords(strtolower($resR['Fname'].' '.$resR['Sname'].' '.$resR['Lname'])).'-'.$resR['EmpCode'].'&nbsp;/&nbsp;('.$resR['DesigCode'].')'; 
echo '<option value='.$resDP['HOD_EmployeeID'].' selected>'.$Name.'</option>'; }else{ echo '<option value="0">Select Management</option>'; } $sqlApp=mysql_query("select a.*,e.EmployeeID,EmpCode,Fname,Sname,Lname,Gender,Married,designation_name as DesigCode from hrm_pms_reviewer a left join hrm_employee e ON a.EmployeeID_UserId=e.EmployeeID left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join core_designation de on g.DesigId=de.id where e.EmpStatus='A' and e.CompanyId=".$CompanyId." AND a.DepartmentId=".$DPid." Order By a.ReviewerId ASC", $con); $no=1; while($resApp=mysql_fetch_array($sqlApp)){ $NameApp=ucwords(strtolower($resApp['Fname'].' '.$resApp['Sname'].' '.$resApp['Lname'])).'-'.$resApp['EmpCode'].'&nbsp;/&nbsp;('.$resApp['DesigCode'].')'; ?> 
<option style="background-color:#FFFFFF;" value="<?php echo $resApp['EmployeeID']; ?>"><?php echo $NameApp;?></option>
<?php $no++; } ?><option value="0">&nbsp;</option></select>
