...
$sqlSY=mysql_query("select CurrY,NewY from hrm_pms_setting where CompanyId=".$CompanyId." AND Process='KRA'", $con);
...
<table border="1" id="table1" style="width:100%;" cellspacing="1">
 ...
 <tr>
  <td colspan="11" align="Right" class="fontButton">
   <table border="0">
    <tr><?php //echo $_REQUEST['YeId'].'-'.$resSYP['NewY']; ?>
     <td style="width:90px;" ><input type="button" name="ChangeE" id="ChangeE" style="width:90px;" <?php if($_REQUEST['YeId']==$resSYP['CurrY'] OR $_REQUEST['YeId']==$resSY['CurrY'] OR $_REQUEST['YeId']==$resSY['NewY'] OR $_REQUEST['YeId']==$resSYP['NewY']){echo '';}else{echo 'disabled';} ?> value="Edit" onClick="EditAppRev()"><input type="submit" name="SaveAppRev" id="EditE" style="width:90px;display:none;" value="save"></td>
 ...
 <tr bgcolor="#7a6189">
  ...
  <td class="th" style="width:5%;">Working Day</td> 
 ...
 if($_REQUEST['YId']==1){$Y=2012;}elseif($_REQUEST['YId']==2){$Y=2013;}elseif($_REQUEST['YId']==3){$Y=2014;}elseif($_REQUEST['YId']==4){$Y=2015;}elseif($_REQUEST['YId']==5){$Y=2016;}elseif($_REQUEST['YId']==6){$Y=2017;}elseif($_REQUEST['YId']==7){$Y=2018;}elseif($_REQUEST['YId']==8){$Y=2019;}elseif($_REQUEST['YId']==9){$Y=2020;}elseif($_REQUEST['YId']==10){$Y=2021;}elseif($_REQUEST['YId']==11){$Y=2022;}elseif($_REQUEST['YId']==12){$Y=2023;}elseif($_REQUEST['YId']==13){$Y=2024;} elseif($_REQUEST['YId']==14){$Y=2025;}elseif($_REQUEST['YId']==15){$Y=2026;}
 
 if($YId==1){$Yy=2012;}elseif($YId==2){$Yy=2013;}elseif($YId==3){$Yy=2014;}elseif($YId==4){$Yy=2015;}elseif($YId==5){$Yy=2016;}elseif($YId==6){$Yy=2017;}elseif($YId==7){$Yy=2018;}elseif($YId==8){$Yy=2019;}elseif($YId==9){$Yy=2020;}elseif($YId==10){$Yy=2021;}elseif($YId==11){$Yy=2022;}elseif($YId==12){$Yy=2023;}elseif($YId==13){$Yy=2024;} elseif($YId==14){$Yy=2025;} elseif($YId==15){$Yy=2026;} 
 
 $y1=$Yy; $y2=$Yy-1;
 if($DPid!=9999){ $subQD='g.DepartmentId='.$DPid; }else{ $subQD='1=1'; }
 if($CompanyId==1)
 {  
  if($_REQUEST['tp']==1 AND $_REQUEST['tk']==0){ $sqlDP = mysql_query("SELECT e.EmployeeID,EmpCode,Fname,Sname,Lname,DateJoining,EmpPmsId,Reviewer_EmployeeID,Rev2_EmployeeID,HOD_EmployeeID,Appraiser_EmployeeID,DesigCode FROM hrm_employee e left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_pms pms ON e.EmployeeID=pms.EmployeeID left join hrm_designation de on g.DesigId=de.DesigId WHERE e.EmpStatus='A' AND e.EmpType='E' AND e.CompanyId=".$CompanyId." AND pms.AssessmentYear=".$YId." AND (g.DateJoining<='".$Y."-06-30' OR pms.Appraiser_EmployeeID>0) AND ".$subQD." order by e.ECode ASC", $con); }
  elseif($_REQUEST['tp']==0 AND $_REQUEST['tk']==1){ $sqlDP = mysql_query("SELECT e.EmployeeID,EmpCode,Fname,Sname,Lname,DateJoining,EmpPmsId,Reviewer_EmployeeID,Rev2_EmployeeID,HOD_EmployeeID,Appraiser_EmployeeID,DesigCode FROM hrm_employee e left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_pms pms ON e.EmployeeID=pms.EmployeeID left join hrm_designation de on g.DesigId=de.DesigId WHERE e.EmpStatus='A' AND e.EmpType='E' AND e.CompanyId=".$CompanyId." AND pms.AssessmentYear=".$YId." AND g.DateJoining<='".date("Y-m-d")."' AND ".$subQD." order by e.ECode ASC", $con); }
 }
 else
 { 
     
     //echo "SELECT e.EmployeeID,EmpCode,Fname,Sname,Lname,DateJoining,EmpPmsId,Reviewer_EmployeeID,Rev2_EmployeeID,HOD_EmployeeID,Appraiser_EmployeeID,DesigCode FROM hrm_employee e left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_pms pms ON e.EmployeeID=pms.EmployeeID left join hrm_designation de on g.DesigId=de.DesigId WHERE e.EmpStatus='A' AND e.EmpType='E' AND e.CompanyId=".$CompanyId." AND pms.AssessmentYear=".$YId." AND g.DepartmentId=".$DPid." order by e.EmpCode ASC";
     
  $sqlDP = mysql_query("SELECT e.EmployeeID,EmpCode,Fname,Sname,Lname,DateJoining,EmpPmsId,Reviewer_EmployeeID,Rev2_EmployeeID,HOD_EmployeeID,Appraiser_EmployeeID,DesigCode FROM hrm_employee e left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_pms pms ON e.EmployeeID=pms.EmployeeID left join hrm_designation de on g.DesigId=de.DesigId WHERE e.EmpStatus='A' AND e.EmpType='E' AND e.CompanyId=".$CompanyId." AND pms.AssessmentYear=".$YId." AND ".$subQD." order by e.ECode ASC", $con); 
 }
...
?>
 ...
 <tr bgcolor="#FFFFFF"> 
  ...
  <?php $TotW=0;
  if($resDP['DateJoining']>=date($y2."-07-01") AND $resDP['DateJoining']<=date($y1."-30-06"))
  {
  $CountWL=0; $Count2WL=0;
  $sW=mysql_query("select * from hrm_employee_attendance_".$y2." where EmployeeID=".$resDP['EmployeeID']." AND (AttValue='P' OR AttValue='WFH' OR AttValue='OD') AND AttDate>='".$y2."-07-01' AND AttDate<='".$y2."-12-31' GROUP BY AttDate", $con);
  $sL=mysql_query("select * from hrm_employee_attendance_".$y2." where EmployeeID=".$resDP['EmployeeID']." AND (AttValue='CH' OR AttValue='ACH' OR AttValue='SH' OR AttValue='ASH' OR AttValue='PH' OR AttValue='APH' OR AttValue='HF') AND AttDate>='".$y2."-07-01' AND AttDate<='".$y2."-12-31' GROUP BY AttDate", $con); 
  $rowW=mysql_num_rows($sW); $rL=mysql_num_rows($sL); $rowL=$rL/2; 
  $CountWL=$rowW+$rowL;
  
  $s2W=mysql_query("select * from hrm_employee_attendance_".$y1." where EmployeeID=".$resDP['EmployeeID']." AND (AttValue='P' OR AttValue='WFH' OR AttValue='OD') AND AttDate>='".$y1."-01-01' AND AttDate<='".$y1."-06-30' GROUP BY AttDate", $con);
  $s2L=mysql_query("select * from hrm_employee_attendance_".$y1." where EmployeeID=".$resDP['EmployeeID']." AND (AttValue='CH' OR AttValue='ACH' OR AttValue='SH' OR AttValue='ASH' OR AttValue='PH' OR AttValue='APH' OR AttValue='HF') AND AttDate>='".$y1."-01-01' AND AttDate<='".$y1."-06-30' GROUP BY AttDate", $con); 
  $row2W=mysql_num_rows($s2W); $r2L=mysql_num_rows($s2L); $row2L=$r2L/2; 
  $Count2WL=$row2W+$row2L;
  $TotW=$CountWL+$Count2WL;
  }
  ?>
  <td class="tdc"><?php echo $TotW;?></td>
...
<?php if($resDP['Appraiser_EmployeeID']!=0){$sqlR = mysql_query("SELECT e.*,DesigId2,Gender,Married,DesigCode from hrm_employee e left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join hrm_designation de on g.DesigId=de.DesigId where e.EmpStatus='A' AND e.EmployeeID=".$resDP['Appraiser_EmployeeID'], $con); $resR=mysql_fetch_assoc($sqlR); 
$Name=ucwords(strtolower($resR['Fname'].' '.$resR['Sname'].' '.$resR['Lname'])).'-'.$resR['EmpCode'].'&nbsp;/&nbsp;('.$resR['DesigCode'].')'; 
echo '<option value='.$resDP['Appraiser_EmployeeID'].' selected>'.$Name.'</option>'; }else{ echo '<option value="0">Select Appraiser</option>'; } $sqlApp=mysql_query("select a.*,e.EmployeeID,EmpCode,Fname,Sname,Lname,Gender,Married,DesigCode from hrm_pms_appraiser a left join hrm_employee e ON a.EmployeeID=e.EmployeeID left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join hrm_designation de on g.DesigId=de.DesigId where e.EmpStatus='A' AND a.DepartmentId=".$DPid." Order By a.AppraiserId ASC", $con); $no=1; while($resApp=mysql_fetch_array($sqlApp)){ $NameApp=ucwords(strtolower($resApp['Fname'].' '.$resApp['Sname'].' '.$resApp['Lname'])).'-'.$resApp['EmpCode'].'&nbsp;/&nbsp;('.$resApp['DesigCode'].')'; ?> 
...
<?php if($resDP['Reviewer_EmployeeID']!=0){$sqlR = mysql_query("SELECT e.*,DesigId2,Gender,Married,DesigCode from hrm_employee e left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join hrm_designation de on g.DesigId=de.DesigId where e.EmpStatus='A' AND e.EmployeeID=".$resDP['Reviewer_EmployeeID'], $con); $resR=mysql_fetch_assoc($sqlR); 
$Name2=ucwords(strtolower($resR['Fname'].' '.$resR['Sname'].' '.$resR['Lname'])).'-'.$resR['EmpCode'].'&nbsp;/&nbsp;('.$resR['DesigCode'].')'; 
echo '<option value='.$resDP['Reviewer_EmployeeID'].' selected>'.$Name2.'</option>'; }else{ echo '<option value="0">Select Reviewer</option>'; } $sqlApp=mysql_query("select a.*,e.EmployeeID,EmpCode,Fname,Sname,Lname,Gender,Married,DesigCode from hrm_pms_reviewer a left join hrm_employee e ON a.EmployeeID_UserId=e.EmployeeID left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join hrm_designation de on g.DesigId=de.DesigId where e.EmpStatus='A' AND a.DepartmentId=".$DPid." Order By a.ReviewerId ASC", $con); $no=1; while($resApp=mysql_fetch_array($sqlApp)){ $NameApp=ucwords(strtolower($resApp['Fname'].' '.$resApp['Sname'].' '.$resApp['Lname'])).'-'.$resApp['EmpCode'].'&nbsp;/&nbsp;('.$resApp['DesigCode'].')'; ?> 
<option style="background-color:#FFFFFF;" value="<?php echo $resApp['EmployeeID']; ?>"><?php echo $NameApp;?></option>
<?php $no++; } ?><option value="0">&nbsp;</option></select>
</td>		
...
<?php if($resDP['Rev2_EmployeeID']!=0){$sqlR = mysql_query("SELECT e.*,DesigId2,Gender,Married,DesigCode from hrm_employee e left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join hrm_designation de on g.DesigId=de.DesigId where e.EmpStatus='A' AND e.EmployeeID=".$resDP['Rev2_EmployeeID'], $con); $resR=mysql_fetch_assoc($sqlR); 
$Name=ucwords(strtolower($resR['Fname'].' '.$resR['Sname'].' '.$resR['Lname'])).'-'.$resR['EmpCode'].'&nbsp;/&nbsp;('.$resR['DesigCode'].')'; 
echo '<option value='.$resDP['Rev2_EmployeeID'].' selected>'.$Name.'</option>'; }else{ echo '<option value="0">Select HOD</option>'; } $sqlApp=mysql_query("select a.*,e.EmployeeID,EmpCode,Fname,Sname,Lname,Gender,Married,DesigCode from hrm_pms_reviewer a left join hrm_employee e ON a.EmployeeID_UserId=e.EmployeeID left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join hrm_designation de on g.DesigId=de.DesigId where e.EmpStatus='A' AND a.DepartmentId=".$DPid." Order By a.ReviewerId ASC", $con); $no=1; while($resApp=mysql_fetch_array($sqlApp)){ $NameApp=ucwords(strtolower($resApp['Fname'].' '.$resApp['Sname'].' '.$resApp['Lname'])).'-'.$resApp['EmpCode'].'&nbsp;/&nbsp;('.$resApp['DesigCode'].')'; ?> 
<option style="background-color:#FFFFFF;" value="<?php echo $resApp['EmployeeID']; ?>"><?php echo $NameApp;?></option>
<?php $no++; } ?><option value="0">&nbsp;</option></select>
</td>
...
<?php if($resDP['HOD_EmployeeID']!=0){$sqlR = mysql_query("SELECT e.*,DesigId2,Gender,Married,DesigCode from hrm_employee e left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join hrm_designation de on g.DesigId=de.DesigId where e.EmpStatus='A' AND e.EmployeeID=".$resDP['HOD_EmployeeID'], $con); $resR=mysql_fetch_assoc($sqlR); 
$Name=ucwords(strtolower($resR['Fname'].' '.$resR['Sname'].' '.$resR['Lname'])).'-'.$resR['EmpCode'].'&nbsp;/&nbsp;('.$resR['DesigCode'].')'; 
echo '<option value='.$resDP['HOD_EmployeeID'].' selected>'.$Name.'</option>'; }else{ echo '<option value="0">Select Management</option>'; } $sqlApp=mysql_query("select a.*,e.EmployeeID,EmpCode,Fname,Sname,Lname,Gender,Married,DesigCode from hrm_pms_reviewer a left join hrm_employee e ON a.EmployeeID_UserId=e.EmployeeID left join hrm_employee_general g ON e.EmployeeID=g.EmployeeID left join hrm_employee_personal p ON e.EmployeeID=p.EmployeeID left join hrm_designation de on g.DesigId=de.DesigId where e.EmpStatus='A' AND a.DepartmentId=".$DPid." Order By a.ReviewerId ASC", $con); $no=1; while($resApp=mysql_fetch_array($sqlApp)){ $NameApp=ucwords(strtolower($resApp['Fname'].' '.$resApp['Sname'].' '.$resApp['Lname'])).'-'.$resApp['EmpCode'].'&nbsp;/&nbsp;('.$resApp['DesigCode'].')'; ?> 
<option style="background-color:#FFFFFF;" value="<?php echo $resApp['EmployeeID']; ?>"><?php echo $NameApp;?></option>
<?php $no++; } ?><option value="0">&nbsp;</option></select>
