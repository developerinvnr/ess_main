...
function SelectYear(v,s){window.location='AppsalPmseIncHis.php?YI='+v+'&Sts='+s;}

function SelectSts(s,v){window.location='AppsalPmseIncHis.php?YI='+v+'&Sts='+s;}

function SelectEIncHis(v,yi,e,s)
{ if(e=='d'){var ee='Dept';}else if(e=='a'){var ee='App';}else if(e=='r'){var ee='Rev';}else if(e=='r2'){var ee='Rev2';}else if(e=='h'){var ee='Hod';} 
  ...
  window.location='AppsalPmseIncHis.php?action=EmpIncHis&ee='+ee+'&value='+v+'&YI='+yi+'&Sts='+s+'&wd='+wd;}

function ExportIncHis(v,yi,ee,s)
{ var ComId=document.getElementById("ComId").value; var YId=document.getElementById("YId").value;   
  var wd=document.getElementById("WDate").value;
  window.open("ExportPmsIncHis.php?action=IncHisExport&value="+v+"&c="+ComId+"&YI="+yi+"&ee="+ee+'&Sts='+s+'&wd='+wd,"ExportForm","menubar=yes,scrollbars=yes,resizable=no,directories=no,width=20,height=20");}
...
<td class="td1" style="font-size:14px;width:140px;font-family:Times New Roman;" >&nbsp;&nbsp;<b>Sts:</b>&nbsp;
<select style="font-size:12px; width:98px; height:20px; background-color:#DDFFBB;" name="sts" id="sts" onChange="SelectSts(this.value,<?php echo $_REQUEST['YI']; ?>)"><option value="A" <?php if($_REQUEST['Sts']=='A'){echo 'selected';}?>>&nbsp;Active</option>
   <option value="D" <?php if($_REQUEST['Sts']=='D'){echo 'selected';}?>>&nbsp;Deactive</option><option value="All" <?php if($_REQUEST['Sts']=='All'){echo 'selected';}?>>&nbsp;All</option></select></td>

<?php $sY=mysql_query("select * from hrm_year where YearId=".$_REQUEST['YI']."", $con); $rY=mysql_fetch_assoc($sY); 
$FD=date("Y",strtotime($rY['FromDate'])); $TD=date("Y",strtotime($rY['ToDate'])); $PRD=$FD.'-'.$TD; ?>	     
<td class="td1" style="font-size:14px;width:150px;font-family:Times New Roman;" >&nbsp;&nbsp;<b>Year:</b>&nbsp;
<select class="tdsel" style="background-color:#DDFFBB;width:100px;" name="YearID" id="YearID" onChange="SelectYear(this.value,'<?=$_REQUEST['Sts'] ?>')"><?php if($_REQUEST['YI']!=''){ $SqlY=mysql_query("select * from hrm_year where YearId=".$_REQUEST['YI'], $con); $ResY=mysql_fetch_array($SqlY); ?><option value="<?php echo $ResY['YearId']; ?>"><?php echo date("Y",strtotime($ResY['FromDate'])).'-'.date("Y",strtotime($ResY['ToDate'])); if($ResY['YearId']>5){ echo ' - Y'; }?></option><?php }else{ ?><option value="" selected>Year</option><?php } $SqlYear=mysql_query("select y.YearId,FromDate,ToDate from hrm_employee_pms p left join hrm_year y on p.AssessmentYear=y.YearId where CompanyId=".$CompanyId." group by AssessmentYear order by AssessmentYear DESC", $con); while($ResYear=mysql_fetch_array($SqlYear)) { ?><option value="<?php echo $ResYear['YearId']; ?>"><?php echo date("Y",strtotime($ResYear['FromDate'])).'-'.date("Y",strtotime($ResYear['ToDate'])); if($ResYear['YearId']>5){ echo ' - Y'; } ?></option><?php } ?></select></td>	

<td class="td1" style="font-size:11px; width:120px;" align="center"><select style="font-size:12px; width:118px; height:20px; background-color:#DDFFBB;" name="DeptScore" id="DeptScore" onChange="SelectEIncHis(this.value,<?php echo $_REQUEST['YI']; ?>,'d','<?=$_REQUEST['Sts'] ?>')"><option value="" style="margin-left:0px;" selected>DEPARTMENT</option><?php $SqlDept=mysql_query("select * from hrm_department where CompanyId=".$CompanyId." order by DepartmentName ASC", $con); while($ResDept=mysql_fetch_array($SqlDept)) { ?><option value="<?php echo $ResDept['DepartmentId']; ?>"><?php echo '&nbsp;'.$ResDept['DepartmentCode'];?></option><?php } ?><option value="0">&nbsp;All</option></select></td>
					   
<td class="td1" style="font-size:11px; width:120px;" align="center"><select style="font-size:12px; width:118px; height:20px; background-color:#DDFFBB;" name="AppScore" id="AppScore" onChange="SelectEIncHis(this.value,<?php echo $_REQUEST['YI']; ?>,'a','<?=$_REQUEST['Sts'] ?>')"><option value="" style="margin-left:0px;" selected>APPRAISER</option><?php $SqlHod=mysql_query("SELECT Appraiser_EmployeeID,Fname,Sname,Lname from hrm_employee_pms left JOIN hrm_employee ON hrm_employee_pms.Appraiser_EmployeeID=hrm_employee.EmployeeID WHERE hrm_employee_pms.CompanyId=".$CompanyId." AND hrm_employee_pms.AssessmentYear=".$_REQUEST['YI']." GROUP BY Appraiser_EmployeeID ORDER BY Fname ASC", $con); while($ResHod=mysql_fetch_array($SqlHod)) { $EnameH=$ResHod['Fname'].' '.$ResHod['Sname'].' '.$ResHod['Lname']; ?><option value="<?php echo $ResHod['Appraiser_EmployeeID']; ?>"><?php echo '&nbsp;'.$EnameH; ?></option><?php } ?></select></td>

<td class="td1" style="font-size:11px; width:120px;" align="center"><select style="font-size:12px; width:118px; height:20px; background-color:#DDFFBB;" name="RevScore" id="RevScore" onChange="SelectEIncHis(this.value,<?php echo $_REQUEST['YI']; ?>,'r','<?=$_REQUEST['Sts'] ?>')"><option value="" style="margin-left:0px;" selected>REVIEWER</option><?php $SqlHod=mysql_query("SELECT Reviewer_EmployeeID,Fname,Sname,Lname from hrm_employee_pms left JOIN hrm_employee ON hrm_employee_pms.Reviewer_EmployeeID=hrm_employee.EmployeeID WHERE hrm_employee_pms.CompanyId=".$CompanyId." AND hrm_employee_pms.AssessmentYear=".$_REQUEST['YI']." GROUP BY Reviewer_EmployeeID ORDER BY Fname ASC", $con); while($ResHod=mysql_fetch_array($SqlHod)) { $EnameH=$ResHod['Fname'].' '.$ResHod['Sname'].' '.$ResHod['Lname']; ?><option value="<?php echo $ResHod['Reviewer_EmployeeID']; ?>"><?php echo '&nbsp;'.$EnameH; ?></option><?php } ?></select></td>
		
<td class="td1" style="font-size:11px; width:120px;" align="center"><select style="font-size:12px; width:118px; height:20px; background-color:#DDFFBB;" name="Rev2Score" id="Rev2Score" onChange="SelectEIncHis(this.value,<?php echo $_REQUEST['YI']; ?>,'r2','<?=$_REQUEST['Sts'] ?>')"><option value="" style="margin-left:0px;" selected>HOD</option><?php $Sql2Hod=mysql_query("SELECT Rev2_EmployeeID,Fname,Sname,Lname from hrm_employee_pms left JOIN hrm_employee ON hrm_employee_pms.Rev2_EmployeeID=hrm_employee.EmployeeID WHERE hrm_employee_pms.CompanyId=".$CompanyId." AND hrm_employee_pms.AssessmentYear=".$_REQUEST['YI']." AND `Emp_KRASave`='Y' GROUP BY Rev2_EmployeeID ORDER BY Fname ASC", $con); while($Res2Hod=mysql_fetch_array($Sql2Hod)) { $Ename2H=$Res2Hod['Fname'].' '.$Res2Hod['Sname'].' '.$Res2Hod['Lname']; ?><option value="<?php echo $Res2Hod['Rev2_EmployeeID']; ?>"><?php echo '&nbsp;'.$Ename2H; ?></option><?php } ?></select></td>
					 
<td class="td1" style="font-size:11px; width:120px;" align="center"><select style="font-size:12px; width:118px; height:20px; background-color:#DDFFBB;" name="HodScore" id="HodScore" onChange="SelectEIncHis(this.value,<?php echo $_REQUEST['YI']; ?>,'h','<?=$_REQUEST['Sts'] ?>')"><option value="" style="margin-left:0px;" selected>MANAGEMENT</option><?php $SqlHod=mysql_query("SELECT HOD_EmployeeID,Fname,Sname,Lname from hrm_employee_pms left JOIN hrm_employee ON hrm_employee_pms.HOD_EmployeeID=hrm_employee.EmployeeID WHERE hrm_employee_pms.CompanyId=".$CompanyId." AND hrm_employee_pms.AssessmentYear=".$_REQUEST['YI']." AND `Emp_KRASave`='Y' GROUP BY HOD_EmployeeID ORDER BY Fname ASC", $con); while($ResHod=mysql_fetch_array($SqlHod)) { $EnameH=$ResHod['Fname'].' '.$ResHod['Sname'].' '.$ResHod['Lname']; ?><option value="<?php echo $ResHod['HOD_EmployeeID']; ?>"><?php echo '&nbsp;'.$EnameH; ?></option><?php } ?></select></td>

 <td class="td1" style="font-size:11px; width:120px;" align="center">Without YTD:<input type="checkbox" id="chkWD" onclick="Funcheck()" <?php if($_REQUEST['wd']==1){echo 'checked'; } ?>/><input type="hidden" id="WDate" value="<?php if($_REQUEST['wd']==1){echo 1; }else{echo 0;} ?>"/>
 <script>
  function Funcheck()
  { 
     if(document.getElementById("chkWD").checked==true){ document.getElementById("WDate").value=1; }else{ document.getElementById("WDate").value=0; } 
...
<tr>
 <td>
   ...
   <thead>
     <tr>
  <td colspan="33" valign="top" style=" background-color:#0069D2; font-size:14px; color:#FFFFFF; font-family:Times New Roman; font-weight:bold;">&nbsp;Employee Increment History <?php echo $name; ?> : &nbsp;&nbsp;(&nbsp;<?php echo $name2; ?>&nbsp;)&nbsp;&nbsp;&nbsp;<a href="#" onClick="ExportIncHis(<?php echo $_REQUEST['value']; ?>,<?php echo $_REQUEST['YI']; ?>,'<?php echo $_REQUEST['ee']; ?>','<?php echo $_REQUEST['Sts']; ?>')" style="color:#F9F900; font-size:12px;">Export Excel</a>
	  </td>
	 </tr>
	 <tr bgcolor="#7a6189">
...
{ 
  $wdi="left JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID"; 
...
{  
  if($_REQUEST['value']==0)
  { $sql=mysql_query("select h.*,Fname,Sname,Lname from hrm_pms_appraisal_history h LEFT JOIN hrm_employee e ON h.EmpCode=e.EmpCode where h.CompanyId=".$CompanyId." AND ".$sts." AND e.CompanyId=".$CompanyId." order by e.ECode ASC, h.EmpCode ASC, h.SalaryChange_Date ASC", $con); }
    else{ $sql=mysql_query("select h.*,Fname,Sname,Lname from hrm_pms_appraisal_history h left JOIN hrm_employee e ON h.EmpCode=e.EmpCode ".$wdi." left JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where h.CompanyId=".$CompanyId." AND ".$sts." AND g.DepartmentId=".$_REQUEST['value']." AND ".$wdc." order by e.ECode ASC, h.EmpCode ASC, h.SalaryChange_Date ASC", $con); 
    }
}
elseif($_REQUEST['ee']=='App')
{ $sql=mysql_query("select h.*,Fname,Sname,Lname from hrm_pms_appraisal_history h left JOIN hrm_employee e ON h.EmpCode=e.EmpCode ".$wdi." where h.CompanyId=".$CompanyId." AND ".$sts." AND p.Appraiser_EmployeeID=".$_REQUEST['value']." AND ".$wdc." order by e.ECode ASC, h.EmpCode ASC, h.SalaryChange_Date ASC", $con); }
elseif($_REQUEST['ee']=='Rev')
{ $sql=mysql_query("select h.*,Fname,Sname,Lname from hrm_pms_appraisal_history h left JOIN hrm_employee e ON h.EmpCode=e.EmpCode ".$wdi." where h.CompanyId=".$CompanyId." AND ".$sts." AND p.Reviewer_EmployeeID=".$_REQUEST['value']." AND ".$wdc." order by e.ECode ASC, h.EmpCode ASC, h.SalaryChange_Date ASC", $con); 
}
elseif($_REQUEST['ee']=='Rev2')
{ $sql=mysql_query("select h.*,Fname,Sname,Lname from hrm_pms_appraisal_history h left JOIN hrm_employee e ON h.EmpCode=e.EmpCode ".$wdi." where h.CompanyId=".$CompanyId." AND ".$sts." AND p.Rev2_EmployeeID=".$_REQUEST['value']." AND ".$wdc." order by e.ECode ASC, h.EmpCode ASC, h.SalaryChange_Date ASC", $con); 
}
elseif($_REQUEST['ee']=='Hod')
{ $sql=mysql_query("select h.*,Fname,Sname,Lname from hrm_pms_appraisal_history h left JOIN hrm_employee e ON h.EmpCode=e.EmpCode ".$wdi." where h.CompanyId=".$CompanyId." AND ".$sts." AND p.HOD_EmployeeID=".$_REQUEST['value']." AND ".$wdc." order by e.ECode ASC, h.EmpCode ASC, h.SalaryChange_Date ASC", $con); 
}  
