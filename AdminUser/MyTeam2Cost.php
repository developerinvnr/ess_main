   ...
   <tr>
	...
	<table border="0" style="width:100%;">
	 <tr>
	  ...
	  <td class="tdc" style="width:150px;"><select class="sell" name="DeptInc" id="DeptInc" onChange="SelectDept(this.value)"><option value="" style="margin-left:0px;" <?php if($_REQUEST['action']!='Dept'){echo 'selected';} ?>>Select Department</option><?php $SqlDeptI=mysql_query("select * from core_departments where is_active=1 and id!=18 order by department_name", $con); while($ResDeptI=mysql_fetch_array($SqlDeptI)) { ?><option value="<?php echo $ResDeptI['id']; ?>" <?php if($_REQUEST['action']=='Dept' AND $_REQUEST['value']==$ResDeptI['id']){echo 'selected';} ?>><?php echo '&nbsp;'.$ResDeptI['department_name'];?></option><?php } ?><option value="<?php echo 'Over';  ?>">&nbsp;<?php echo 'All'; ?></option></select></td>	
