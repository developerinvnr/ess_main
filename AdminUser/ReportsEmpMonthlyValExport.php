...
$csv_output .= '"ARR BONUS",';
$csv_output .= '"ARR RA",';
$csv_output .= '"ARR LEAVE ENCASH",';
$csv_output .= '"DEPUTATION ALLOW",';
$csv_output .= '"BONUS ADJUSTMENT",';
...
if($_REQUEST['d']>0){ $sql=mysql_query("select mp.*,EmpCode,Fname,Sname,Lname from ".$PayTable." mp INNER JOIN hrm_employee e ON mp.EmployeeID=e.EmployeeID INNER JOIN hrm_employee_general g ON mp.EmployeeID=g.EmployeeID where e.CompanyId=".$_REQUEST['c']." AND (e.EmpStatus='A' OR (e.EmpStatus='D' AND (e.DateOfSepration='0000-00-00' OR e.DateOfSepration>='".$y."-".$m."-01'))) AND Month=".$m." AND Year=".$y." AND g.DepartmentId=".$_REQUEST['d']." order by ECode ASC", $con); } 
   elseif($_REQUEST['d']==0){ $sql=mysql_query("select mp.*,EmpCode,Fname,Sname,Lname from ".$PayTable." mp INNER JOIN hrm_employee e ON mp.EmployeeID=e.EmployeeID where e.CompanyId=".$_REQUEST['c']." AND (e.EmpStatus='A' OR (e.EmpStatus='D' AND (e.DateOfSepration='0000-00-00' OR e.DateOfSepration>='".$y."-".$m."-01'))) AND Month=".$m." AND Year=".$y." order by ECode ASC, DepartmentId ASC", $con); }
...
$csv_output .= '"'.str_replace('"', '""', $res['Month']).'",';
$csv_output .= '"'.str_replace('"', '""', $res['Year']).'",';
$csv_output .= '"'.str_replace('"', '""', $res['PaidDay']).'",';
...
$csv_output .= '"'.str_replace('"', '""', $res['Arr_Bonus']).'",';
$csv_output .= '"'.str_replace('"', '""', $res['Arr_RA']).'",';
$csv_output .= '"'.str_replace('"', '""', $res['Arr_LvEnCash']).'",';
$csv_output .= '"'.str_replace('"', '""', $res['Deputation_Allow']).'",';
$csv_output .= '"'.str_replace('"', '""', $res['Bonus_Adjustment']).'",';
$csv_output .= '"'.str_replace('"', '""', $res['Communication_Allow']).'",';
$csv_output .= '"'.str_replace('"', '""', $res['Car_Allow']).'",';


$Gross=$res['Basic']+$res['Hra']+$res['Convance']+$res['Bonus_Month']+$res['Special']+$res['DA']+$res['Incentive']+$res['PerformancePay']+$res['LeaveEncash']+$res['VariableAdjustment']+$res['CCA']+$res['RA']+$res['Bonus']+$res['YCea']+$res['YMr']+$res['YLta']+$res['Arr_Basic']+$res['Arr_Hra']+$res['Arr_Conv']+$res['Arr_Spl']+$res['Arr_Bonus']+$res['Arr_RA']+$res['Arr_LvEnCash']+$res['Deputation_Allow']+$res['Bonus_Adjustment']+$res['PP_Inc']+$res['PP_year']+$res['Communication_Allow']+$res['Car_Allow'];
...
$TotDeduct=$res['Tot_Pf_Employee']+$res['Arr_Pf']+$res['ESCI_Employee']+$res['Arr_Esic']+$res['TDS']+$res['VolContrib']+$res['NPS_Value']+$res['DeductAdjmt'];
