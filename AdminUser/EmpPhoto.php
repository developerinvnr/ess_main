...
//**********************************
$SqlEmp = mysql_query("SELECT *,hrm_employee_general.*,hrm_employee_personal.*,hrm_grade.GradeValue,CONCAT(Fname, 
        IF(Sname <> '', CONCAT('_', Sname), ''), 
        IF(Lname <> '', CONCAT('_', Lname), '')) AS FullName FROM hrm_employee INNER JOIN hrm_employee_general ON hrm_employee.EmployeeID=hrm_employee_general.EmployeeID INNER JOIN hrm_employee_personal ON hrm_employee.EmployeeID=hrm_employee_personal.EmployeeID
INNER JOIN hrm_grade ON hrm_grade.GradeId = hrm_employee_general.GradeId
WHERE hrm_employee.EmployeeID=".$EMPID, $con) or die(mysql_error());  $ResEmp=mysql_fetch_assoc($SqlEmp);
$Ename = $ResEmp['Fname'].'&nbsp;'.$ResEmp['Sname'].'&nbsp;'.$ResEmp['Lname']; $LEC=strlen($ResEmp['EmpCode']);  $Grade = $ResEmp['GradeValue'];
   ...
   if($fileSize>1000000)
    {
     ...
     if (($ext == "jpg" || $ext == "jpeg") && ($_FILES["EmpPhoto"]["size"] < 1000000))
      { 
	    ...
	    //For Gradewise upload
	    $newFileName_NameWise = $ResEmp['EmpCode'].'_'.$ResEmp['FullName'].'.'.$extt;
        ...
        if((move_uploaded_file($_FILES['EmpPhoto']['tmp_name'],$newname)))
         { 
		  $SqlInsPhoto = mysql_query("UPDATE hrm_employee_photo SET PhotoType='".$type."',EmpPhoto='".$EmpPhoto."',CreatedBy=".$UserId.",CreatedDate='".date('Y-m-d')."',YearId=".$YearId." WHERE EmployeeID=".$EMPID, $con);
		    ...
		    $originalImage = "EmpImg{$CompanyId}Emp/{$ResEmp['EmpCode']}.jpg";

		   
              $newImage = "EmployeeImage/$Grade/$newFileName_NameWise";
              // Perform file copying, replace if already exists
                if (file_exists($newImage)) {
                    unlink($newImage); // Delete the existing file
                }
