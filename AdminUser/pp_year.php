...
<td class="td1" style="font-size:11px; width:160px;" align="center"><select style="font-size:12px; width:158px; height:20px; background-color:#DDFFBB;" name="DeptInc" id="DeptInc" onChange="FilterSel(this.value,'d')"><option value="" style="margin-left:0px;" selected>SELECT DEPARTMENT</option><?php $SqlDept=mysql_query("select * from core_departments where is_active=1 order by department_name", $con); while($ResDept=mysql_fetch_array($SqlDept)) { ?><option value="<?php echo $ResDept['id']; ?>" <?php if($_REQUEST['d']==$ResDept['id']){echo 'selected';} ?>><?php echo '&nbsp;'.$ResDept['department_name'];?></option><?php } ?><option value="0" <?php if($_REQUEST['d']==0){echo 'selected';} ?>>&nbsp;All</option></select></td>
