...
if($_REQUEST['D']!='All'){ $sqlDP = mysql_query("SELECT e.EmployeeID,EmpCode,Fname,Sname,Lname,EmpStatus,department_name as DepartmentCode,designation_name as DesigName,Gender,Married,DR FROM hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_personal p ON e.EmployeeID=p.EmployeeID LEFT JOIN core_departments d ON g.DepartmentId=d.id LEFT JOIN core_designation de ON g.DesigId=de.id WHERE e.EmpStatus!='De'  AND e.CompanyId=".$CompanyId." AND g.DepartmentId=".$_REQUEST['D']." AND (e.DateOfSepration='0000-00-00' OR e.DateOfSepration='1970-01-01' OR e.DateOfSepration>='".date($_REQUEST['Y']."-".$_REQUEST['m']."-01")."') AND g.DateJoining<='".date($_REQUEST['Y']."-".$_REQUEST['m']."-31")."' order by ECode ASC", $con); }
