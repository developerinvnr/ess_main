...
{
 document.getElementById("DesigNN").value=de;
}


function FValid(formu)
{
 if(document.getElementById("DeptName").value==''){ alert("Department field is required!"); return false; }
 else if(document.getElementById("DesigName").value==''){ alert("Department field is required!"); return false; }
 else if(document.getElementById("GradeName").value==''){ alert("Department field is required!"); return false; }
 else if(document.getElementById("HQName").value==''){ alert("Department field is required!"); return false; }
 else
 {
...
<tr>
<td style="vertical-align:top;width:100%;" colspan="15">
...
<form method="post" name="formu" onsubmit="return FValid(this)">
...
<table style="width:100%;" border="1" cellspacing="0" cellpadding="1">
	<tr style="height:25px;">
		...
		<td class="tdl"><select class="All_200" name="DeptName" id="DeptName" onChange="DeptSelect(this.value)" style="text-transform:uppercase;" required><option value="">Select</option><?php $SqlDept=mysql_query("select * from hrm_department where CompanyId=".$CompanyId." order by DepartmentName ASC", $con); while($ResDept=mysql_fetch_array($SqlDept)) { ?>
  ...
  <option value="<?php echo $resGr['GradeId']; ?>" <?php if($res['New_Grade']==$resGr['GradeId']){echo 'selected';}?>><?php echo $resGr['GradeValue']; ?></option><?php } ?>
	...
	<tr style="height:25px;">
		<td class="stdl">HQ</td>
		<td class="tdl"><select class="All_200" name="HQName" id="HQName" onChange="HQSelect(this.value)" required><option value="">Select</option><?php $SqlHQ=mysql_query("select * from hrm_headquater where HQStatus='A' AND CompanyId=".$CompanyId." order by HqName ASC", $con); while($ResHQ=mysql_fetch_array($SqlHQ)) { ?>
  <option value="<?php echo $ResHQ['HqId']; ?>" <?php if($res['New_Hq']==$ResHQ['HqId']){echo 'selected';}?>><?php echo $ResHQ['HqName']; ?></option><?php } ?></select></td>
	...
	<tr style="height:25px;">
		...
		<td class="tdl"><input name="DOE" id="DOE" class="All_100" value="<?php if($res['DOE']>'2023-01-01'){ echo date("d-m-Y",strtotime($res['DOE'])); } ?>" readonly><button id="f_btn1" class="CalenderButton"></td>
	</tr>
	<script type="text/javascript">  var cal = Calendar.setup({ onSelect:  function(cal) { cal.hide()}, showTime: true }); 
  cal.manageFields("f_btn1", "DOE", "%d-%m-%Y"); </script>
	
	<tr style="height:25px;">
	    <td colspan="2" class="stdl" style="text-align:center;">
	<input type="hidden" name="DesigNN" id="DesigNN" value="" />
