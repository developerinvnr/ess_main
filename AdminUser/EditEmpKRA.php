...
<?php $sql=mysql_query("select * from hrm_pms_kra where YearId=".$_REQUEST['YeId']." AND CompanyId=".$CompanyId." AND EmployeeID=".$_REQUEST['e']." order by KRAId ASC", $con); $row=mysql_num_rows($sql); ?>
	  ...
	  <?php if($_REQUEST['YeId']==$resSY['CurrY'] OR $_REQUEST['YeId']==$resSYP['CurrY'] OR $_REQUEST['YeId']==$resSY['NewY']){ ?>
...
<?php $sSubK=mysql_query("select * from hrm_pms_krasub where KRAId=".$res['KRAId']." AND KSubStatus='A' order by KRASubId ASC",$con); $rowSubK=mysql_num_rows($sSubK); ?>  
  ...
  <td align="center"><?php if($rowSubK==0){?><select name="Logic_<?php echo $k; ?>" id="Logic_<?php echo $k; ?>" class="Input" style="width:68px; height:20px;" readonly><option value="<?php echo $res['Logic']; ?>"><?php echo $res['Logic']; ?></option>
  <?php foreach($LogArr as $logic){ ?>
   <option value="<?=$logic['logicMn']?>"><?=$logic['logicMn_for']?></option>
  <?php } ?>
  
  </select><?php }else{?><input type="" style="width:68px;height:20px;border:hidden;" name="Logic_<?php echo $k; ?>" id="Logic_<?php echo $k; ?>" value=""><?php } ?></td>
...
<?php /* While Open*/ $m=1; while($rSubK=mysql_fetch_assoc($sSubK)){ ?>
  ...
  <td align="center" style="background-color:#CAE4FF;"><select id="Log_a<?php echo $k.'_'.$m; ?>" class="Inputa" style="width:70px; height:20px;"><option value="<?php echo $rSubK['Logic']; ?>" selected><?php echo $rSubK['Logic']; ?></option>
  <?php foreach($LogArr as $logic){ ?>
   <option value="<?=$logic['logicMn']?>"><?=$logic['logicMn_for']?></option>
  <?php } ?>
...
<?php for($mm=$m; $mm<=5; $mm++){ ?>
  ...
  <td align="center" style="background-color:#CAE4FF;"><select id="Log_a<?php echo $k.'_'.$mm; ?>" class="Inputa" style="width:70px; height:20px;">
  <?php foreach($LogArr as $logic){ ?>
   <option value="<?=$logic['logicMn']?>"><?=$logic['logicMn_for']?></option>
  <?php } ?>
...
<tr style="height:22px;" id="TR_<?php echo $i; ?>">	
  ...
  <td align="center"><select name="Logic_<?php echo $i; ?>" id="Logic_<?php echo $i; ?>" class="Input" style="width:68px; height:20px;" onChange="ChangeLogic(<?php echo $i; ?>)">
  <?php foreach($LogArr as $logic){ ?>
   <option value="<?=$logic['logicMn']?>"><?=$logic['logicMn_for']?></option>
  <?php } ?>
