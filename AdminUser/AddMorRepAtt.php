...
{ 
  ...
  elseif($_POST['Att']=='' AND $ei!='' AND $_POST['RDate']!='')
  {
   ...
   $SqlLWP=mysql_query("select count(DISTINCT AttDate)as LWP from ".$AttTable." where AttValue='LWP' AND AND EmployeeID=".$EMPID." AND (AttDate between '".date($Y."-".$m."-1")."' AND '".date($Y."-".$m."-31")."')", $con); $ResLWP=mysql_fetch_array($SqlLWP); 
   $TotalLWP = $ResLWP['LWP'];
