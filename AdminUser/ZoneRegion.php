...
function FunExport(c)
{ 
  window.open("ZoneRegionExport.php?action=HqRegExpor&c=1&v=true","PrintForm","menubar=yes,scrollbars=yes,resizable=no,directories=no,width=20,height=20");}
...
<?php if($_SESSION['login'] = true AND ($_SESSION['Master']==1 OR $_SESSION['Mas_DetailMand_HQ']==1)){ ?>	
 <tr>
...
<option value="">Select Department</option>
  <?php $sqlSt=mysql_query("select * from hrm_department where DeptStatus='A' AND CompanyId=".$CompanyId." order by DepartmentName ASC", $con); 
      while($resSt=mysql_fetch_array($sqlSt)){ ?><option value="<?=$resSt['DepartmentId']?>" <?php if($_REQUEST['vt_v']==$resSt['DepartmentId']){ echo 'selected';} ?>)>&nbsp;<?=$resSt['DepartmentName']?></option><?php } ?>
          ...
          &nbsp;&nbsp;<a href="#" onClick="FunExport(<?=$CompanyId?>)" style="font-size:12px;">Export All</a>
...
<form name="formEdit" method="post" onSubmit="return validate2Edit(this)">	
 <tr style="height:24px;">
  ...
  <td class="tdc"><input name="Vertical" id="Vertical" class="EditInput" style="width:99%;border:hidden;" value="<?php echo $resRat['VerticalName']; ?>" readonly/><input type="hidden" name="VerticalId<?=$SNo?>" id="VerticalId<?=$SNo?>" value="<?php echo $resRat['EmpVertical']; ?>" /></td>
