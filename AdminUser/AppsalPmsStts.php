...
function SelectFunAppRev(v,y){ window.location='AppsalPmsStts.php?actty=true&opt=false&vlu=45&sec=comback&ee=%rr%&ff=true&r2r=true&action=Fun&value='+v+'&YI='+y;}
...
<?php if($_REQUEST['YI']!=''){ $sY=mysql_query("select * from hrm_year where YearId=".$_REQUEST['YI']."", $con); $rY=mysql_fetch_assoc($sY); $FD=date("Y",strtotime($rY['FromDate'])); $TD=date("Y",strtotime($rY['ToDate'])); $PRD=$FD.'-'.$TD; }?>	     
	...
	<td class="td1" style="width:150px;"><select class="tdsel" style="background-color:#DDFFBB;width:150px;" name="FunAppRev" id="FunAppRev" onChange="SelectFunAppRev(this.value,<?php echo $_REQUEST['YI'];?>)" <?php if($_REQUEST['YI']==''){echo 'disabled';}?>><option value="" style="margin-left:0px;" selected>SELECT FUNCTION</option><?php $SqlFun=mysql_query("select * from core_functions where is_active=1 order by function_name", $con); while($ResFun=mysql_fetch_array($SqlFun)) { ?><option value="<?php echo $ResFun['id']; ?>"><?php echo $ResFun['function_name'];?></option><?php } ?><option value="All">&nbsp;All</option></select></td>
	
	<td class="td1" style="width:150px;"><select class="tdsel" style="background-color:#DDFFBB;width:150px;" name="DeptAppRev" id="DeptAppRev" onChange="SelectAppRev(this.value,<?php echo $_REQUEST['YI'];?>)" <?php if($_REQUEST['YI']==''){echo 'disabled';}?>><option value="" style="margin-left:0px;" selected>SELECT DEPARTMENT</option><?php $SqlDept=mysql_query("select * from core_departments where is_active=1 order by department_name", $con); while($ResDept=mysql_fetch_array($SqlDept)) { ?><option value="<?php echo $ResDept['id']; ?>"><?php echo $ResDept['department_name'];?></option><?php } ?><option value="All">&nbsp;All</option></select></td>
...
<td colspan="20" valign="top" style=" background-color:#0069D2; font-size:14px; color:#FFFFFF; font-family:Georgia; font-weight:bold;">&nbsp;PMS Status Appraiser Wise :&nbsp;&nbsp;(&nbsp;Appraiser - <?php echo $resA['Fname'].' '.$resA['Sname'].' '.$resA['Lname']; ?>&nbsp;)&nbsp;&nbsp;&nbsp;</td>
...
<td colspan="20" valign="top" style=" background-color:#0069D2; font-size:14px; color:#FFFFFF; font-family:Georgia; font-weight:bold;">&nbsp;PMS Status Reviewer Wise :&nbsp;&nbsp;(&nbsp;Reviewer - <?php echo $resA['Fname'].' '.$resA['Sname'].' '.$resA['Lname']; ?>&nbsp;)&nbsp;&nbsp;&nbsp;</td>
...
<td colspan="20" valign="top" style=" background-color:#0069D2; font-size:14px; color:#FFFFFF; font-family:Georgia; font-weight:bold;">&nbsp;PMS Status HOD Wise :&nbsp;&nbsp;(&nbsp;HOD - <?php echo $resA['Fname'].' '.$resA['Sname'].' '.$resA['Lname']; ?>&nbsp;)&nbsp;&nbsp;&nbsp;</td>
<?php } ?>
</tr>
<tr bgcolor="#7a6189">
...
<?php 
if($_REQUEST['action']=='Fun')
{ 
  if($_REQUEST['value']>0){ $SubF="g.EmpFunction=".$_REQUEST['value']; } 
  else{ $SubF="1=1"; }
}
else{ $SubF="1=1"; }
...
{ 
  if($_REQUEST['value']>0){ $SubD="g.DepartmentId=".$_REQUEST['value']; } 
  else{ $SubD="1=1"; }
}
else{ $SubD="1=1"; }

if($_REQUEST['action']=='APPW'){ $SubRep="p.Appraiser_EmployeeID=".$_REQUEST['value']; }
elseif($_REQUEST['action']=='REVW'){ $SubRep="p.Reviewer_EmployeeID=".$_REQUEST['value']; }
elseif($_REQUEST['action']=='HODW'){ $SubRep="p.HOD_EmployeeID=".$_REQUEST['value']; }
else{ $SubRep="1=1"; }

$sql = mysql_query("SELECT e.EmployeeID, EmpCode, Fname, Sname, Lname, function_name, department_name as DepartmentCode, Appraiser_EmployeeID, Reviewer_EmployeeID, Rev2_EmployeeID, Rev2_PmsStatus, HOD_EmployeeID, Emp_PmsStatus, Appraiser_PmsStatus, Reviewer_PmsStatus, HodSubmit_IncStatus, HR_PmsStatus, ExtraAllowPMS FROM hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN core_departments d ON g.DepartmentId=d.id INNER JOIN core_functions f ON g.EmpFunction=f.id WHERE (e.EmpStatus='A' OR p.Appraiser_TotalFinalScore>0) AND g.DateJoining<='".$YYear."-09-30' AND p.AssessmentYear=".$_REQUEST['YI']." AND e.CompanyId=".$CompanyId." AND ".$SubD." AND ".$SubF." AND ".$SubRep." order by e.ECode ASC", $con);

/*
if($_REQUEST['action']=='DW') { if($_REQUEST['value']=='All'){ $sql = mysql_query("SELECT e.EmployeeID, EmpCode, Fname, Sname, Lname, department_name as DepartmentCode, Appraiser_EmployeeID, Reviewer_EmployeeID, Rev2_EmployeeID, Rev2_PmsStatus, HOD_EmployeeID, Emp_PmsStatus, Appraiser_PmsStatus, Reviewer_PmsStatus, HodSubmit_IncStatus, HR_PmsStatus, ExtraAllowPMS FROM hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN core_departments d ON g.DepartmentId=d.id WHERE (e.EmpStatus='A' OR p.Appraiser_TotalFinalScore>0) AND g.DateJoining<='".$YYear."-09-30' AND p.AssessmentYear=".$_REQUEST['YI']." AND e.CompanyId=".$CompanyId." AND p.Appraiser_EmployeeID!=0 order by e.ECode ASC", $con); }
else { $sql = mysql_query("SELECT e.EmployeeID, EmpCode, Fname, Sname, Lname, department_name as DepartmentCode, Appraiser_EmployeeID, Reviewer_EmployeeID, HOD_EmployeeID, Emp_PmsStatus, Appraiser_PmsStatus, Reviewer_PmsStatus, Rev2_EmployeeID, Rev2_PmsStatus, HodSubmit_IncStatus, HR_PmsStatus, ExtraAllowPMS FROM hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN core_departments d ON g.DepartmentId=d.id WHERE (e.EmpStatus='A' OR p.Appraiser_TotalFinalScore>0) AND g.DateJoining<='".$YYear."-09-30' AND p.AssessmentYear=".$_REQUEST['YI']." AND e.CompanyId=".$CompanyId." AND g.DepartmentId=".$_REQUEST['value']." AND p.Appraiser_EmployeeID!=0 order by e.ECode ASC", $con); }

} if($_REQUEST['action']=='APPW'){ $sql = mysql_query("SELECT e.EmployeeID, EmpCode, Fname, Sname, Lname, department_name as DepartmentCode, Appraiser_EmployeeID, Reviewer_EmployeeID, HOD_EmployeeID, Emp_PmsStatus, Appraiser_PmsStatus, Reviewer_PmsStatus, Rev2_EmployeeID, Rev2_PmsStatus, HodSubmit_IncStatus, HR_PmsStatus, ExtraAllowPMS FROM hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN core_departments d ON g.DepartmentId=d.id WHERE (e.EmpStatus='A' OR p.Appraiser_TotalFinalScore>0) AND g.DateJoining<='".$YYear."-09-30' AND p.AssessmentYear=".$_REQUEST['YI']." AND e.CompanyId=".$CompanyId." AND p.Appraiser_EmployeeID=".$_REQUEST['value'], $con);

} if($_REQUEST['action']=='REVW'){ $sql = mysql_query("SELECT e.EmployeeID, EmpCode, Fname, Sname, Lname, DepartmentCode, Appraiser_EmployeeID, Reviewer_EmployeeID, HOD_EmployeeID, Emp_PmsStatus, Appraiser_PmsStatus, Reviewer_PmsStatus, Rev2_EmployeeID, Rev2_PmsStatus, HodSubmit_IncStatus, HR_PmsStatus, ExtraAllowPMS FROM hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN core_departments d ON g.DepartmentId=d.id WHERE (e.EmpStatus='A' OR p.Appraiser_TotalFinalScore>0) AND g.DateJoining<='".$YYear."-09-30' AND p.AssessmentYear=".$_REQUEST['YI']." AND e.CompanyId=".$CompanyId." AND p.Reviewer_EmployeeID=".$_REQUEST['value'], $con);

} if($_REQUEST['action']=='HODW'){ $sql = mysql_query("SELECT e.EmployeeID, EmpCode, Fname, Sname, Lname, department_name as DepartmentCode, Appraiser_EmployeeID, Reviewer_EmployeeID, HOD_EmployeeID, Emp_PmsStatus, Appraiser_PmsStatus, Reviewer_PmsStatus, Rev2_EmployeeID, Rev2_PmsStatus, HodSubmit_IncStatus, HR_PmsStatus, ExtraAllowPMS FROM hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN core_departments d ON g.DepartmentId=d.id WHERE (e.EmpStatus='A' OR p.Appraiser_TotalFinalScore>0) AND g.DateJoining<='".$YYear."-09-30' AND p.AssessmentYear=".$_REQUEST['YI']." AND e.CompanyId=".$CompanyId." AND p.HOD_EmployeeID=".$_REQUEST['value'], $con); }
*/
