...
$sMain=mysql_query("select e.EmployeeID from hrm_employee e inner join hrm_employee_general g on e.EmployeeID=g.EmployeeID inner join hrm_employee_pms p on e.EmployeeID=p.EmployeeID where e.EmpStatus='A' AND g.DateJoining<='".$rSet['AllowEmpDoj']."' AND p.HOD_EmployeeID>0 AND p.AssessmentYear=".$rSet['CurrY']." AND ".$qry." AND e.CompanyId=".$_REQUEST['C'],$con);
	  ...
	  <td style="font-family:Times New Roman;color:#000000; width:785;" align="center">
	    <table border="0">
		  ...
		  <tr>
		   ...
		   <td style="width:50px;">&nbsp;</td><td style="width:685px;font-size:16px;color:#660000;font-weight:bold;">Function:&nbsp;<?php echo $resDept['FunName']; ?></td>    
		      
		  <?php /*<td style="width:50px;">&nbsp;</td><td style="width:685px;font-size:16px;color:#660000;font-weight:bold;">Dept:&nbsp;<?php echo $resDept['DepartmentName']; ?></td>*/ ?>
...
$SqlEligEmp = mysql_query("SELECT * FROM hrm_employee_eligibility_pms WHERE EmployeeID=".$ENew." AND Status='A' order by EligibilityId DESC limit 0,1", $con) or die(mysql_error());  $ResEligEmp=mysql_fetch_assoc($SqlEligEmp); 
$SqlCtc = mysql_query("SELECT ESCI FROM hrm_employee_ctc_pms WHERE EmployeeID=".$ENew." AND Status='A' order by CtcId DESC limit 0,1", $con); $ResCtc=mysql_fetch_assoc($SqlCtc);
