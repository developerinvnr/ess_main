  ...
  <td align="left" height="20" valign="top" colspan="3">
   <table border="0">
    <tr>
	  ...
	  <option value="12" <?php if($_REQUEST['t']==12){echo 'selected';}?>>Group Term Insurance</option>
	  </select> </td>
	  <td align="left">
...
<!-------------------------------------->
 <table border="0" cellpadding="0" cellspacing="4" cellpadding="0"> 
   <tr> 
<?php $sqlDe=mysql_query("select * from core_departments where is_active=1 order by department_name", $con); $no=0; $rowDe=mysql_num_rows($sqlDe); while($resDe=mysql_fetch_array($sqlDe)){ $no=$no+1; //AND DepartmentName!='Management' ?>		
	...
	<input type="checkbox" id="Chk_<?php echo $no; ?>" <?php if($_REQUEST['dp'.$no]>0){echo 'checked';} ?> onClick="ClickChkHq(<?php echo $no.','.$resDe['id']; ?>)" />&nbsp;<?php echo substr_replace($resDe['department_code'], '', 10); ?></td>
...
<?php if($_SESSION['login'] = true) { ?>	
 <tr>
<?php $Dpt="AND (id='".$_REQUEST['dp1']."' OR id='".$_REQUEST['dp2']."' OR id='".$_REQUEST['dp3']."' OR id='".$_REQUEST['dp4']."' OR id='".$_REQUEST['dp5']."' OR id='".$_REQUEST['dp6']."' OR id='".$_REQUEST['dp7']."' OR id='".$_REQUEST['dp8']."' OR id='".$_REQUEST['dp9']."' OR id='".$_REQUEST['dp10']."' OR id='".$_REQUEST['dp11']."' OR id='".$_REQUEST['dp12']."' OR id='".$_REQUEST['dp13']."' OR id='".$_REQUEST['dp14']."' OR id='".$_REQUEST['dp15']."' OR id='".$_REQUEST['dp16']."' OR id='".$_REQUEST['dp17']."' OR id='".$_REQUEST['dp18']."' OR id='".$_REQUEST['dp19']."' OR id='".$_REQUEST['dp20']."')"; ?>

<?php $sD=mysql_query("select * from core_departments where is_active=1 ".$Dpt." order by department_name",$con); //AND DepartmentName!='Management' 
	...
	<tr>
	  <td align="left" style="width:100%;">
	     <table border="1" id="table1" style="width:100%;background-color:#FFFFFF;" cellspacing="0">
		  ...
		  <tr>
		   <td class="th" rowspan="2"><?php if($_REQUEST['t']==1){echo 'Lodging Entitlement';}elseif($_REQUEST['t']==2){echo 'Daily Allowance';}elseif($_REQUEST['t']==3){echo 'Mobile Handset & Reimbursement'; }elseif($_REQUEST['t']==4){echo 'Laptop Eligibility & Limit'; } elseif($_REQUEST['t']==5){echo 'Mediclaim Cov. & Helth Checkup';}elseif($_REQUEST['t']==6){echo 'Travel Entitlement - Flight';}elseif($_REQUEST['t']==7){echo 'Travel Entitlement - Train';}elseif($_REQUEST['t']==8){echo 'Travel Eligibility - Two Wheeler';}elseif($_REQUEST['t']==9){echo 'Travel Eligibility - Four Wheeler';}elseif($_REQUEST['t']==10){echo 'Vehicle Value Limit';}elseif($_REQUEST['t']==11){echo 'Travel Mode';}elseif($_REQUEST['t']==12){echo 'Group Term Insurance';} ?>
		   </td>
		   <?php foreach($Dptry as $data){ //while($rD=mysql_fetch_assoc($sD)){ ?>
		    <?php $sVr=mysql_query("select v.id,v.vertical_name from core_verticals v left join core_vertical_function_mapping vf on v.id=vf.vertical_id left join core_vertical_department_mapping vd on vf.id=vd.function_vertical_id where vd.department_id=".$data['id']." group by v.vertical_name order by v.vertical_name ASC", $con); $rowVr=mysql_num_rows($sVr);  
			$Verry=array(); 
			?>
		   <td class="th" style="width:100px;" <?php if($rowVr>0){echo "colspan=".$rowVr;}else{echo "rowspan=2"; }?>><?=$data['department_code']?></td>
		   <?php } ?>
		 </tr>
		 <tr>
		   ...
		   <td class="th" style="width:100px;"><?=$rVr['vertical_name']?></td>
		   <?php } } ?>
		 </tr>
		 </thead>
		 </div>
         ...
         <?php $sqlG = mysql_query("select * from core_grades where company_id=".$CompanyId." AND is_active=1 order by id DESC", $con); $sno=1; while($resG = mysql_fetch_array($sqlG)){ ?>
		 ...
		 <tr style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>">
	      ...
	      elseif($i==3){echo 'C'; $iv='CategoryC'; }?></td>
		    <?php foreach($Dptry as $data){ $sVr=mysql_query("select v.id,v.vertical_name from core_verticals v left join core_vertical_function_mapping vf on v.id=vf.vertical_id left join core_vertical_department_mapping vd on vf.id=vd.function_vertical_id where vd.department_id=".$data['id']." group by v.vertical_name order by v.vertical_name ASC", $con); $rowVr=mysql_num_rows($sVr);
			if($rowVr==0){ $sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id'],$con); $res=mysql_fetch_assoc($sql); ?>
			 <td class="tdc">
			 <input class="inputc" id="Category<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" value="<?php if($i==1){echo $res['CategoryA'];}elseif($i==2){echo $res['CategoryB'];}elseif($i==3){echo $res['CategoryC'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'Category','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td> 
		    <?php }else{ $Verry=array(); while($rVr=mysql_fetch_assoc($sVr)){ 
			$sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id']." AND VerticalId=".$rVr['id'],$con); $res=mysql_fetch_assoc($sql);?>
		     <td class="tdc"><input class="inputc" id="Category<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" value="<?php if($i==1){echo $res['CategoryA'];}elseif($i==2){echo $res['CategoryB'];}elseif($i==3){echo $res['CategoryC'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'Category','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td>
		  ...
		  <?php } elseif($_REQUEST['t']==2){ /******* -------- 2222222222 --------**********/ ?>
		   ...
		   elseif($i==5){echo 'ExHQ'; $iv='ExHQ'; }
		   elseif($i==6){echo 'ExHQ Remark'; $iv='ExHQ_Rmk'; }  ?></td>
		   <?php foreach($Dptry as $data){ $sVr=mysql_query("select v.id,v.vertical_name from core_verticals v left join core_vertical_function_mapping vf on v.id=vf.vertical_id left join core_vertical_department_mapping vd on vf.id=vd.function_vertical_id where vd.department_id=".$data['id']." group by v.vertical_name order by v.vertical_name ASC", $con); $rowVr=mysql_num_rows($sVr);
			if($rowVr==0){ $sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id'],$con); $res=mysql_fetch_assoc($sql);?>
			 <td class="tdc"><input class="inputc" id="DA<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" value="<?php if($i==1){echo $res['DA_OutSiteHQ'];}elseif($i==2){echo $res['DA_OutSiteHQ_Rmk'];}elseif($i==3){echo $res['DA_InSiteHQ'];}elseif($i==4){echo $res['DA_InSiteHQ_Rmk'];}elseif($i==5){echo $res['ExHQ'];}elseif($i==6){echo $res['ExHQ_Rmk'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'DA','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td> 
		    <?php }else{ $Verry=array(); while($rVr=mysql_fetch_assoc($sVr)){ 
			$sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id']." AND VerticalId=".$rVr['id'],$con); $res=mysql_fetch_assoc($sql);?>
		     <td class="tdc"><input class="inputc" id="DA<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" value="<?php if($i==1){echo $res['DA_OutSiteHQ'];}elseif($i==2){echo $res['DA_OutSiteHQ_Rmk'];}elseif($i==3){echo $res['DA_InSiteHQ'];}elseif($i==4){echo $res['DA_InSiteHQ_Rmk'];}elseif($i==5){echo $res['ExHQ'];}elseif($i==6){echo $res['ExHQ_Rmk'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'DA','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td>
		    <?php }/*while*/  }/*else*/ }/*foreach*/ ?>
		  
		  <?php } elseif($_REQUEST['t']==3){ /******* -------- 3333333333 --------**********/ ?>
		   <td class="tdl"><?php if($i==1){echo 'Mobile Normal(Rs)'; $iv='Mobile'; }
		   elseif($i==2){echo 'Mobile Normal(Rs) Remark'; $iv='Mobile_Rmk'; }
		   elseif($i==3){echo 'Mobile GPRS(Rs)'; $iv='Mobile_WithGPS'; }
		   elseif($i==4){echo 'Mobile GPRS(Rs) Remark'; $iv='Mobile_WithGPS_Rmk'; }
		   elseif($i==5){echo 'Reimb. <b>Prepaid</b> (Rs)'; $iv='Mobile_Remb'; }
		   elseif($i==6){echo 'Reimb. <b>Prepaid</b> Period'; $iv='Mobile_Remb_Period'; }
		   elseif($i==7){echo 'Reimb. <b>Prepaid</b> Remark/Condition'; $iv='Mobile_Remb_Period_Rmk'; }
		   elseif($i==8){echo 'Reimb. <b>Postpaid</b> (Rs) '; $iv='Mobile_RembPost'; }
		   elseif($i==9){echo 'Reimb. <b>Postpaid</b> Period '; $iv='Mobile_RembPost_Period'; }
		   elseif($i==10){echo 'Reimb. <b>Postpaid</b> Remark/Condition '; $iv='Mobile_RembPost_Period_Rmk'; }?></td>
		   <?php foreach($Dptry as $data){ $sVr=mysql_query("select v.id,v.vertical_name from core_verticals v left join core_vertical_function_mapping vf on v.id=vf.vertical_id left join core_vertical_department_mapping vd on vf.id=vd.function_vertical_id where vd.department_id=".$data['id']." group by v.vertical_name order by v.vertical_name ASC", $con); $rowVr=mysql_num_rows($sVr);
			
			if($rowVr==0){ $sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id'],$con); $res=mysql_fetch_assoc($sql);?>
			 <td class="tdc"><?php if($i!=6 AND $i!=9){?><input class="inputc" id="Mob<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" value="<?php if($i==1){echo $res['Mobile'];}elseif($i==2){echo $res['Mobile_Rmk'];}elseif($i==3){echo $res['Mobile_WithGPS'];}elseif($i==4){echo $res['Mobile_WithGPS_Rmk'];}elseif($i==5){echo $res['Mobile_Remb'];}elseif($i==7){echo $res['Mobile_Remb_Period_Rmk'];}elseif($i==8){echo $res['Mobile_RembPost'];}elseif($i==10){echo $res['Mobile_RembPost_Period_Rmk'];} ?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'Mob','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/><?php }elseif($i==6){ ?><select class="selb" id="Mob<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'Mob','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"><option value="" <?php if($res['Mobile_Remb_Period']==''){echo 'selected';}?>></option>
		   ...
		   </select><?php } elseif($i==9){ ?><select class="selb" id="Mob<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'Mob','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"><option value="" <?php if($res['Mobile_RembPost_Period']==''){echo 'selected';}?>></option>
		    <option value="Mnt" <?php if($res['Mobile_RembPost_Period']=='Mnt'){echo 'selected';}?>>Monthly</option>
		    <option value="Qtr" <?php if($res['Mobile_RembPost_Period']=='Qtr'){echo 'selected';}?>>Quarter</option>
		    <option value="1/2 Yearly" <?php if($res['Mobile_RembPost_Period']=='1/2 Yearly'){echo 'selected';}?>>1/2 Yearly</option>
		   <option value="Yearly" <?php if($res['Mobile_RembPost_Period']=='Yearly'){echo 'selected';}?>>Yearly</option>
		   </select><?php } ?>
			 </td> 
		    <?php }else{ $Verry=array(); while($rVr=mysql_fetch_assoc($sVr)){ 
			$sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id']." AND VerticalId=".$rVr['id'],$con); $res=mysql_fetch_assoc($sql);?>
		     <td class="tdc"><?php if($i!=6 AND $i!=9){?><input class="inputc" id="Mob<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" value="<?php if($i==1){echo $res['Mobile'];}elseif($i==2){echo $res['Mobile_Rmk'];}elseif($i==3){echo $res['Mobile_WithGPS'];}elseif($i==4){echo $res['Mobile_WithGPS_Rmk'];}elseif($i==5){echo $res['Mobile_Remb'];}elseif($i==7){echo $res['Mobile_Remb_Period_Rmk'];}elseif($i==8){echo $res['Mobile_RembPost'];}elseif($i==10){echo $res['Mobile_RembPost_Period_Rmk'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'Mob','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/><?php }elseif($i==6){ ?><select class="selb" id="Mob<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'Mob','<?=$resG['grade_name']?>')"><option value="" <?php if($res['Mobile_Remb_Period']==''){echo 'selected';}?>></option>
		    <option value="Mnt" <?php if($res['Mobile_Remb_Period']=='Mnt'){echo 'selected';}?>>Monthly</option>
		    <option value="Qtr" <?php if($res['Mobile_Remb_Period']=='Qtr'){echo 'selected';}?>>Quarter</option>
		    <option value="1/2 Yearly" <?php if($res['Mobile_Remb_Period']=='1/2 Yearly'){echo 'selected';}?>>1/2 Yearly</option>
		   <option value="Yearly" <?php if($res['Mobile_Remb_Period']=='Yearly'){echo 'selected';}?>>Yearly</option>
		  ...
		  <?php } elseif($_REQUEST['t']==4){ /******* -------- 4444444444 --------**********/ ?>
		   <td class="tdl"><?php if($i==1){echo 'Laptop (Rs)'; $iv='Laptop_Amt'; }
		   elseif($i==2){echo 'Remark/Condition'; $iv='Laptop_Remark'; }  ?></td>
		   <?php foreach($Dptry as $data){ $sVr=mysql_query("select v.id,v.vertical_name from core_verticals v left join core_vertical_function_mapping vf on v.id=vf.vertical_id left join core_vertical_department_mapping vd on vf.id=vd.function_vertical_id where vd.department_id=".$data['id']." group by v.vertical_name order by v.vertical_name ASC", $con); 
		   $rowVr=mysql_num_rows($sVr);
			if($rowVr==0){ $sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id'],$con); $res=mysql_fetch_assoc($sql);?>
			 <td class="tdc"><input class="inputc" id="Laptop<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" value="<?php if($i==1){echo $res['Laptop_Amt'];}elseif($i==2){echo $res['Laptop_Remark'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'Laptop','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td> 
		    <?php }else{ $Verry=array(); while($rVr=mysql_fetch_assoc($sVr)){ 
			$sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id']." AND VerticalId=".$rVr['id'],$con); $res=mysql_fetch_assoc($sql);?>
		     <td class="tdc"><input class="inputc" id="Laptop<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" value="<?php if($i==1){echo $res['Laptop_Amt'];}elseif($i==2){echo $res['Laptop_Remark'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'Laptop','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td>
		    <?php }/*while*/  }/*else*/ }/*foreach*/ ?>
		  
		  <?php } elseif($_REQUEST['t']==5){ /******* -------- 5555555555 --------**********/ ?>
		  <td class="tdl"><?php if($i==1){echo 'Mediclaim Slabs(Rs)'; $iv='Mediclaim_Coverage_Slabs'; }
		  elseif($i==2){echo 'Helth Checkup(Rs)'; $iv='Helth_CheckUp'; }
		  elseif($i==3){echo 'Remark/Condition'; $iv='Helth_CheckUp_Rmk'; }?></td>
		   <?php foreach($Dptry as $data){ $sVr=mysql_query("select v.id,v.vertical_name from core_verticals v left join core_vertical_function_mapping vf on v.id=vf.vertical_id left join core_vertical_department_mapping vd on vf.id=vd.function_vertical_id where vd.department_id=".$data['id']." group by v.vertical_name order by v.vertical_name ASC", $con); $rowVr=mysql_num_rows($sVr);
			if($rowVr==0){ $sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id'],$con); $res=mysql_fetch_assoc($sql);?>
			 <td class="tdc"><input class="inputc" id="Medi<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" value="<?php if($i==1){echo $res['Mediclaim_Coverage_Slabs'];}elseif($i==2){echo $res['Helth_CheckUp'];}elseif($i==3){echo $res['Helth_CheckUp_Rmk'];} ?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'Medi','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td> 
		    <?php }else{ $Verry=array(); while($rVr=mysql_fetch_assoc($sVr)){ 
			$sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id']." AND VerticalId=".$rVr['id'],$con); $res=mysql_fetch_assoc($sql);?>
		     <td class="tdc"><input class="inputc" id="Medi<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" value="<?php if($i==1){echo $res['Mediclaim_Coverage_Slabs'];}elseif($i==2){echo $res['Helth_CheckUp'];}elseif($i==3){echo $res['Helth_CheckUp_Rmk'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'Medi','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td>
		    <?php }/*while*/  }/*else*/ }/*foreach*/ ?>
		  ...
		  <?php } elseif($_REQUEST['t']==6){ /******* -------- 6666666666 --------**********/ ?>
		   ...
		   <?php foreach($Dptry as $data){ $sVr=mysql_query("select v.id,v.vertical_name from core_verticals v left join core_vertical_function_mapping vf on v.id=vf.vertical_id left join core_vertical_department_mapping vd on vf.id=vd.function_vertical_id where vd.department_id=".$data['id']." group by v.vertical_name order by v.vertical_name ASC", $con); $rowVr=mysql_num_rows($sVr);
			if($rowVr==0){ $sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id'],$con); $res=mysql_fetch_assoc($sql);?>
			 <td class="tdc">
			 <?php if($i==1){ ?><select class="selb" id="Flight<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'Flight','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"><option value="N" <?php if($res['Flight_YN']=='N'){echo 'selected';}?>>No</option><option value="Y" <?php if($res['Flight_YN']=='Y'){echo 'selected';}?>>Yes</option></select>
			 <?php } elseif($i==2){ ?><select class="selb" id="Flight<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'Flight','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"><option value="Economy" <?php if($res['Flight_Class']=='Economy'){echo 'selected';}?>>Economy</option><option value="Business" <?php if($res['Flight_Class']=='Business'){echo 'selected';}?>>Business</option><option value="" <?php if($res['Flight_Class']==''){echo 'selected';}?>></option></select>
			 <?php } elseif($i==3){ ?><input class="inputc" id="Flight<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" value="<?=$res['Flight_Class_Rmk']?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'Flight','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/>
			 <?php } ?>
			 </td> 
		    <?php }else{ $Verry=array(); while($rVr=mysql_fetch_assoc($sVr)){ 
			$sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id']." AND VerticalId=".$rVr['id'],$con); $res=mysql_fetch_assoc($sql);?>
		     <td class="tdc">
			 <?php if($i==1){ ?><select class="selb" id="Flight<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'Flight','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"><option value="N" <?php if($res['Flight_YN']=='N'){echo 'selected';}?>>No</option><option value="Y" <?php if($res['Flight_YN']=='Y'){echo 'selected';}?>>Yes</option></select>
			 <?php } elseif($i==2){ ?><select class="selb" id="Flight<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'Flight','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"><option value="Economy" <?php if($res['Flight_Class']=='Economy'){echo 'selected';}?>>Economy</option><option value="Business" <?php if($res['Flight_Class']=='Business'){echo 'selected';}?>>Business</option><option value="" <?php if($res['Flight_Class']==''){echo 'selected';}?>></option></select>
			 <?php } elseif($i==3){ ?><input class="inputc" id="Flight<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" value="<?=$res['Flight_Class_Rmk']?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'Flight','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/>
			 <?php } ?>
			 </td>
		    <?php }/*while*/  }/*else*/ }/*foreach*/ ?>
		  ...
		  <?php foreach($Dptry as $data){ $sVr=mysql_query("select v.id,v.vertical_name from core_verticals v left join core_vertical_function_mapping vf on v.id=vf.vertical_id left join core_vertical_department_mapping vd on vf.id=vd.function_vertical_id where vd.department_id=".$data['id']." group by v.vertical_name order by v.vertical_name ASC", $con); $rowVr=mysql_num_rows($sVr);
		   ...
		   <option value="AC" <?php if($res['Train_Class']=='AC'){echo 'selected';}?>>AC</option>
			 ...
			 <?php } elseif($i==3){ ?><input class="inputc" id="Train<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" value="<?=$res['Train_Class_Rmk']?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'Train','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/>
			 <?php } ?>
			 </td> 
		    <?php }else{ $Verry=array(); while($rVr=mysql_fetch_assoc($sVr)){ 
			$sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id']." AND VerticalId=".$rVr['id'],$con); $res=mysql_fetch_assoc($sql);?>
		     <td class="tdc">
			 <?php if($i==1){ ?><select class="selb" id="Train<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'Train','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"><option value="N" <?php if($res['Train_YN']=='N'){echo 'selected';}?>>No</option><option value="Y" <?php if($res['Train_YN']=='Y'){echo 'selected';}?>>Yes</option></select>
			 <?php } elseif($i==2){ ?><select class="selb" id="Train<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'Train','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>">
		   ...
		   <option value="AC" <?php if($res['Train_Class']=='AC'){echo 'selected';}?>>AC</option>
			 ...
			 <?php } elseif($i==3){ ?><input class="inputc" id="Train<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" value="<?=$res['Train_Class_Rmk']?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'Train','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/>
			 <?php } ?>
			 </td>
		    <?php }/*while*/  }/*else*/ }/*foreach*/ ?> 
		  ...
		  <?php } elseif($_REQUEST['t']==8){ /******* -------- 8888888888 --------**********/ ?>
		   ...
		   elseif($i==3){echo '2W: Remark/Condition'; $iv='TW_InHQ_D'; } ?></td>
		   <?php foreach($Dptry as $data){ $sVr=mysql_query("select v.id,v.vertical_name from core_verticals v left join core_vertical_function_mapping vf on v.id=vf.vertical_id left join core_vertical_department_mapping vd on vf.id=vd.function_vertical_id where vd.department_id=".$data['id']." group by v.vertical_name order by v.vertical_name ASC", $con); $rowVr=mysql_num_rows($sVr);
			if($rowVr==0){ $sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id'],$con); $res=mysql_fetch_assoc($sql);?>
			 <td class="tdc"><input class="inputc" id="Two<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" value="<?php if($i==1){echo $res['TW_Km'];}elseif($i==2){echo $res['TW_InHQ_M'];}elseif($i==3){echo $res['TW_InHQ_D'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'Two','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td> 
		    <?php }else{ $Verry=array(); while($rVr=mysql_fetch_assoc($sVr)){ 
			$sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id']." AND VerticalId=".$rVr['id'],$con); $res=mysql_fetch_assoc($sql);?>
		     <td class="tdc"><input class="inputc" id="Two<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" value="<?php if($i==1){echo $res['TW_Km'];}elseif($i==2){echo $res['TW_InHQ_M'];}elseif($i==3){echo $res['TW_InHQ_D'];} ?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'Two','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td>
		    <?php }/*while*/  }/*else*/ }/*foreach*/ ?> 
		   
		  <?php } elseif($_REQUEST['t']==9){ /******* -------- 9999999999 --------**********/ ?>
		   <td class="tdl"><?php if($i==1){echo '4W: Rs/Km'; $iv='FW_Km'; }
		   elseif($i==2){echo '4W: Km/Month'; $iv='FW_InHQ_M'; }
		   elseif($i==3){echo '4W: Remark/Condition'; $iv='FW_InHQ_D'; } ?></td>
		   <?php foreach($Dptry as $data){ $sVr=mysql_query("select v.id,v.vertical_name from core_verticals v left join core_vertical_function_mapping vf on v.id=vf.vertical_id left join core_vertical_department_mapping vd on vf.id=vd.function_vertical_id where vd.department_id=".$data['id']." group by v.vertical_name order by v.vertical_name ASC", $con); $rowVr=mysql_num_rows($sVr);
			if($rowVr==0){ $sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id'],$con); $res=mysql_fetch_assoc($sql);?>
			 <td class="tdc"><input class="inputc" id="Four<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" value="<?php if($i==1){echo $res['FW_Km'];}elseif($i==2){echo $res['FW_InHQ_M'];}elseif($i==3){echo $res['FW_InHQ_D'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'Four','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td> 
		    <?php }else{ $Verry=array(); while($rVr=mysql_fetch_assoc($sVr)){ 
			$sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id']." AND VerticalId=".$rVr['id'],$con); $res=mysql_fetch_assoc($sql);?>
		     <td class="tdc"><input class="inputc" id="Four<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" value="<?php if($i==1){echo $res['FW_Km'];}elseif($i==2){echo $res['FW_InHQ_M'];}elseif($i==3){echo $res['FW_InHQ_D'];} ?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'Four','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td>
		    <?php }/*while*/  }/*else*/ }/*foreach*/ ?> 
		  ...
		  <?php } elseif($_REQUEST['t']==10){ /******* -------- 10 10 10 10 10 --------**********/ ?>
		   ...
		   <?php foreach($Dptry as $data){ $sVr=mysql_query("select v.id,v.vertical_name from core_verticals v left join core_vertical_function_mapping vf on v.id=vf.vertical_id left join core_vertical_department_mapping vd on vf.id=vd.function_vertical_id where vd.department_id=".$data['id']." group by v.vertical_name order by v.vertical_name ASC", $con); $rowVr=mysql_num_rows($sVr);
			if($rowVr==0){ $sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id'],$con); $res=mysql_fetch_assoc($sql);?>
			 <td class="tdc"><input class="inputc" id="VehicleL<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" value="<?php if($i==1){echo $res['Vehicle_Value_Limit'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'VehicleL','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td> 
		    <?php }else{ $Verry=array(); while($rVr=mysql_fetch_assoc($sVr)){ 
			$sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id']." AND VerticalId=".$rVr['id'],$con); $res=mysql_fetch_assoc($sql);?>
		     <td class="tdc"><input class="inputc" id="VehicleL<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" value="<?php if($i==1){echo $res['Vehicle_Value_Limit'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'VehicleL','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td>
		    <?php }/*while*/  }/*else*/ }/*foreach*/ ?>	
		  ...
		  <?php } elseif($_REQUEST['t']==11){ /******* -------- 11 11 11 11 11 --------**********/ ?>
		   ...
		   <?php foreach($Dptry as $data){ $sVr=mysql_query("select v.id,v.vertical_name from core_verticals v left join core_vertical_function_mapping vf on v.id=vf.vertical_id left join core_vertical_department_mapping vd on vf.id=vd.function_vertical_id where vd.department_id=".$data['id']." group by v.vertical_name order by v.vertical_name ASC", $con); $rowVr=mysql_num_rows($sVr);
			if($rowVr==0){ $sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id'],$con); $res=mysql_fetch_assoc($sql);?>
			 <td class="tdc"><input class="inputc" id="TravelM<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" value="<?php if($i==1){echo $res['Travel_Mode'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'TravelM','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td> 
		    <?php }else{ $Verry=array(); while($rVr=mysql_fetch_assoc($sVr)){ 
			$sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id']." AND VerticalId=".$rVr['id'],$con); $res=mysql_fetch_assoc($sql);?>
		     <td class="tdc"><input class="inputc" id="TravelM<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" value="<?php if($i==1){echo $res['Travel_Mode'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'TravelM','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td>
		    <?php }/*while*/  }/*else*/ }/*foreach*/ ?>	
		   
		   <?php } elseif($_REQUEST['t']==12){ /******* -------- 12 12 12 12 12 --------**********/ ?>
		   <td class="tdl"><?php if($i==1){echo 'Term Insurance'; $iv='Term_Insurance'; }
		   elseif($i==2){echo 'Remark/Condition'; $iv='Term_Insurance_Rmk'; }  ?></td>
		   <?php foreach($Dptry as $data){ $sVr=mysql_query("select v.id,v.vertical_name from core_verticals v left join core_vertical_function_mapping vf on v.id=vf.vertical_id left join core_vertical_department_mapping vd on vf.id=vd.function_vertical_id where vd.department_id=".$data['id']." group by v.vertical_name order by v.vertical_name ASC", $con); $rowVr=mysql_num_rows($sVr);
			if($rowVr==0){ $sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id'],$con); $res=mysql_fetch_assoc($sql);?>
			 <td class="tdc"><input class="inputc" id="TermInsu<?=$i.'_'.$resG['id'].'_'.$data['id'].'_0'?>" value="<?php if($i==1){echo $res['Term_Insurance'];}elseif($i==2){echo $res['Term_Insurance_Rmk'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].',0,'.$i.','.$CompanyId.','.$UserId?>,'TermInsu','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td> 
		    <?php }else{ $Verry=array(); while($rVr=mysql_fetch_assoc($sVr)){ 
			$sql=mysql_query("select * from hrm_master_eligibility where CompanyId=".$CompanyId." AND DepartmentId=".$data['id']." AND GradeId=".$resG['id']." AND VerticalId=".$rVr['id'],$con); $res=mysql_fetch_assoc($sql);?>
		     <td class="tdc"><input class="inputc" id="TermInsu<?=$i.'_'.$resG['id'].'_'.$data['id'].'_'.$rVr['id']?>" value="<?php if($i==1){echo $res['Term_Insurance'];}elseif($i==2){echo $res['Term_Insurance_Rmk'];}?>" onChange="FunSetElig(this.value,<?=$_REQUEST['t']?>,'<?=$iv?>',<?=$resG['id'].','.$data['id'].','.$rVr['id'].','.$i.','.$CompanyId.','.$UserId?>,'TermInsu','<?=$resG['grade_name']?>')" style="background-color:<?php if($sno%2==0){echo '#ECE0F5';}else{echo '#FFFFFF';}?>"/></td>
		    <?php }/*while*/  }/*else*/ }/*foreach*/ ?>
			
			<?php }//elseif($_REQUEST['t']==12) ?>	  		  
