...
{  
 $SqlInseart = mysql_query("INSERT INTO hrm_department_vertical(VerticalName, DeptId, GradeId, ComId, crby, crdate) VALUES('".$_POST['VerticalName']."', '".$_POST['DeptId']."', '".$_POST['GradeId']."', ".$CompanyId.", ".$UserId.", '".date('Y-m-d')."')", $con);  
...
{ 
 ...
 $SqlUpdate = mysql_query("UPDATE hrm_department_vertical SET VerticalName='".$_POST['VerticalName']."', DeptId='".$_POST['DeptId']."', GradeId='".$_POST['GradeId']."', crby=".$UserId.", crdate='".date("Y-m-d")."' WHERE VerticalId=".$_POST['VerticalId'], $con) or die(mysql_error());  
...
if(isset($_POST['VerticalUp']))
{
 ...
 {
  $queryup=mysql_query("update hrm_employee_general set EmpVertical=".$_POST['EmpVertical_'.$i].", EmpSection=".$_POST['EmpSection_'.$i]."  where EmployeeId=".$_POST['eid_'.$i]."",$con);
 }
 if($queryup){ echo '<script>alert("Vertical Update Successfully");</script>'; }
}


if(isset($_POST['SecSaveNew']))
{  
 $SqlInseart = mysql_query("INSERT INTO hrm_department_section(SectionName, DeptId, ComId, crby, crdate) VALUES('".$_POST['SectionName']."','".$_POST['DeptId']."', ".$CompanyId.", ".$UserId.", '".date('Y-m-d')."')", $con);  
 if($SqlInseart){ $msg = "Data has been Inserted successfully..."; }
}

if(isset($_POST['SecSaveEdit']))
{ 

 $SqlUpdate = mysql_query("UPDATE hrm_department_section SET SectionName='".$_POST['SectionName']."', DeptId='".$_POST['DeptId']."', crby=".$UserId.", crdate='".date("Y-m-d")."' WHERE SectionId=".$_POST['SectionId'], $con);  
 if($SqlUpdate){$msg="Data has been Updeted successfully...";}
}

if($_REQUEST['action']=='Secdelete')
{ 

 $SqlDel = mysql_query("delete from hrm_department_section WHERE SectionId=".$_REQUEST['sid'], $con);  
 if($SqlDel){$msg="Data has been Deleted successfully...";}
}
...
function validateEdit(formEdit){
  ...
  if(test_bool4==false) { alert('Please enter only alphabets in the vertical name Field');  return false; } 
}   


function Secedit(value,ed) { var agree=confirm("Are you sure you want to edit this record?");
if (agree) { var x = "vertical.php?actty=true&opt=false&vlu=45&sec=comback&ee=%rr%&ff=true&r2r=true&action=Secedit&sid="+value+"&d="+ed;  window.location=x;}}

function Secdel(value,ed) { var agree=confirm("Are you sure you want to delete this record?");
if (agree) { var x = "vertical.php?actty=true&opt=false&vlu=45&sec=comback&ee=%rr%&ff=true&r2r=true&action=Secdelete&sid="+value+"&d="+ed;  window.location=x;}}

function Secnewsave(ed) { var x = "vertical.php?action=Secnewsave&act=checkdept&d=23&rr=false&rr=98&ff=23&dd=3&&d="+ed;  
window.location=x;}

function SecvalidateEdit(formEdit){
  var SectionName = formEdit.SectionName.value; var filter=/^[a-zA-Z. /]+$/; var test_bool4 = filter.test(SectionName);
  if (SectionName.length === 0) { alert("please enter section name field.");  return false; } 
  if(test_bool4==false) { alert('Please enter only alphabets in the section name Field');  return false; } 
}   
...
<?php if($_SESSION['login'] = true){ ?>	
 <tr>
  ...
  <td width="300" class="heading"><u>Department Vertical/Section Master :</u></td>    
...
<table border="1" width="100%" bgcolor="#FFFFFF" cellspacing="1">
 <tr bgcolor="#7a6189" style="height:25px;">
  ...
  <td class="th" style="width:10%;"><b>Grade</b></td>
  <td class="th" style="width:5%;"><b>Action</b></td>
 </tr>
<?php $sqlRat=mysql_query("select v.*,GradeValue from hrm_department_vertical v left join hrm_grade g on v.GradeId=g.GradeId where v.ComId=".$CompanyId." AND v.DeptId=".$_REQUEST['d']." order by v.VerticalName ASC", $con); $SNo=1; while($resRat=mysql_fetch_array($sqlRat)) {
...
<input type="hidden" id="DeptId" name="DeptId" value="<?php echo $_REQUEST['d']; ?>" /> 
 ...
 <tr style="height:24px;">
  ...
  <td class="tdc">
   <select name="GradeId" id="GradeId" class="tdinput" style="width:98%;"><option value="0">Select</option>
 <?php $sqlGrade = mysql_query("SELECT * FROM hrm_grade WHERE CompanyId=".$CompanyId." and CreatedDate>='2014-02-01' order by GradeId",$con); 
     while($resG = mysql_fetch_assoc($sqlGrade)){ ?>
    <option value="<?=$resG['GradeId']?>" <?php if($resG['GradeId']==$resRat['GradeId']){echo 'selected';} ?>>&nbsp;<?=$resG['GradeValue']?></option><?php } ?>
  </select>
  </td>
...
<tr style="height:24px;">
 ...
 <td class="tdc">&nbsp;<?php echo $resRat['GradeValue']; ?></td>
...
<tr>
 ...
 <td class="tdc"><select name="GradeId" id="GradeId" class="tdinput" style="width:98%;"><option value="0">Select</option>
 <?php $sqlGrade = mysql_query("SELECT * FROM hrm_grade WHERE CompanyId=".$CompanyId." and CreatedDate>='2014-02-01' order by GradeId",$con); 
     while($resG = mysql_fetch_assoc($sqlGrade)){ ?>
    <option value="<?=$resG['GradeId']?>">&nbsp;<?=$resG['GradeValue']?></option><?php } ?>
  </select>
  </td>
...
<table border="1" width="100%" bgcolor="#FFFFFF" cellspacing="1">
 <tr bgcolor="#7a6189" style="height:25px;">
  <td class="th" style="width:5%;"><b>Sn</b></td>
  <td class="th" style="width:30%;"><b>Section Name</b></td>
  <td class="th" style="width:5%;"><b>Action</b></td>
 </tr>
<?php $sqlRat=mysql_query("select * from hrm_department_section where ComId=".$CompanyId." AND DeptId=".$_REQUEST['d']." order by SectionName ASC", $con); $SNo2=1; while($resRat=mysql_fetch_array($sqlRat)) {
  if(isset($_REQUEST['action']) && $_REQUEST['action']=="Secedit" && $_REQUEST['sid']==$resRat['SectionId']){ ?>
<form name="form2Edit" method="post" onSubmit="return SecvalidateEdit(this)">	
<input type="hidden" id="DeptId" name="DeptId" value="<?php echo $_REQUEST['d']; ?>" /> 
 
 <tr style="height:24px;">
  <td class="tdc"><?php echo $SNo; ?></td>
  <td class="tdl"><input name="SectionName" id="SectionName" class="EditInput" style="width:99%;border:hidden;" value="<?php echo $resRat['SectionName']; ?>" /></td>
  
  <td class="tdc"><input type="hidden" id="ed" name="ed" value="<?php echo $_REQUEST['d']; ?>" />  
<?php if($_SESSION['User_Permission']=='Edit'){ ?>&nbsp;<input type="submit" name="SecSaveEdit"  value="" class="SaveButton">&nbsp;<input type="hidden" name="SectionId" id="SectionId" value="<?php echo $_REQUEST['sid']; ?>"/><?php } ?></td>
 </tr>
</form>
...
<tr style="height:24px;">
 ...
 <td class="tdc"><?php if($_SESSION['User_Permission']=='Edit'){ ?><a href="#"><img src="images/edit.png" border="0" alt="Edit" onClick="Secedit(<?php echo $resRat['SectionId'].', '.$_REQUEST['d']; ?>)"></a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#"><img src="images/delete.png" alt="Delete" border="0" onClick="Secdel(<?php echo $resRat['SectionId'].', '.$_REQUEST['d']; ?>)"></a><?php } ?></td>
</tr>
<?php } $SNo2++;} ?>
<tr><td bgcolor="#B6E9E2" colspan="10"></td></tr>

<?php if(isset($_REQUEST['action']) && $_REQUEST['action']=="Secnewsave"){ ?>
<form name="formEdit" method="post" onSubmit="return validateEdit(this)">
<input type="hidden" id="DeptId" name="DeptId" value="<?php echo $_REQUEST['d']; ?>" /> 
<tr>
 <td class="tdc"><?php echo $SNo; ?></td>
 <td class="tdc"><input name="SectionName" id="SectionName" class="EditInput" style="width:99%;"/></td> 
 
 <td class="tdc"><?php if($_SESSION['User_Permission']=='Edit'){ ?>&nbsp;<input type="submit" name="SecSaveNew"  value="" class="SaveButton">&nbsp;<?php } ?></td>
</tr>
</form>
<?php } ?>    
...
<td colspan="8" align="Right" class="fontButton"><table border="0" width="550">
<tr><td align="right">
<input type="button" name="SecNewSave" id="SecNewSave" value="new" onClick="Secnewsave(<?php echo $_REQUEST['d']; ?>)" <?php if($_REQUEST['action']=="Secnewsave" OR $_REQUEST['action']=="Secedit"){ echo "style=display:none;"; }?>/>
<input type="button" name="back" id="back" style="width:90px;" value="back" onClick="javascript:window.location='Index.php?log=<?php echo $_SESSION['logCheckUser']; ?>'"/>
<input type="button" name="Refresh" value="refresh" onClick="javascript:window.location='vertical.php?actty=true&opt=false&vlu=45&sec=comback&ee=%rr%&ff=true&r2r=true&d=<?=$_REQUEST['d'];?>&ed=4'"/>&nbsp;</td>
</tr></table>
</td>
...
$sSat=mysql_query("select * from hrm_department_section where ComId=".$CompanyId." AND DeptId=".$_REQUEST['d']." order by SectionName ASC"); while($rSat=mysql_fetch_assoc($sSat)){ $SatArr[$rSat['SectionId']]=$rSat['SectionName']; }
?>  
<tr>
<td colspan="5" align="left" id="type" valign="top">             
...
<table border="1" width="100%" bgcolor="#FFFFFF"  cellspacing="1">
 <tr bgcolor="#7a6189" style="height:25px;">
  ...
  <td class="thh" style="width:30%;"><b>Name</b></td>
  <td class="thh" style="width:15%;"><b>Department</b></td>
  <td class="thh" style="width:5%;"><b>Garde</b></td>
  <td class="thh" style="width:15%;"><b>Vertical</b></td>
  <td class="thh" style="width:15%;"><b>Section</b></td>
 </tr>
<?php $sqlEmp=mysql_query("select e.EmployeeID,e.EmpCode,Fname,Sname,Lname,de.DepartmentCode,gr.GradeValue,g.EmpVertical,g.EmpSection from hrm_employee e inner join hrm_employee_general g on e.EmployeeID=g.EmployeeID inner join hrm_department de on g.DepartmentId=de.DepartmentId inner join hrm_grade gr on g.GradeId=gr.GradeId where e.EmpStatus='A' AND e.CompanyId=".$CompanyId." AND g.DepartmentId=".$_REQUEST['d']." order by e.ECode ASC", $con); $no=1; while($resEmp=mysql_fetch_array($sqlEmp)) { ?>
...
<tr style="height:22px;">
 ...
 <td class="tdcc"><select name="EmpVertical_<?=$no;?>" id="EmpVertical_<?=$no;?>" class="tdinput" style="width:99%;">
  ...
  <td class="tdcc"><select name="EmpSection_<?=$no;?>" id="EmpSection_<?=$no;?>" class="tdinput" style="width:99%;">
  <?php if($resEmp['EmpSection']==0){?><option value="0">Select</option><?php } ?>
  <?php foreach($SatArr as $key =>$value){ ?>
				 <option value="<?=$key?>" <?=($resEmp['EmpSection']==$key)?'selected':'';?>><?=$value?></option>
  <?php } ?>
  <option value="0">Blank</option>
  </select></td>
</tr>
