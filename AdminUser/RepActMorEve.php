...
{ 
 ...
 {  
  ...
  //echo $ctr.'-'.$c3.'-'.$c5.'-'.$c7.'<br>';
  if ($ctr>1 AND $c3!='' AND $c5!='' AND $c7!=0) //AND $c6!=''
  { 
 ...
 <tr>
  ...
  <td class="td1" style="font-size:11px;width:100px;" align="left"><select style="font-size:12px; width:100px;height:20px;background-color:#DDFFBB;display:block;" name="Month4" id="Month4" onChange="FunMonth4(this.value)"><option value="<?php echo $_REQUEST['m4'];?>"><?php echo $Month4;?></option><?php echo $ExMonth;?></select></td>
	 ...
	 <td class="td1" style="font-size:11px; width:150px;">			   
	   <select style="font-size:11px; width:150px; height:19px; background-color:#DDFFBB; display:block;" name="Dept" id="Dept" onChange="FunDept(this.value)">
<?php if($_REQUEST['D']!='All') { $sqlD=mysql_query("select department_name,department_code from core_departments where id=".$_REQUEST['D'], $con); $resD=mysql_fetch_assoc($sqlD); ?> <option value="<?php echo $_REQUEST['D']; ?>" style="margin-left:0px; background-color:#84D9D5;">&nbsp;<?php echo $resD['department_name']; ?></option>
<?php } else { ?><option value="All" style="margin-left:0px; background-color:#84D9D5;">&nbsp;All</option><?php } $sqlDept=mysql_query("select * from core_departments where is_active=1 order by department_name", $con); while($resDept=mysql_fetch_array($sqlDept)) { ?><option value="<?php echo $resDept['id']; ?>"><?php echo '&nbsp;'.$resDept['department_name'];?></option><?php } ?>
...
<?php } ?></select></td> 
	 <td class="td1" style="font-size:11px;width:150px;"><select style="font-size:11px;width:150px;height:19px;background-color:#DDFFBB;display:block;" name="Dept2" id="Dept2" onChange="FunDept2(this.value)"><?php if($_REQUEST['D2']!='All') { $sqlD2=mysql_query("select department_name from core_departments where id=".$_REQUEST['D2'], $con); $resD2=mysql_fetch_assoc($sqlD2); ?><option value="<?php echo $_REQUEST['D2']; ?>" style="margin-left:0px; background-color:#84D9D5;">&nbsp;<?php echo $resD2['department_name']; ?></option><?php  } else { ?><option value="All" style="margin-left:0px; background-color:#84D9D5;">&nbsp;All</option><?php } ?>						
<?php $sqlDept2=mysql_query("select * from core_departments where is_active=1 order by department_name", $con); while($resDept2=mysql_fetch_array($sqlDept2)){ ?><option value="<?php echo $resDept2['id']; ?>"><?php echo '&nbsp;'.$resDept2['department_name'];?></option><?php } ?><option value="All">&nbsp;All</option></select></td>	 
