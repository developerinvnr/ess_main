...
<td class="cell" style="text-align:left;" colspan="<?php echo date("t",strtotime(date("Y-".$_REQUEST['m']."-d"))); ?>"><b>&nbsp;Month :</b>&nbsp;<font style="font:Times New Roman; color:#EAEF18; font-size:14px; background-color:#7a6189; font-weight:bold;"><?php echo date("F",strtotime(date("Y-".$_REQUEST['m']."-d"))); ?></font>&nbsp;&nbsp;&nbsp;<?php if($_REQUEST['D']!='All') { $sqlD=mysql_query("select department_name as DepartmentName from core_departments where id=".$_REQUEST['D'], $con); $resD=mysql_fetch_assoc($sqlD); }?><b>Department :</b>&nbsp;
