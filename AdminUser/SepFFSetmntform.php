	  ...
	  <td>
	   <table border="1">
		...
		<tr bgcolor="#FFFFFF">
		  ...
		  <td class="Text" style="width:90px;" align="">&nbsp;<?php if($resSE['TerMination']=='Y'){ echo 'Termination';}else{echo 'Relieving'; } ?> : </td><td class="Text" style="width:140px;">&nbsp;<input name="HR_RelievingDate" id="HR_RelievingDate" maxlength="25" style="font-family:Georgia; font-size:12px;width:80px; height:20px; border-style:hidden;" value="<?php if($resSE['HR_RelievingDate3']>='2010-01-01'){echo date("d-m-Y",strtotime($resSE['HR_RelievingDate3']));}elseif($resSE['HR_RelievingDate2']>='2010-01-01'){echo date("d-m-Y",strtotime($resSE['HR_RelievingDate2']));}elseif($resSE['HR_RelievingDate']>='2010-01-01'){echo date("d-m-Y",strtotime($resSE['HR_RelievingDate']));} ?>" readonly><button id="f_btn2" class="CalenderButton" disabled></button>
   <script type="text/javascript">  var cal = Calendar.setup({ onSelect:  function(cal) { cal.hide()}, showTime: true }); 
...
$sqlAcc=mysql_query("select * from hrm_employee_separation_nocacc where EmpSepId=".$_REQUEST['si'],$con); $resAcc=mysql_fetch_assoc($sqlAcc); ?>			
...
<?php } ?>
      ...
      <tr id="TTR2"><td colspan="2" class="Text2"><b>Note:</b><?php if($resSE['EmployeeID']!=242 AND $resSE['EmployeeID']!=100 AND $resSE['EmployeeID']!=1079 AND $resSE['EmployeeID']!=236 AND $resSE['EmployeeID']!=1609){ if($TotalEarn<$TotalDeduct){ ?>&nbsp;Relieving shall be processed post signed acceptance of the final F & F statement and clearance of dues, subject to deposit of aforesaid amount. <?php }elseif($TotalEarn>=$TotalDeduct){ ?>&nbsp;Relieving shall be processed post signed acceptance of the final F & F statement.<?php } } ?> </td></tr>
	  ...
	  <tr>
	   <td>
	    <table border="0">
		 <tr>
		  <td class="Text2" style="width:500px;" align=""><b><?php if($resSE['EmployeeID']==242 OR $resSE['EmployeeID']==100 OR $resSE['EmployeeID']==1079 OR $resSE['EmployeeID']==236 OR $resSE['EmployeeID']==733 OR $resSE['EmployeeID']==1609){echo 'Nominee';}else{?>Employee<?php } ?> Name :&nbsp;
		  <?php if($resSE['EmployeeID']==242){echo 'Seema';}elseif($resSE['EmployeeID']==100){echo 'Sonal Lukhey';}elseif($resSE['EmployeeID']==1079){echo 'Laxmi Shrivastava';}elseif($resSE['EmployeeID']==236){echo 'Nambu Jayalakshmi';}elseif($resSE['EmployeeID']==733){echo 'Mrs. Neha Laxmikant Verma';}elseif($resSE['EmployeeID']==1609){echo 'Mr. Krishna Mohan Roy';}else{?>
