...
if($_REQUEST['value']=='All') {$DeptV='All_Employee';}else{ $sqlDeptV=mysql_query("select department_name as DepartmentCode from core_departments where id=".$_REQUEST['value'], $con); $resDeptV=mysql_fetch_assoc($sqlDeptV); $DeptV=$resDeptV['DepartmentCode'];} 
...
if($_REQUEST['value']=='All'){ $sql = mysql_query("SELECT e.EmployeeID, EmpCode, Fname, Sname, Lname, department_name as DepartmentCode, Appraiser_EmployeeID, Reviewer_EmployeeID, HOD_EmployeeID, Emp_PmsStatus, Appraiser_PmsStatus, Reviewer_PmsStatus, Rev2_EmployeeID, Rev2_PmsStatus, HodSubmit_IncStatus, HR_PmsStatus, ExtraAllowPMS FROM hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN core_departments d ON g.DepartmentId=d.id WHERE (e.EmpStatus='A' OR p.Appraiser_TotalFinalScore>0) AND g.DateJoining<='".$YYear."-06-30' AND p.AssessmentYear=".$_REQUEST['Y']." AND e.CompanyId=".$_REQUEST['C']." AND p.Appraiser_EmployeeID!=0 order by e.EmpCode ASC", $con); }
else { $sql = mysql_query("SELECT e.EmployeeID, EmpCode, Fname, Sname, Lname, department_name as DepartmentCode, Appraiser_EmployeeID, Reviewer_EmployeeID, HOD_EmployeeID, Emp_PmsStatus, Appraiser_PmsStatus, Reviewer_PmsStatus, Rev2_EmployeeID, Rev2_PmsStatus, HodSubmit_IncStatus, HR_PmsStatus, ExtraAllowPMS FROM hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN core_departments d ON g.DepartmentId=d.id WHERE (e.EmpStatus='A' OR p.Appraiser_TotalFinalScore>0) AND g.DateJoining<='".$YYear."-06-30' AND p.AssessmentYear=".$_REQUEST['Y']." AND e.CompanyId=".$_REQUEST['C']." AND g.DepartmentId=".$_REQUEST['value']." AND p.Appraiser_EmployeeID!=0 order by e.EmpCode ASC", $con); }
$no=1;
