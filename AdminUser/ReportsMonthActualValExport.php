...
if($_REQUEST['d']>0){ $sql=mysql_query("select e.EmployeeID,EmpCode,Fname,Sname,Lname from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where g.DepartmentId=".$_REQUEST['d']." AND g.DateJoining<'".$TD.'-03-31'."' AND e.CompanyId=".$_REQUEST['c']." order by e.ECode ASC", $con); }
elseif($_REQUEST['d']==0){ $sql=mysql_query("select e.EmployeeID,EmpCode,Fname,Sname,Lname from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID where g.DateJoining<'".$TD.'-03-31'."' AND e.CompanyId=".$_REQUEST['c']." order by e.ECode ASC", $con); }
