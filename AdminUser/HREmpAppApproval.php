...
//Employer ESIC
if(Cal_TotCtc<=275304.01 && BonusM==2566){ var ComESIC=Math.round(((Cal_TotCtc*2.974893101)/100)*100)/100;}
else if(Cal_TotCtc<=275429.87 && BonusM==2410){ var ComESIC=Math.round(((Cal_TotCtc*2.973533698)/100)*100)/100;}
else if(Cal_TotCtc<=275555.73 && BonusM==2254){ var ComESIC=Math.round(((Cal_TotCtc*2.972175538)/100)*100)/100;}
else if(Cal_TotCtc<=275660.61 && BonusM==2124){ var ComESIC=Math.round(((Cal_TotCtc*2.971044721)/100)*100)/100;}
else { var ComESIC=0; } 

//Mediclaim
if((Cal_TotCtc<=275304.01 && BonusM==2566) || (Cal_TotCtc<=275429.87 && BonusM==2410) || (Cal_TotCtc<=275555.73 && BonusM==2254) || (Cal_TotCtc<=275660.61 && BonusM==2124))
...
{ 
 ...
 $sqVer=mysql_query("select Min_Ctc,Max_Ctc from hrm_pms_vertical_increment where DepId=".$ResP['HR_Curr_DepartmentId']." AND SectionId=".$ResEmp['EmpSection']." AND ".$nxtGrade.">=Min_GradeId AND ".$nxtGrade."<=Max_GradeId",$con); 
...
<input type="hidden" id="GAS" name="GAS" value="<?php if($ResP['HR_PmsStatus']==2){echo floatval($ResP['HR_GrossAnnualSalary']); } else { echo floatval($ResP['Hod_GrossAnnualSalary']); } ?>" readonly/>
	 ...
	 <td class="td22"><input type="text" class="td22" style="border-style:hidden;border:0;width:100%;color:#0080FF;font-weight:bold;text-align:center;background-color:#FFFFFF;" id="GMS" name="GMS" value="<?php if($ResP['HR_PmsStatus']==2 AND $ResP['HR_GrossMonthlySalary']>0){echo floatval($ResP['HR_GrossMonthlySalary']); } elseif($ResP['Hod_GrossMonthlySalary']>0){ echo floatval($ResP['Hod_GrossMonthlySalary']); }else{ echo floatval($ResP['EmpCurrGrossPM']); }?>" readonly/></td>
