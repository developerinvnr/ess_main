...
<?php $nan=1; 
	  ...
	  elseif($rE['AttValue']=='A' OR $rE['AttValue']=='LWP'){echo '#FF7171';}
	  ?>" >
...
<?php $lday=sprintf('%02d',$day); 
	  ...
	  <font style="margin-left:<?php $l1=strlen($day); $l2=strlen($rE['AttValue']); if($l1==1 AND $l2==1){echo '45';}elseif($l1==1 AND $l2==2){echo '40';}elseif($l1==2 AND $l2==1){echo '40';}else{echo '30';}?>px;font-style:normal;font-size:18px;font-weight:bold;color:<?php if(($resCer['TimeApply']=='Y' AND $rE['InnLate']==1 AND $rE['InnCnt']=='Y' AND $rE['AttValue']!='OD') OR ($resCer['TimeApply']=='Y' AND $rE['OuttLate']==1 AND $rE['OuttCnt']=='Y' AND $rE['AttValue']!='OD')){echo '#FF1C1C';} ?>;">
	      
	      <?php if(($resCer['TimeApply']=='Y' AND $rE['InnLate']==1 AND $rE['InnCnt']=='Y' AND $rE['AttValue']!='OD' AND $rowFD==0) OR ($resCer['TimeApply']=='Y' AND $rE['OuttLate']==1 AND $rE['OuttCnt']=='Y' AND $rE['AttValue']!='OD' AND $rowFD==0)){ echo 'L'.$nan; if($rE['AttValue']!='P'){echo '<font size="2">('.$rE['AttValue'].')</font>'; } $nan++; }else{ echo $rE['AttValue']; } ?>
...
$SqlLWP=mysql_query("select count(DISTINCT AttDate)as LWP from hrm_employee_attendance where AttValue='LWP' AND EmployeeID=".$EmployeeId." AND (AttDate between '".date($y."-".$m."-1")."' AND '".date($y."-".$m."-".$NoD)."')", $con); $ResLWP=mysql_fetch_array($SqlLWP); 
$TotalLWP = $ResLWP['LWP'];
...
$CountHf=$ResHf['Hf']/2;  
$TotalAbsent=$ResAbsent['Absent']+$CountHf+$CountAch+$CountAsh+$CountAph+$TotalLWP;  
...
<table class="Ld" cellpadding="0" cellspacing="1">
 ...
 <td class="tt2body">&nbsp;&nbsp;Absent/ LWP</td></tr>	 </table>
	 </td>
	</tr>	
	<tr><td style="height:5px;"></td></tr>
