...
/*********************************************************/
/*********************************************************/
function FunFushData(yi,ei,ci,di,gi)
{
  var agree=confirm("Are you sure for data push in increment sheet? Please ensure that the current working sheet data is saved or not"); 
  if(agree)
  {
    //var url = 'PmsWorkingSheet_Ajax.php'; var pars = 'Action=workingSheet&ei='+Ei+'&Dp='+Dp+'&Gr='+Gr+'&Yi='+Yi+'&type=SaveIncData&act=SaveInc&empid='+document.getElementById("empid_"+i).value+'&Rating='+document.getElementById("rating_"+i).value+'&PreCtc='+document.getElementById("oldctc_"+i).value+'&Per_Prorata='+document.getElementById("Per_Prorata"+i).value+'&Per_Actual='+document.getElementById("Per_Actual"+i).value+'&Ctc='+document.getElementById("Ctc_"+i).value+'&Corr='+document.getElementById("Corr_"+i).value+'&Per_Corr='+document.getElementById("Per_Corr"+i).value+'&Inc='+document.getElementById("Inc"+i).value+'&TotCtc='+document.getElementById("TotCtc"+i).value+'&Per_TotCtc='+document.getElementById("Per_TotCtc"+i).value+'&Rows='+rows+'&i='+i;  
    var myAjax = new Ajax.Request(
    url, { 	method: 'post', parameters: pars, onComplete: show_rst });
  }
  else {return false; }
...
<tr style="background-color:#CBFF97;height:30px;">
 <td class="tdl" colspan="6" style="border-right:hidden;">
    &nbsp;
	...
	&nbsp;&nbsp; 
	<a href="#" onClick="FunExpFormat(<?=$_SESSION['PmsYId'].','.$EmployeeId.','.$CompanyId?>,'<?=$_REQUEST['FilD']?>','<?=$_REQUEST['TeE']?>','<?=$_REQUEST['TrE']?>','<?=$_REQUEST['FilS']?>')"><input type="button" id="ExportBtn" style="width:90px;background-color:#F7B52D;" value="Export" <?php if($row==0){echo 'disabled';}?>/></a>
	&nbsp;&nbsp;
	<a href="#" onClick="FunFushData(<?=$_SESSION['PmsYId'].','.$EmployeeId.','.$CompanyId?>,'<?=$Dp?>','<?=$Gr?>')"><input type="button" id="PushBtn" style="width:200px;background-color:#F7B52D;" value="Push Data in Increment Sheet" <?php if($row==0){echo 'disabled';}else{echo 'disabled';}?>/></a>
	...
	<a href="#" onClick="FunExpAllFormat(<?=$_SESSION['PmsYId'].','.$EmployeeId.','.$CompanyId?>,'<?=$_REQUEST['FilD']?>','<?=$_REQUEST['TeE']?>','<?=$_REQUEST['TrE']?>','<?=$_REQUEST['FilS']?>')"><input type="button" id="ExportBtn" style="width:90px;background-color:#F7B52D;" value="Export" <?php if($roow==0){echo 'disabled';}?>/></a>
...
<div class="tbody">  
<tbody> 
<tr id="TR<?=$sno?>" style="background-color:#FFFFFF;">
 ...
 <td class="tdl"><input class="tdcinp" value="<?=$res['FullName']?>" id="TRa<?=$sno?>" readOnly/></td>
 <td class="tdl"><input class="tdcinp" value="<?=$res['DepartmentCode']?>" id="TRb<?=$sno?>" readOnly/></td>
 <td class="tdl"><input class="tdcinp" value="<?=$res['DesigCode']?>" id="TRc<?=$sno?>" readOnly/></td>
 ...
 <td class="tdl"><input class="tdcinp" value="<?=$rDe['DesigCode']?>" id="TRd<?=$sno?>" readOnly/>
