...
function FunClickAss(ei,ec,c,di,dei,mo,h,fn,sn,ln,mail)
{
  ...
  window.open("http://www.yarms.in/reg/vspl/index.php?act=newvspl&ei="+ei+"&ec="+ec+"&uH="+h+"&fname="+fn+"&sname="+sn+"&lname="+ln+"&com="+c+"&dept="+di+"&desg="+dei+"&email="+mail+"&mobile="+mo, '_blank'); window.focus();
   ...
   $sql_Conf=mysql_query("select * from hrm_employee_confletter where (EmpShow='Y' OR EmpShow_Trr='Y' OR EmpShow_Ext='Y') AND EmployeeID=".$EmployeeId." AND Status='A'",$con); 
         $row_Conf=mysql_num_rows($sql_Conf);
         $LockDate=date("Y-m-d");
         if($row_Conf>0)
         { 
             $res_Conf=mysql_fetch_assoc($sql_Conf); 
             $ConfDate=$res_Conf['ConfDate'];
             $LockDate = date('Y-m-d', strtotime($ConfDate. ' + 1 months'));
         }
		 if($CompanyId!=4 AND $LockDate>=date("Y-m-d") AND ($res_Conf['EmpShow']=='Y' OR $res_Conf['EmpShow_Trr']=='Y' OR $res_Conf['EmpShow_Ext']=='Y')){ 
	?>
    ...
    <td style="width:100%;">
      ...
      */ ?>
      ...
      <tr>  
       <td style="width:100%;text-align:center;font-size:14px;font-family:Georgia;">
        <?php /*<a href="https://play.google.com/store/apps/details?id=in.co.vnrseeds.www.xeasynew" target="_blank">Xeasy App</a>*/ ?>
        <a href="https://expense.vnrseeds.co.in" target="_blank">Xeasy Web Version</a>
       </td>
      </tr>
	 ...
	 <table style="width:100%;" cellpadding="5">
      <tr>  
       <td style="width:100%;text-align:center;font-size:16px;font-family:Georgia;">
        <a href="http://vnrdev.in/HR_Mannual" target="_blank"><font style="color:#94305A;">HR Policy Manual</font></a>
       </td>
      </tr>
     </table>
    </td>
	   ...
	   <td style='font-family:Times New Roman;font-size:16px;color:#FF0000;width:100%;font-weight:bold;'>
	    ...
	    <table style="width:100%;">
		 <tr>  
		  ...
		  :&nbsp;The PMS 2024 begins, and all those who joined before 30th June 2023 are covered in this cycle. Please complete your appraisals on time.<img src="images/new_blink.gif" border="0">
		 </td>  
		 </tr>
		</table>
...
for($i=1; $i<date("d"); $i++)
{
 ...
 { //echo date("Y-m-".$i).', '.$resE['DateJoining'];
 if(strlen($i)==1){$i='0'.$i;}
 ...
 if($rowatt==0 && $rowslv==0 && date("Y-m-".$i)>=$resE['DateJoining']){  ?>
...
$EC2=$resE['PanNo'];
$_SESSION['PN']=$resE['PanNo'];

//$filename = 'ImgTds'.$CompanyId.'212022/'.$EC2.'_2022-23.pdf';
//$filename2 = 'ImgTds'.$CompanyId.'212022/'.$EC2.'_PARTB_2022-23.pdf';

$filename = 'ImgTds'.$CompanyId.'232024/'.$EC2.'_2024-25.pdf';
$filename2 = 'ImgTds'.$CompanyId.'232024/'.$EC2.'_PARTB_2024-25.pdf';
...
<img src="images/new_blink.gif" border="0"><b style="color:#00509F;">TDS Cert. 2023-2024:&nbsp;</b>

<?php if(file_exists($filename)){ ?>
<a href="MyTdsFile.php?a=open&filepath=<?=$filename?>&filename=<?=$EC2.'_2024-25.pdf'?>&no=<?=$EC2?>" style="text-decoration:none;" target="_blank"><b style="color:#FF0000;"><u>Form-A</u></b></a><?php } if(file_exists($filename2)){ ?>&ensp;<a href="MyTdsFile.php?a=open&filepath=<?=$filename2?>&filename=<?=$EC2.'_PARTB_2024-25.pdf'?>&no=<?=$EC2?>" style="text-decoration:none;" target="_blank"><b style="color:#FF0000;"><u>Form-B</u></b></a><?php } ?>
...
<!-- Emp Ledger Emp Ledger Emp Ledger Emp Ledger Emp Ledger Emp Ledger Emp Ledger Emp Ledger -->
<!-- Emp Ledger Emp Ledger Emp Ledger Emp Ledger Emp Ledger Emp Ledger Emp Ledger Emp Ledger -->
<?php if($CompanyId==1){ ?>
	 <tr>
	  <td style='background-color:#D7CCE3;width:70%;'>
<?php $EC2=$resE['EmpCode'];
$LEC2=strlen($resE['EmpCode']);
//if($LEC2==1){$ECL='E000'.$resE['EmpCode'];}
//elseif($LEC2==2){$ECL='E00'.$resE['EmpCode'];}
//elseif($LEC2==3){$ECL='E0'.$resE['EmpCode'];}
//elseif($LEC2==4){$ECL='E'.$resE['EmpCode'];}  
//else{$ECL='E'.$resE['EmpCode'];}
$ECL='E'.$resE['EmpCode'];
$_SESSION['ECL'] = $ECL; 

if($resE['VCode']=='V'){ $EC=$resE['EmpCode']; $fileLgr = 'Emp'.$CompanyId.'Lgr/'.$resE['EmpCode'].'.pdf'; }else{ $EC='E'.$resE['EmpCode']; $fileLgr = 'Emp'.$CompanyId.'Lgr/E'.$resE['EmpCode'].'.pdf'; }

if(file_exists($fileLgr)){ ?><img src="images/new_blink.gif" border="0"><a href="MyLgrFile.php?a=open&filepath=<?=$fileLgr?>&filename=<?=$EC.'.pdf'?>&no=<?=$ECL?>" style="text-decoration:none;" target="_blank"><b style="color:#FF0000;"><u>Ledger 2023-2024</u></b></a><?php } ?>
...
$EC3=$resE['EmpCode'];
$filename3 = 'HealthIDCard/'.$CompanyId.'/'.$EC3.'/'.$EC3.'_A.pdf';
...
<?php 
$json = file_get_contents('https://hrrec.vnress.in/get_job_opening');
...
<tr>    
 <td valign="top" align="center" style="width:100%; padding-left:7px;padding-right:7px; padding-top:0px; padding-bottom:3px;">
  ...
  <tr>
    ...
    <th style="width:50%;">Vacancy For</th>
  </tr>
  </thead>
  <tbody>
  <?php
  $i=1;
  foreach($data->regular_job as $key =>$value){
  ?>
  <tr>
    ...
    <td style="text-align:center;"><a href="https://hrrec.vnress.in/jobportal/jobs" target="_blank">Click here</a></td> 
