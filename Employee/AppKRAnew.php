...
<?php $sql=mysql_query("select e.EmployeeID,EmpCode,Fname,Sname,Lname,EmpPmsId,DepartmentCode,DesigName,HqName,StateName from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID INNER JOIN hrm_department d ON g.DepartmentId=d.DepartmentId INNER JOIN hrm_designation de ON g.DesigId=de.DesigId INNER JOIN hrm_headquater hq ON g.HqId=hq.HqId INNER JOIN hrm_state st ON hq.StateId=st.StateId where e.EmpStatus='A' AND p.AssessmentYear=".$rnKra['NewY']." AND p.Appraiser_EmployeeID=".$EmployeeId, $con); 
$sno=1; while($res=mysql_fetch_array($sql)){ 

$sql3E=mysql_query("select EmpStatus,AppStatus,RevStatus,UseKRA from hrm_pms_kra where YearId=".$rnKra['NewY']." AND EmployeeID=".$res['EmployeeID'], $con); $row3E=mysql_num_rows($sql3E); 
...
?> 
   ...
   <tr style="height:22px;background-color:#FFFFFF;">
	...
	<td class="tdc"><?php if($row3E>0){?><a href="#" onClick="EmpKRA(<?php echo $CompanyId.', '.$rnKra['NewY'].', '.$res['EmployeeID']; ?>)">Click</a><?php } ?></td>
	<?php if($_SESSION['Dept']==4){ ?><td class="tdc"><?php $sqlR=mysql_query("select * from hrm_employee_kra_uploadfile where EmployeeID=".$res['EmployeeID']." AND YearId=".$rnKra['NewY'], $con); $no=1; $resR=mysql_num_rows($sqlR); if($resR>0){ ?><a href="#" onClick="UploadEmpfile(<?php echo $CompanyId.', '.$rnKra['NewY'].', '.$res['EmployeeID']; ?>)">Yes</a><?php } if($resR==0){echo 'No'; }?></td><?php } ?> 

	 <td class="tdc"><?php if($row3E>0){ ?><a href="#"><img src="images/edit.png" border="0" onClick="javascript:window.location='AppKRAnewAdd.php?e=<?php echo $res['EmployeeID']?>&ee=0&aa=1&rr=1&hh=1&sh=1&hp=1&fr=1&kr=1&fq=1&prt=1&msg=1&pd=1&mt=1&mts=1&scr=1&prom=1&inc=1&incr=1&pmsr=1&rg=1&h=1&fachiv=0&fa=1&fb=1&ffeedb=1&org=1&nkra=1'"/></a><?php } if($res3E['UseKRA']=='A'){?>&nbsp;&nbsp;&nbsp;<a href="#"><img src="images/go-back-icon.png" border="0" onClick="return ResentKRA(<?php echo $res['EmployeeID'].', '.$rnKra['NewY'];?>)"/></a><?php } ?></td>
    </tr>
    </tbody>
    </div>
<?php $sno++;} ?>		
