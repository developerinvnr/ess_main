...
{$LDate=date("Y-09-30");}
	 ...
	 <?php 
	 if($_REQUEST['kid']==21348)
	 { 
	     //echo $PerM.'-'.$res['lockk'].'-'.$Nxt10Day; 
	 
	 } 
	 ?>
	 
	  <img src="images/edit.png" border="0" id="edit_<?php echo $i; ?>" style="display:<?php if(($PerM==1 AND $res['lockk']==0 AND date("Y-m-d")<=$Nxt10Day) OR ($PerM==1 AND $res['lockk']==2 AND date("Y-m-d")<=$Nxt14Day) OR ($PerM==1 AND $res['lockk']==0 AND $rpms['UnLckKRA']==1) OR ($PerM==1 AND $res['lockk']==0 AND $res['Applockk']==0 AND $res['AppRevert']=='Y' AND date("Y-m-d")<=$Nxt14Day) OR ($res['Cmnt']=='' AND ($res['Ach']=='' OR $res['Ach']==0) AND $res['Wgt']!='' AND $res['Wgt']!=0)){echo 'block';}else{echo 'block';} ?>" onClick="FunEdit(<?php echo $i; ?>)" />
