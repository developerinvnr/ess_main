We are pleased to inform you that your performance for the assessment period <b><?php echo $Ass2Year; ?></b> has been rated as <b>"<?php echo $ResR['RatingName']; ?>"</b> with "<b>Rating-3.9</b>".<?php /*?><b>"<?php echo $ResR['RatingName']; ?>"</b> with "<b>Rating-3.9</b>".<?php */?> <p>   
          ...
          <?php  if($_REQUEST['E']==209 AND $_REQUEST['Y']==11){ ?>
		   Based on your performance, there is an increase in your CTC by <b>Rs. 189908/-</b>.<p> 
		   We are also pleased to announce that you have achieved the first milestone of the Utkrisht reward being given for your consistent performance towards company’s research programs. 
		   
		   You are being rewarded by <b>Rs. 791300/-</b>.  <p>                    
           <?php } else { ?>           
           ...
           Based on your performance, there is an <?php echo $cmt;?> <b>Rs. <?php echo floatval($IncVal); ?>/- </b>.<?php if($_REQUEST['C']==1 AND $ResE['DateJoining']>=$FD2.'-01-01' AND $ResE['DateJoining']<=$FD2.'-06-30') { ?> As your service is less than a year in the assessment cycle <?php echo $FD2; ?>, your increment is calculated on pro-rata basis as per the duration served during this period against the actual increment of <b><?php echo intval(($ResPMS['EmpCurrCtc']*$resMaxMin['IncDistriFrom'])/100); //floatval($ResPMS['HR_IncNetCTC']); ?>/-</b> as per your performance rating.<p><?php } ?><?php if($_REQUEST['C']==1 AND $ResE['DateJoining']>=$FD1.'-07-01' AND $ResE['DateJoining']<=$FD1.'-12-31') { ?> As your assessment tenure is more than a year (DOJ on or after 1st July <?php echo $FD1; ?>), your increment is calculated on pro-rata basis for the duration served since your joining against the actual annual increment for assessment cycle<?php //echo $AssYear; ?> that is <b><?php if($_REQUEST['E']==1327 AND $_REQUEST['Y']==11){ echo '57001'; }else{ echo intval(($ResPMS['EmpCurrCtc']*$resMaxMin['IncDistriFrom'])/100); } //floatval($ResPMS['HR_IncNetCTC']); ?>/-</b> as per your performance rating.<p><?php } ?>
