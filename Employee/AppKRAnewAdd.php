     ...
     for($i=1; $i<=$_POST['EditTNRow']; $i++)
      ...
      if($sqlSaveK)
	  {
	   ...
	   if($TotalWeight==100)
       {
	    ...
	    { 
		  $sqlU=mysql_query("update hrm_pms_kra set KRAStatus='R',UseKRA='R',AppStatus='A' where YearId=".$rnKra['NewY']." AND EmployeeID=".$_REQUEST['e'], $con); 
...
<?php $sqlEK=mysql_query("select * from hrm_employee_pms_kraforma k INNER JOIN hrm_employee_pms p ON k.EmpPmsId=p.EmpPmsId where p.EmployeeID=".$_REQUEST['e']." AND YearId=".$rnKra['CurrY'], $con); $rowEK=mysql_num_rows($sqlEK); 
...
<?php $sql=mysql_query("select * from hrm_pms_kra where YearId=".$rnKra['NewY']." AND CompanyId=".$CompanyId." AND EmployeeID=".$_REQUEST['e']." AND (KRAStatus='R' OR KRAStatus='A') order by KRAId ASC", $con); $row=mysql_num_rows($sql); $sql2=mysql_query("select * from hrm_pms_kra where YearId=".$rnKra['NewY']." AND CompanyId=".$CompanyId." AND EmployeeID=".$_REQUEST['e']." AND (KRAStatus='R' OR KRAStatus='A') AND (UseKRA='R' OR UseKRA='H') order by KRAId ASC", $con); $row2=mysql_num_rows($sql2); ?>
...
<input type="hidden" id="row2" value="<?php echo $row2; ?>" />       	
	...
	function FunCalculator()
	{
	 ...
	 <?php //} ?>    
		...
		<input type="button" name="back" id="back" style="width:100%;" value="back" onClick="javascript:window.location='AppKRAnew.php?ee=0&aa=1&rr=1&hh=1&sh=1&hp=1&fr=1&kr=1&fq=1&prt=1&msg=1&pd=1&mt=1&mts=1&scr=1&prom=1&inc=1&incr=1&pmsr=1&rg=1&h=1&fachiv=0&fa=1&fb=1&ffeedb=1&org=1&nkra=1'"/></td>
