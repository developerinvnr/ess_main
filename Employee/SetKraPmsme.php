...
<tr>
 <td>
 <?php if($_SESSION['Joining']<=$AllowDoj){ ?>     
 ...
 <?php if($_SESSION['NewY_AllowEntry']=='Y'){ ?>
 
  <?php if($_SESSION['eKraform']=='Y' AND $CuDate>=$_SESSION['ekFromn'] AND $CuDate<=$_SESSION['ekTon'] AND $_SESSION['ekStsn']=='A'){ $LastDate=$_SESSION['ekTon']; $CurrentDate=date("Y-m-d");
		 $diff = abs(strtotime($LastDate) - strtotime($CurrentDate));
         $days = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/(60*60*24)); ?>
 <font class="msg_r"><font color="#00366C">KRA :</font><span class="blink_me"> <?php echo $days;?> Days Remaining! Last date : <?php echo date("d-F",strtotime($_SESSION['ekTon'])); ?> </span></font>
 <?php } ?>
...
function LetterAllPdf(P,E,Y,C,R,G,D,DP)
{
 if(DP==2 || DP==4 || DP==6)
 {
  window.open("pmsletter/"+DP+"/VeiwAppAllPdf.php?action=All&test=true&rightform=false&cc=102&prp=55&rtr=%true%&ff=ok&P="+P+"&E="+E+"&Y="+Y+"&C="+C+"&R="+R+"&G="+G+"&D="+D,"AppLetter","scrollbars=yes,resizable=no,menubar=yes,width=820,height=750,location=no,directories=no");
 }   
 else
 {
...
<?php //if($EmployeeId==169 OR $EmployeeId==142){ ?><td class="tdc"><a href="#" onClick="LetterAllPdf(<?php echo $_SESSION['ePmsId'].','.$EmployeeId.','.$_SESSION['PmsYId'].','.$CompanyId.','.$EmpRating; ?>,'<?php echo $EmpGrade; ?>',<?php echo $Desig; ?>,<?=$rdp['DepartmentId']?>)"><span class="blink_me"><img src="images/AppLet.png" border="0"/></span></a>&nbsp;</td><?php //} ?>
...
<td class="tdc"><?php $sLk=mysql_query("select NewY,NewY_AllowEntry from hrm_pms_setting where CompanyId=".$CompanyId." AND Process='KRA'",$con); $rLk=mysql_fetch_assoc($sLk); if($rLk['NewY_AllowEntry']=='Y'){ ?><a href="EmpKRAnew.php?ee=0&aa=1&rr=1&hh=1&sh=1&hp=1&fr=1&kr=1&fq=1&prt=1&msg=1&pd=1&mt=1&mts=1&scr=1&prom=1&inc=1&incr=1&pmsr=1&rg=1&h=1&fachiv=0&fa=1&fb=1&ffeedb=1&org=1&nkr=1"/><?php if($_REQUEST['nkr']==1){ ?><img src="images/NewKRA1.png" border="0"/><?php }else{ ?><img src="images/NewKRA.png" border="0"/><?php } ?></a><?php } ?></td>
