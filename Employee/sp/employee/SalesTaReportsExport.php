...
{
  $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_headquater hq ON ".$Hqj." INNER JOIN hrm_sales_reporting_level rl ON ".$Rlj." where d.DealerSts='A' AND (".$TerrCon."=".$_REQUEST['EmpId']." OR rl.R1=".$_REQUEST['EmpId']." OR rl.R2=".$_REQUEST['EmpId']." OR rl.R3=".$_REQUEST['EmpId']." OR rl.R4=".$_REQUEST['EmpId']." OR rl.R5=".$_REQUEST['EmpId'].") AND hq.StateId=".$_REQUEST['SteName']." AND sd.YearId=".$yset." AND DealerSts='A' AND sp.ProductSts='A' AND ".$qin."", $con); 
}
elseif($_REQUEST['NameWise']>0)
{
 if($_REQUEST['SelName']>0)
...
if($_REQUEST['SteWise']>0 AND $_REQUEST['SteName']>0)
{ 
 $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_headquater hq ON ".$Hqj." INNER JOIN hrm_sales_reporting_level rl ON ".$Rlj." where d.DealerSts='A' AND (".$TerrCon."=".$_REQUEST['EmpId']." OR rl.R1=".$_REQUEST['EmpId']." OR rl.R2=".$_REQUEST['EmpId']." OR rl.R3=".$_REQUEST['EmpId']." OR rl.R4=".$_REQUEST['EmpId']." OR rl.R5=".$_REQUEST['EmpId'].") AND hq.StateId=".$_REQUEST['SteName']." AND sd.ProductId=".$res['ProductId']." AND d.DealerSts='A' AND sp.ProductSts='A' AND sd.YearId=".$yset, $con); 
}
elseif($_REQUEST['NameWise']>0)
{ 
 if($_REQUEST['SelName']>0)
