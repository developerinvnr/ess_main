...
if($EmployeeId==1315)
{ 
$result=mysql_query("select * from fa_details where (Remove_Reporting_AppSts=".$_REQUEST['rsts']." OR Remove_Rev_AppSts=".$_REQUEST['rsts']." OR Remove_Hod_AppSts=".$_REQUEST['rsts'].") AND ".$sqry." AND (FaStatus='A' OR (FaStatus='D' AND RemoveSts=3)) AND RemoveSts>=1 order by FaId ASC",$con); 
}
else
{
...
if($EmployeeId==1315)
{
$sql=mysql_query("select * from fa_details where (Remove_Reporting_AppSts=".$_REQUEST['rsts']." OR Remove_Rev_AppSts=".$_REQUEST['rsts']." OR Remove_Hod_AppSts=".$_REQUEST['rsts'].") AND ".$sqry." AND (FaStatus='A' OR (FaStatus='D' AND RemoveSts=3)) AND RemoveSts>=1 order by FaId ASC LIMIT ".$from.",".$offset,$con);
}
else
{
...
<td class="tdf2"><input class="InputType" style="width:100px;" id="Hq<?php echo $sn; ?>" value="<?php echo ucfirst(strtolower($rhq['HqName'])); ?>" readonly/></td>
	 ...
	 */ ?>
	 ...
	 <?php if($EmployeeId==1315){ ?>
	 
	 <select style="width:80px;" class="InputSel" onChange="FucApp(this.value,<?php echo $sn.','.$res['FaId'].', '.$_REQUEST['m'].','.$_REQUEST['y'].','.$EmployeeId.','.$res['Remove_Reporting'].','.$res['Remove_Rev'].','.$res['Remove_Hod'].','.$res['RemoveReqBy'].','.$res['Remove_lvl']; ?>)"><option value="" selected>Select</option><option value="2">Approve</option><option value="3">Reject</option></select>
	 
	 <?php }else{ ?>
	 
	 <?php if(($res['Remove_Reporting']==$EmployeeId && ($res['Remove_Reporting_AppSts']==0 OR $res['Remove_Reporting_AppSts']==1)) OR ($res['Remove_Rev']==$EmployeeId && ($res['Remove_Rev_AppSts']==0 OR $res['Remove_Rev_AppSts']==1)) OR ($res['Remove_Hod']==$EmployeeId && ($res['Remove_Hod_AppSts']==0 OR $res['Remove_Hod_AppSts']==1))){ ?><select style="width:80px;" class="InputSel" onChange="FucApp(this.value,<?php echo $sn.','.$res['FaId'].', '.$_REQUEST['m'].','.$_REQUEST['y'].','.$EmployeeId.','.$res['Remove_Reporting'].','.$res['Remove_Rev'].','.$res['Remove_Hod'].','.$res['RemoveReqBy'].','.$res['Remove_lvl']; ?>)"><option value="" selected>Select</option><option value="2">Approve</option><option value="3">Reject</option></select><?php } ?>
