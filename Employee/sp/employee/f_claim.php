...
if($EmployeeId==1315)
{
$result=mysql_query("select * from fa_notaccemp nf inner join fa_details fd on nf.RepEmpId=fd.Reporting where ((FaStatus='A' AND (LWD='0000-00-00' OR LWD='1970-01-01' OR LWD>='".date($y."-".$m."-01")."')) OR (FaStatus='D' AND LWD>='".date($y."-".$m."-01")."')) AND DOJ<='".date($y."-".$m."-31")."' AND ".$qry." group by FaId order by FaId ASC",$con);
}
else
{
...
if($EmployeeId==1315)
{
$sql=mysql_query("select * from fa_notaccemp nf inner join fa_details fd on nf.RepEmpId=fd.Reporting where ((FaStatus='A' AND (LWD='0000-00-00' OR LWD='1970-01-01' OR LWD>='".date($y."-".$m."-01")."')) OR (FaStatus='D' AND LWD>='".date($y."-".$m."-01")."')) AND DOJ<='".date($y."-".$m."-31")."' AND ".$qry." group by FaId order by FaId ASC LIMIT ".$from.",".$offset,$con);
}
else
{
