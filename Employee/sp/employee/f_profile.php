...
if($EmployeeId==1315)
{
  ...
  if($rowee>0)
 {
  if($_REQUEST['md']==4){ $sql=mysql_query("select * from fa_notaccemp nf inner join fa_details fd on nf.EmpId=fd.Reporting where (FaStatus='A' OR FaStatus='D') order by FaId ASC LIMIT ".$from.",".$offset,$con); }
  elseif($_REQUEST['md']!=4){ $sql=mysql_query("select * from fa_notaccemp nf inner join fa_details fd on nf.EmpId=fd.Reporting where (FaStatus='A' OR FaStatus='D') AND Mode=".$_REQUEST['md']." order by FaId ASC LIMIT ".$from.",".$offset,$con); }
 }
 else
 {
  if($_REQUEST['md']==4){ $sql=mysql_query("select * from fa_details fd where (FaStatus='A' OR FaStatus='D') order by FaId ASC LIMIT ".$from.",".$offset,$con); }
  elseif($_REQUEST['md']!=4){ $sql=mysql_query("select * from fa_details fd where (FaStatus='A' OR FaStatus='D') AND Mode=".$_REQUEST['md']." order by FaId ASC LIMIT ".$from.",".$offset,$con); }
 }    
