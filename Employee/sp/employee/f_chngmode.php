...
if($EmployeeId==1315)
{
$result=mysql_query("select * from fa_details fd where ".$qry." AND (FaStatus='A' OR (FaStatus='D' AND MdChngSts=3)) order by FaId ASC",$con);
}
else
{
$result=mysql_query("select * from fa_details fd inner join hrm_sales_reporting_level rl on fd.Reporting=rl.EmployeeID where (fd.Reporting=".$EmployeeId." OR rl.R1=".$EmployeeId." OR rl.R2=".$EmployeeId." OR rl.R3=".$EmployeeId." OR rl.R4=".$EmployeeId." OR rl.R5=".$EmployeeId.") AND ".$qry." AND (FaStatus='A' OR (FaStatus='D' AND MdChngSts=3)) order by FaId",$con);    
}
...
if($EmployeeId==1315)
{
$sql=mysql_query("select * from fa_details fd where ".$qry." AND (FaStatus='A' OR (FaStatus='D' AND MdChngSts=3)) order by FaId ASC LIMIT ".$from.",".$offset,$con);
}
else
{
