...
?>     
     ...
     <?php $sDis=mysql_query("select DealerId,DealerName,DealerCity from hrm_sales_dealer where ".$Dqry." and DealerSts='A' group by DealerId order by DealerName ASC", $con); 
	 while($rDis=mysql_fetch_assoc($sDis)){ ?><option value="<?php echo $rDis['DealerId']; ?>"><?php echo strtoupper($rDis['DealerName']).'-'.$rDis['DealerCity']; ?></option><?php } ?>
	 <?php } ?>	 
	 </select>
	</td>
	<td style="width:1px;"></td>
...
{
 $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_seedsitem si ON sp.ItemId=si.ItemId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_headquater hq ON ".$Hqj." INNER JOIN hrm_sales_reporting_level rl ON ".$Rlj." where d.DealerSts='A' AND (".$TerrCon."=".$EmployeeId." OR rl.R1=".$EmployeeId." OR rl.R2=".$EmployeeId." OR rl.R3=".$EmployeeId." OR rl.R4=".$EmployeeId." OR rl.R5=".$EmployeeId.") AND hq.StateId=".$_REQUEST['SteName']." AND sd.YearId=".$yset." AND sp.ProductSts='A' AND ".$qin."", $con); 
}
...
{ 
 $sqlPd=mysql_query("select ".$subquery." from hrm_sales_sal_details_".$yset." sd INNER JOIN hrm_sales_seedsproduct sp ON sd.ProductId=sp.ProductId INNER JOIN hrm_sales_dealer d ON sd.DealerId=d.DealerId INNER JOIN hrm_headquater hq ON ".$Hqj." INNER JOIN hrm_sales_reporting_level rl ON ".$Rlj." where d.DealerSts='A' AND (".$TerrCon."=".$EmployeeId." OR rl.R1=".$EmployeeId." OR rl.R2=".$EmployeeId." OR rl.R3=".$EmployeeId." OR rl.R4=".$EmployeeId." OR rl.R5=".$EmployeeId.") AND hq.StateId=".$_REQUEST['SteName']." AND sd.ProductId=".$res['ProductId']." AND d.DealerSts='A' AND sp.ProductSts='A' AND sd.YearId=".$yset, $con); 
}
elseif($_REQUEST['NameWise']>0)
{ 
 if($_REQUEST['SelName']>0)
