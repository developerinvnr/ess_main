...
<input type="hidden" name="wgt" id="wgt" value="<?php echo $_REQUEST['wgt']; ?>"/>
	...
	<td align="center">
	 <?php $Nxt10Day=date("Y-m-d",strtotime('+14 day', strtotime($res['Ldate'])));  //date("2019-01-30") +14 day 
	 
	    //$Nxt10Day=date("2023-01-20");
	 ?>
	 <?php if($res['Tital']!='' AND $res['Tgt']!==''){ ?>
	  ...
	  <span style="cursor:pointer"><img src="images/edit.png" border="0" id="edit_<?php echo $i; ?>" style="display:<?php if(($res['Applockk']==0 AND date("Y-m-d")<=$Nxt10Day) OR (($res['AppAch']==0 OR $res['AppAch']=='') AND $res['AppCmnt']=='')){echo 'block';}else{echo 'block';} ?>;" onClick="FunEdit(<?php echo $i; ?>)"/></span>
