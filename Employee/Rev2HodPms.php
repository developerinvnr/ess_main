 ...
 <td style="width:100%;">
  <table border="0" style="width:100%;">
   <tr>
    ...
    while($ResHQ=mysql_fetch_array($SqlHQ)) { ?><option value="<?php echo $ResHQ['id']; ?>"><?php echo $ResHQ['city_village_name'];?></option><?php } ?><option value="All">All</option></select>
...
<?php $sql=mysql_query("select e.EmployeeID, EmpCode, Fname, Sname, Lname, department_code as DepartmentCode, designation_name as DesigName, grade_name as GradeValue, city_village_name as HqName, state_name as StateName, EmpPmsId, Emp_PmsStatus, Appraiser_EmployeeID, Appraiser_PmsStatus, Reviewer_EmployeeID, Reviewer_PmsStatus from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID left join core_departments dept on g.DepartmentId=dept.id
  ...
  left join core_city_village_by_state vlg on g.HqId=vlg.id where e.EmpStatus='A' AND p.AssessmentYear=".$_SESSION['PmsYId']." AND p.Rev2_EmployeeID=".$EmployeeId." order by e.ECode", $con); $sno=1; while($res=mysql_fetch_array($sql)){ ?>
