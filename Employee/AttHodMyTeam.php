...
<?php $SqlEmp=mysql_query("SELECT hrm_employee.EmployeeID,DepartmentId,DesigId,EmpCode,Fname,Sname,Lname,Married,Gender,DR FROM hrm_employee_reporting r INNER JOIN hrm_employee e ON r.EmployeeID=e.EmployeeID INNER JOIN hrm_employee_general g ON r.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_personal p ON r.EmployeeID=p.EmployeeID WHERE (r.HodId=".$EmployeeId." OR r.ReviewerId=".$EmployeeId.") AND e.EmpStatus='A' AND e.CompanyId=".$CompanyId." order by e.ECode ASC", $con); $Sno=1; while($ResEmp=mysql_fetch_array($SqlEmp)) { 
