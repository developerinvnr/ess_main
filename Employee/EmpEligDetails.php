...
function SelectDept(v,cid){ window.location='EmpEligDetails.php?action=DeptElig&value='+v+'&cid='+cid;}
	 ...
	 <tr>
	  <td valign="top">
	     <table border="0" style="width:100%; height:380px; float:none;" cellpadding="0">
		  <tr><td class="thh">Eligibility: &nbsp;<select style=" font-family:Times New Roman;font-size:12px;width:150px;height:22px;background-color:#DDFFBB;" name="Dept" id="Dept" onChange="SelectDept(this.value,<?=$_REQUEST['cid']?>)"><option value="" style="margin-left:0px;" selected>Select Department</option><?php $SqlDept=mysql_query("select * from core_departments where is_active=1 group by department_name order by department_name ASC", $con); while($ResDept=mysql_fetch_array($SqlDept)){?>
<option value="<?php echo $ResDept['id']; ?>" <?php if($ResDept['id']==$_REQUEST['value']){echo 'selected';} ?>><?php echo '&nbsp;'.$ResDept['department_name'];?></option><?php } ?><option value="All">&nbsp;All</option></select>

&nbsp;&nbsp;
<a href="https://www.vnress.in/Employee/EmpEligNext.php?ID=<?=$EmployeeId?>" target="_blank" style="font-size:12px;">Other Team Group</a>

&nbsp;&nbsp;
<a href="OthDet.php?value=<?=$_REQUEST['value']?>&c=<?=$_REQUEST['cid']?>" target="_blank" style="font-size:12px;">Other Details</a>
...
<?php if($_REQUEST['value']=='All') {$sql=mysql_query("select g.EmployeeID, EmpCode, Fname, Sname, Lname, g.DepartmentId, department_code as DepartmentCode, grade_name as GradeValue, eg.* from hrm_employee_general g INNER JOIN hrm_employee e ON g.EmployeeID=e.EmployeeID INNER JOIN hrm_employee_eligibility eg ON g.EmployeeID=eg.EmployeeID INNER JOIN core_departments d ON g.DepartmentId=d.id INNER JOIN core_grades gr ON g.GradeId=gr.id where e.CompanyId=".$_REQUEST['cid']." AND e.EmpStatus='A' AND eg.Status='A' order by e.ECode ASC", $con); }
else {$sql=mysql_query("select g.EmployeeID, EmpCode, Fname, Sname, Lname, g.DepartmentId, department_code as DepartmentCode, grade_name as GradeValue, eg.* from hrm_employee_general g INNER JOIN hrm_employee e ON g.EmployeeID=e.EmployeeID INNER JOIN hrm_employee_eligibility eg ON g.EmployeeID=eg.EmployeeID INNER JOIN core_departments d ON g.DepartmentId=d.id INNER JOIN core_grades gr ON g.GradeId=gr.id where g.DepartmentId=".$_REQUEST['value']." AND e.CompanyId=".$_REQUEST['cid']." AND e.EmpStatus='A' AND eg.Status='A' order by e.ECode ASC", $con); } 
