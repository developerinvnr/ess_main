...
while($ResDept=mysql_fetch_array($SqlDept)) { ?><option value="<?php echo $ResDept['HR_Curr_DepartmentId']; ?>"><?php echo $ResDept['DepartmentName'];?></option><?php } ?><option value="All">All</option></select></td>
    ...
    <td class="td1" style="width:150px;"><select class="tdsel" name="StE" id="StE" onChange="SelectStateEmp2(this.value,<?php echo $EmployeeId.', '.$_SESSION['PmsYId'].', '.$CompanyId; ?>)"><option value="" selected>Select State</option><?php $SqlSt=mysql_query("select st.* from core_state st inner join core_city_village_by_state hq on st.id=hq.state_id inner join hrm_employee_general g on hq.id=g.HqId inner join hrm_employee_pms pms on g.EmployeeID=pms.EmployeeID where pms.Reviewer_EmployeeID=".$EmployeeId." AND (pms.AssessmentYear=".$_SESSION['PmsYId']." OR pms.AssessmentYear=".$_SESSION['KraYId'].") group by st.state_name order by st.state_name ASC");
    while($ResSt=mysql_fetch_array($SqlSt)) { ?><option value="<?php echo $ResSt['id']; ?>"><?php echo $ResSt['state_name'];?></option><?php } ?><option value="All">All</option></select></td>
    
    <td class="fhead" style="width:100px;">Head Quarter :</td>
 <td class="td1" style="width:150px;"><select class="tdsel" name="HQE" id="HQE" onChange="SelectHQEmp2(this.value,<?php echo $EmployeeId.', '.$_SESSION['PmsYId'].', '.$CompanyId ?>)"><option value="" style="margin-left:0px; background-color:#84D9D5;" selected>Head Quarter</option><?php 
 $SqlHQ=mysql_query("select hq.* from core_city_village_by_state hq inner join hrm_employee_general g on hq.id=g.HqId inner join hrm_employee_pms pms on g.EmployeeID=pms.EmployeeID where pms.Reviewer_EmployeeID=".$EmployeeId." group by hq.id order by hq.city_village_name ASC", $con);
 while($ResHQ=mysql_fetch_array($SqlHQ)) { ?><option value="<?php echo $ResHQ['id']; ?>"><?php echo $ResHQ['city_village_name'];?></option><?php } ?><option value="All">All</option></select></td>
  <td>&nbsp;</td>
...
$sql=mysql_query("select e.EmployeeID, EmpCode, Fname, Sname, Lname, g.DateJoining, g.DateConfirmationYN, department_code as DepartmentCode, designation_name as DesigName, grade_name as GradeValue, city_village_name as HqName, state_name as StateName, EmpPmsId, Kra_filename, Kra_ext, Emp_PmsStatus, Appraiser_PmsStatus, Reviewer_PmsStatus, Rev2_PmsStatus, Emp_TotalFinalRating, Appraiser_TotalFinalRating, Reviewer_TotalFinalRating from hrm_employee e INNER JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID INNER JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID left join core_sub_department_master subdept on g.SubDepartmentId=subdept.id
  ...
  left join core_city_village_by_state vlg on g.HqId=vlg.id where e.EmpStatus='A' AND p.AssessmentYear=".$_SESSION['PmsYId']." AND p.Rev2_EmployeeID=".$EmployeeId." order by e.ECode", $con); 
