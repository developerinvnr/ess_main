<?php 
require_once('../AdminUser/config/config.php');  $EmployeeId=$_REQUEST['EI'];?>
...
<?php $sqlHi2=mysql_query("select SalaryChange_Date, Previous_GrossSalaryPM, Proposed_Designation, Proposed_Grade, TotalProp_GSPM, Proposed_GrossSalaryPM, TotalProp_PerInc_GSPM, Rating from hrm_pms_appraisal_history where EmpCode=".$resE['EmpCode']." AND CompanyId=".$resE['CompanyId']." AND SalaryChange_Date>='".date('2012-01-01')."' order by SalaryChange_Date DESC", $con); 
	  ...
	  ?>
       <tr style="height:22px;background-color:#FFFFFF;">
		...
		<td class="datac">
<?php if($resHi2['TotalProp_PerInc_GSPM']>0){ echo $resHi2['TotalProp_PerInc_GSPM']; }
/*
elseif($resHi2['TotalProp_PerInc_GSPM']==0)
{ 
  
  if($resHi2['Previous_GrossSalaryPM']!=$resHi2['TotalProp_GSPM'] AND $resHi2['Previous_GrossSalaryPM']>0 AND $resHi2['TotalProp_GSPM']>0)
		{
...
<?php $sqlEx=mysql_query("select * from hrm_employee_experience where EmployeeID=".$EmployeeId." order by ExpFromDate DESC", $con); $Sno2=1; while($resEx=mysql_fetch_array($sqlEx)){ ?>	
       <tr style="height:22px;background-color:#FFFFFF;">
		...
		<td class="datac"><?php if($resEx['ExpFromDate']=='0000-00-00' OR $resEx['ExpFromDate']=='1970-01-01' OR $resEx['ExpFromDate']=='') { echo ''; }else {echo date("d-m-Y",strtotime($resEx['ExpFromDate'])); } ?></td>
		<td class="datac"><?php if($resEx['ExpToDate']=='0000-00-00' OR $resEx['ExpToDate']=='1970-01-01' OR $resEx['ExpToDate']=='') { echo ''; }else {echo date("d-m-Y",strtotime($resEx['ExpToDate'])); }?></td>
