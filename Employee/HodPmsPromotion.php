...
<?php  
if($_REQUEST['ord']==''){$ordn='order by ECode ASC';}elseif($_REQUEST['ord']==0){$ordn='order by HR_CurrGradeId DESC';}elseif($_REQUEST['ord']==1){$ordn='order by HR_CurrGradeId ASC';}
if($_REQUEST['FilD']>0 AND $_REQUEST['FilS']=='All')
{
 $sql=mysql_query("select e.EmployeeID, EmpCode, Fname, Sname, Lname, EmpPmsId, Appraiser_EmployeeID, Reviewer_EmployeeID, Appraiser_EmpDesignation, Appraiser_EmpGrade, Reviewer_EmpDesignation, HodSubmit_IncStatus, Reviewer_EmpGrade, Hod_EmpDesignation, Hod_EmpGrade, Appraiser_Justification, Reviewer_Justification, Hod_Justification, HR_CurrDesigId, HR_CurrGradeId, designation_code as DesigCode, designation_name as DesigName, grade_name as GradeValue, DepartmentId, g.EmpVertical from hrm_employee e LEFT JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID LEFT JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID LEFT JOIN core_designation de ON p.HR_CurrDesigId=de.DesigId LEFT JOIN hrm_grade gr ON p.HR_CurrGradeId=gr.GradeId where e.EmpStatus='A' AND g.DateJoining<='".$_SESSION['AllowDoj']."' AND p.AssessmentYear=".$_SESSION['PmsYId']." AND p.HOD_EmployeeID=".$EmployeeId." AND g.DepartmentId=".$_REQUEST['FilD']." ".$ordn."", $con);
}
elseif($_REQUEST['FilD']>0 AND $_REQUEST['FilS']>0)
{
 $sql=mysql_query("select e.EmployeeID, EmpCode, Fname, Sname, Lname, EmpPmsId, Appraiser_EmployeeID, Reviewer_EmployeeID, Appraiser_EmpDesignation, Appraiser_EmpGrade, Reviewer_EmpDesignation, HodSubmit_IncStatus, Reviewer_EmpGrade, Hod_EmpDesignation, Hod_EmpGrade, Appraiser_Justification, Reviewer_Justification, Hod_Justification, HR_CurrDesigId, HR_CurrGradeId, designation_code as DesigCode, designation_name as DesigName, grade_name as GradeValue, DepartmentId, g.EmpVertical from hrm_employee e LEFT JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID LEFT JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID LEFT JOIN hrm_headquater hq ON g.HqId=hq.HqId LEFT JOIN core_designation de ON p.HR_CurrDesigId=de.id LEFT JOIN core_grades gr ON p.HR_CurrGradeId=gr.id where e.EmpStatus='A' AND g.DateJoining<='".$_SESSION['AllowDoj']."' AND p.AssessmentYear=".$_SESSION['PmsYId']." AND p.HOD_EmployeeID=".$EmployeeId." AND g.DepartmentId=".$_REQUEST['FilD']." AND hq.StateId=".$_REQUEST['FilS']." ".$ordn."", $con);
}
elseif($_REQUEST['FilD']=='All' AND $_REQUEST['FilS']>0)
{
 $sql=mysql_query("select e.EmployeeID, EmpCode, Fname, Sname, Lname, EmpPmsId, Appraiser_EmployeeID, Reviewer_EmployeeID, Appraiser_EmpDesignation, Appraiser_EmpGrade, Reviewer_EmpDesignation, HodSubmit_IncStatus, Reviewer_EmpGrade, Hod_EmpDesignation, Hod_EmpGrade, Appraiser_Justification, Reviewer_Justification, Hod_Justification, HR_CurrDesigId, HR_CurrGradeId, designation_code as DesigCode, designation_name as DesigName, grade_name as GradeValue, DepartmentId, g.EmpVertical from hrm_employee e LEFT JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID LEFT JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID LEFT JOIN hrm_headquater hq ON g.HqId=hq.HqId LEFT JOIN core_designation de ON p.HR_CurrDesigId=de.id LEFT JOIN core_grades gr ON p.HR_CurrGradeId=gr.id where e.EmpStatus='A' AND g.DateJoining<='".$_SESSION['AllowDoj']."' AND p.AssessmentYear=".$_SESSION['PmsYId']." AND p.HOD_EmployeeID=".$EmployeeId." AND hq.StateId=".$_REQUEST['FilS']." ".$ordn."", $con);
}
elseif(($_REQUEST['FilD']=='All' AND $_REQUEST['FilS']=='All') OR ($_REQUEST['FilD']=='' AND $_REQUEST['FilS']==''))
{
 $sql=mysql_query("select e.EmployeeID, EmpCode, Fname, Sname, Lname, EmpPmsId, Appraiser_EmployeeID, Reviewer_EmployeeID, Appraiser_EmpDesignation, Appraiser_EmpGrade, Reviewer_EmpDesignation, HodSubmit_IncStatus, Reviewer_EmpGrade, Hod_EmpDesignation, Hod_EmpGrade, Appraiser_Justification, Reviewer_Justification, Hod_Justification, HR_CurrDesigId, HR_CurrGradeId, designation_code as DesigCode, designation_name as DesigName, grade_name as GradeValue, DepartmentId, g.EmpVertical from hrm_employee e LEFT JOIN hrm_employee_general g ON e.EmployeeID=g.EmployeeID LEFT JOIN hrm_employee_pms p ON e.EmployeeID=p.EmployeeID LEFT JOIN hrm_headquater hq ON g.HqId=hq.HqId LEFT JOIN core_designation de ON p.HR_CurrDesigId=de.id LEFT JOIN core_grades gr ON p.HR_CurrGradeId=gr.id where e.EmpStatus='A' AND g.DateJoining<='".$_SESSION['AllowDoj']."' AND p.AssessmentYear=".$_SESSION['PmsYId']." AND p.HOD_EmployeeID=".$EmployeeId." ".$ordn."", $con);
}
$sno=1; while($res=mysql_fetch_array($sql)){ 
$sql5=mysql_query("select designation_name as DesigCode from core_designation where id=".$res['Appraiser_EmpDesignation'], $con); 
$sql6=mysql_query("select designation_name as DesigCode from core_designation where id=".$res['Reviewer_EmpDesignation'], $con); 
$res5=mysql_fetch_assoc($sql5); $res6=mysql_fetch_assoc($sql6);
$sql7=mysql_query("select grade_name as GradeValue from core_grades where id=".$res['Appraiser_EmpGrade'], $con);
$sql8=mysql_query("select grade_name as GradeValue from core_grades where id=".$res['Reviewer_EmpGrade'], $con);  
...
<?php $sqlDm = mysql_query("SELECT DesigCode,DesigName from core_designation where DesigId=".$res['HR_CurrDesigId'],$con); 
	  ...
	  $sqlD = mysql_query("SELECT DesigCode,DesigName FROM core_designation WHERE DesigId=".$res['Hod_EmpDesignation'],$con); 
...
<?php $sqlG = mysql_query("SELECT GradeValue FROM hrm_grade WHERE GradeId=".$res['Hod_EmpGrade'], $con);
	 ...
	 <td class="tdc"><SPAN id="SpanEdit_<?php echo $sno; ?>">
	     <?php /*
	     <?php if($res['HodSubmit_IncStatus']!=2){ ?><img src="images/edit.png" border="0" onClick="ClickEdit(<?php echo $sno; ?>)"/><?php } ?></SPAN>
	 <SPAN id="SpanEditSave_<?php echo $sno; ?>" style="display:none;"><img src="images/Floppy-Small-icon.png" border="0" onClick="return ClickSaveEdit(<?php echo $sno.','.$res['EmpPmsId'].','.$res['EmployeeID']; ?>)"></SPAN>
	 */ ?>
	 </td>
	</tr>
	</tbody>
