...
$sql=mysql_query("select kdf.*,Fname,Sname,Lname,DepartmentId from hrm_pms_kra_tgtdefin kdf inner join hrm_employee_pms g on kdf.EmployeeID=g.EmployeeID inner join hrm_employee_general ge on kdf.EmployeeID=ge.EmployeeID inner join hrm_employee e on kdf.EmployeeID=e.EmployeeID where e.EmpStatus='A' AND g.Appraiser_EmployeeID=".$_REQUEST['e']." AND g.AssessmentYear=".$_REQUEST['y']." AND ".$Tit." AND kdf.Ldate>='".date("Y-01-31")."' order by e.EmpCode asc, kdf.TgtDefId asc, kdf.NtgtN asc ",$con); $row=mysql_num_rows($sql);
...
?>	 	
	<tr style="background-color:#FFFFFF;height:24px;">
	 ...
	 <td class="d" bgcolor="#D5FFAA" id="TD2<?php echo $i; ?>" valign="top"><textarea type="text" name="Cmnt<?php echo $i; ?>" id="Cmnt<?php echo $i; ?>" class="d" style="width:200px;background-color:#D5FFAA;"><?php echo $res['AppCmnt']; ?></textarea></td>
