...
<tr>
 ...
 <?php if($_SESSION['eKraform']=='Y' AND $CuDate>=$_SESSION['akFrom'] AND $CuDate<=$_SESSION['akTon'] AND $_SESSION['akStsn']=='A'){ $LastDate=$_SESSION['akTon']; $CurrentDate=date("Y-m-d");
		 $diff = abs(strtotime($LastDate) - strtotime($CurrentDate));
         $days = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/(60*60*24)); ?>
 <font class="msg_r"><font color="#00366C">KRA :</font><span class="blink_me"> <?php echo $days;?> Days Remaining! Last date : <?php echo date("d-F",strtotime($_SESSION['akTon'])); ?> </span></font>
 <?php } ?>
...
<td class="tdc"><?php  $sLk=mysql_query("select NewY,NewY_AllowEntry from hrm_pms_setting where CompanyId=".$CompanyId." AND Process='KRA'",$con); $rLk=mysql_fetch_assoc($sLk); if($rLk['NewY_AllowEntry']=='Y'){ ?><a href="AppKRAnew.php?ee=0&aa=1&rr=1&hh=1&sh=1&hp=1&fr=1&kr=1&fq=1&prt=1&msg=1&pd=1&mt=1&mts=1&scr=1&prom=1&inc=1&incr=1&pmsr=1&rg=1&h=1&fachiv=0&fa=1&fb=1&ffeedb=1&org=1&nkra=1"/><?php if($_REQUEST['nkra']==1){ ?><img src="images/TeamNewKRA1.png" border="0"/><?php }else{ ?><img src="images/TeamNewKRA.png" border="0"/><?php } ?></a><?php } ?></td>
